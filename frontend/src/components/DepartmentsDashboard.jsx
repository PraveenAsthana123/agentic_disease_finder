...
const CLINICAL_FORMS = {
  ...
  // ---- EEG Technician capture forms (13-section acquisition spec) ----
  eeg_acquisition: {
    label: 'EEG Acquisition + Device + Signal', table: 'eeg_acquisition',
    fields: [
      ...
      { k: 'vendor', label: 'Machine Vendor', type: 'text', placeholder: 'Nicolet' },
      { k: 'model', label: 'Machine Model', type: 'text', placeholder: 'NicoletOne' },
  ...
  recording_conditions: {
    ...
    fields: [
      ...
      { k: 'hyperventilation', label: 'Hyperventilation', type: 'select', options: ['No', 'Yes'] },
      { k: 'hv_duration', label: 'HV Duration (min)', type: 'number' },
      { k: 'photic', label: 'Photic Stimulation', type: 'select', options: ['No', 'Yes'] },
      { k: 'photic_freq', label: 'Photic Frequency Range', type: 'text', placeholder: '1-30 Hz' },
      { k: 'response', label: 'Response Observed', type: 'text', placeholder: 'None' },
    ],
  },
  clinical_history: {
    ...
    fields: [
      ...
      { k: 'stroke_history', label: 'Stroke History', type: 'select', options: ['No', 'Yes'] },
      { k: 'age_of_onset', label: 'Age of Onset', type: 'number' },
    ],
  },
  event_annotations: {
...
function EegAiStackPanel() {
  ...
  const chip = (t) => <span key={t.name} title={t.use || ''} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 10, margin: 2, display: 'inline-block',
    background: (col[t.status] || '#94a3b8') + '22', color: col[t.status] || '#94a3b8', border: `1px solid ${(col[t.status] || '#94a3b8')}55` }}>{t.status === 'installed' ? '✓ ' : ''}{t.name}</span>
  const s = d.summary || {}
  return (
    <div>
      <div style={card}>
        <h3 style={{ marginTop: 0, color: '#0f172a' }}>🧰 EEG AI Stack — {d.layers.length} layers</h3>
        ...
        <div style={{ fontSize: 11, color: '#166534', marginTop: 6 }}>🔗 Pipeline: {d.recommended_pipeline}</div>
      </div>
      {d.layers.map((L, i) => (
        <div key={i} style={{ ...card, marginBottom: 8 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#0f172a', marginBottom: 4 }}>{L.layer}</div>
          <div>{L.tools.map(chip)}</div>
        </div>
      ))}
      <div style={card}>
...
function StroopDashboardPanel() {
  ...
  return (
    <div>
      <div style={{ ...card, borderLeft: '4px solid #7c3aed' }}>
        <h3 style={{ marginTop: 0, color: '#0f172a' }}>🎯 Stroop Color-Word Test <span style={{ fontSize: 12, color: '#64748b' }}>— selective attention & inhibitory control</span></h3>
        <div style={{ fontSize: 12, color: '#475569', marginBottom: 12 }}>
          Three conditions: Word Reading → Color Naming → Interference (color-word conflict). Primary metric: interference score = time<sub>interference</sub> − time<sub>color</sub>. Higher = weaker inhibition.
        </div>
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 10 }}>
          {[['Patients', d.total_patients], ['Mean interference', `${d.mean_interference_score}s`], ['Impairment rate', `${d.impairment_rate_pct}%`]].map(([k, v]) => (
            <div key={k} style={{ padding: '8px 14px', borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
              <div style={{ fontSize: 11, color: '#475569' }}>{k}</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>{v}</div>
            </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e5e7eb' }}>
                {['Patient', 'Age', 'Disease', 'Word (s)', 'Color (s)', 'Interf (s)', 'Score', 'z', '%ile', 'Severity'].map(h => (
            ...
            <tbody>
              {(d.patients || []).map(p => (
                <tr key={p.patient_id} onClick={() => setSelPid(p.patient_id === selPid ? null : p.patient_id)}
                  ...
                  <td style={{ padding: '5px 8px' }}>{p.time_word}</td>
                  <td style={{ padding: '5px 8px' }}>{p.time_color}</td>
                  <td style={{ padding: '5px 8px' }}>{p.time_interference}</td>
                  <td style={{ padding: '5px 8px', fontWeight: 700 }}>{p.interference_score}</td>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace' }}>{p.z_score}</td>
                  <td style={{ padding: '5px 8px' }}>{p.percentile}</td>
        ...
        <div style={{ ...card, borderLeft: '4px solid #0ea5e9' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 12 }}>
            {(detail.conditions || []).map(c => (
              <div key={c.condition} style={{ padding: 10, borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#475569' }}>{c.condition}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: '#0f172a' }}>{c.time_sec}s</div>
                <div style={{ fontSize: 10, color: '#94a3b8' }}>norm: μ={c.norm_mean}s, σ={c.norm_sd}s</div>
                {c.errors > 0 && <div style={{ fontSize: 10, color: '#ef4444' }}>{c.errors} errors</div>}
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
            <div style={{ padding: '6px 12px', borderRadius: 8, background: '#f0f9ff', border: '1px solid #bae6fd' }}>
              <span style={{ fontSize: 11, color: '#475569' }}>Interference: </span>
              <span style={{ fontWeight: 700, color: '#0f172a' }}>{detail.interference_score}s</span>
            </div>
            <div style={{ padding: '6px 12px', borderRadius: 8, background: '#f0f9ff', border: '1px solid #bae6fd' }}>
              <span style={{ fontSize: 11, color: '#475569' }}>Ratio: </span>
              <span style={{ fontWeight: 700 }}>{detail.interference_ratio}</span>
            </div>
            <div style={{ padding: '6px 12px', borderRadius: 8, background: '#f0f9ff', border: '1px solid #bae6fd' }}>
              <span style={{ fontSize: 11, color: '#475569' }}>z-score: </span>
              <span style={{ fontWeight: 700, fontFamily: 'monospace' }}>{detail.z_score}</span>
            </div>
            <div style={{ padding: '6px 12px', borderRadius: 8, background: '#f0f9ff', border: '1px solid #bae6fd' }}>
              <span style={{ fontSize: 11, color: '#475569' }}>Percentile: </span>
              <span style={{ fontWeight: 700 }}>{detail.percentile}%</span>
            </div>
          ...
          {detail.contributing_factors && (
            <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {Object.entries(detail.contributing_factors).filter(([, v]) => v != null && v !== '' && !(Array.isArray(v) && v.length === 0)).map(([k, v]) => (
                  <span key={k} style={{ padding: '2px 8px', borderRadius: 6, fontSize: 10, background: '#f1f5f9', color: '#475569' }}>
                    {k.replace(/_/g, ' ')}: <strong>{Array.isArray(v) ? v.join(', ') : String(v)}</strong>
                  </span>
                ))}
              </div>
            </div>
          ...
          {detail.recommendations && (
            <div>
              ...
              {detail.recommendations.map((r, i) => (
                <div key={i} style={{ fontSize: 12, color: '#0f172a', marginBottom: 4, paddingLeft: 10, borderLeft: '2px solid #7c3aed' }}>{r}</div>
              ))}
            </div>
          )}
        </div>
      ...
      {trend && !trend.error && (
        <div style={{ ...card, borderLeft: '4px solid #22c55e' }}>
          <h4 style={{ marginTop: 0, color: '#0f172a' }}>📈 12-month projected interference trend — {trend.patient_name || trend.patient_id}</h4>
          <div style={{ fontSize: 12, color: '#475569', marginBottom: 8 }}>Baseline: {trend.baseline_interference}s ({trend.baseline_severity})</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
            {(trend.trajectory || []).map(pt => (
              <div key={pt.month} style={{ padding: '4px 8px', borderRadius: 6, background: (sevColors[pt.severity] || '#94a3b8') + '18', border: `1px solid ${sevColors[pt.severity] || '#cbd5e1'}30`, textAlign: 'center', minWidth: 52 }}>
                <div style={{ fontSize: 9, color: '#475569' }}>{pt.label}</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: sevColors[pt.severity] || '#0f172a' }}>{pt.projected_interference}s</div>
                <div style={{ fontSize: 8, color: '#94a3b8' }}>{pt.severity}</div>
              </div>
            ))}
          </div>
          {trend.assumptions && (
...
function WCSTDashboardPanel() {
  ...
  return (
    <div>
      <div style={{ ...card, borderLeft: '4px solid #7c3aed' }}>
        <h3 style={{ marginTop: 0, color: '#0f172a' }}>🧩 Wisconsin Card Sorting Test <span style={{ fontSize: 12, color: '#64748b' }}>— executive function: cognitive flexibility & set-shifting</span></h3>
        <div style={{ fontSize: 12, color: '#475569', marginBottom: 12 }}>
          Sort 128 cards by color, form, or number; the rule shifts without warning. Primary metric: Perseverative Errors (PE) — inability to shift mental set. Gold-standard frontal-lobe executive screen in epilepsy neuropsych.
        </div>
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 10 }}>
          {[['Patients', d.total_patients], ['Mean PE', d.mean_pe], ['Mean CC', `${d.mean_categories_completed}/6`], ['Impairment', `${d.impairment_rate_pct}%`]].map(([k, v]) => (
            <div key={k} style={{ padding: '8px 14px', borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
              <div style={{ fontSize: 11, color: '#475569' }}>{k}</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>{v}</div>
            </div>
      ...
      <div style={card}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e5e7eb' }}>
                {['Patient', 'Age', 'Disease', 'CC', 'PE', 'TE', 'CLR%', 'FMS', 'z(PE)', 'Overall'].map(h => (
                  <th key={h} style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 600, color: '#475569', fontSize: 11 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(d.patients || []).map(p => (
                <tr key={p.patient_id} onClick={() => setSelPid(p.patient_id === selPid ? null : p.patient_id)}
                    style={{ cursor: 'pointer', borderBottom: '1px solid #f1f5f9', background: p.patient_id === selPid ? '#f5f3ff' : 'transparent' }}>
                  <td style={{ padding: '5px 8px', fontWeight: 600, color: '#0f172a' }}>{p.name || p.patient_id}</td>
                  <td style={{ padding: '5px 8px', color: '#475569' }}>{p.age || '—'}</td>
                  <td style={{ padding: '5px 8px', color: '#475569' }}>{p.disease || '—'}</td>
                  <td style={{ padding: '5px 8px', fontWeight: 700 }}>{p.categories_completed}</td>
                  <td style={{ padding: '5px 8px', fontWeight: 700, fontFamily: 'monospace' }}>{p.perseverative_errors}</td>
                  <td style={{ padding: '5px 8px' }}>{p.total_errors}</td>
                  <td style={{ padding: '5px 8px' }}>{p.clr_pct}%</td>
                  <td style={{ padding: '5px 8px' }}>{p.fms}</td>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace' }}>{p.z_score_pe}</td>
                  <td style={{ padding: '5px 8px' }}>
                    <span style={{ padding: '2px 8px', borderRadius: 8, fontSize: 10, fontWeight: 600, background: (sevColors[p.overall_severity] || '#94a3b8') + '18', color: sevColors[p.overall_severity] || '#475569' }}>{p.overall_severity}</span>
                  </td>
                </tr>
        ...
        <div style={{ ...card, borderLeft: '4px solid #8b5cf6' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 10, marginBottom: 12 }}>
            {(detail.metrics || []).map(m => (
              <div key={m.metric} style={{ padding: 10, borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#475569' }}>{m.abbreviation}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: '#0f172a' }}>{m.value}{m.abbreviation === 'CLR%' ? '%' : ''}</div>
                <div style={{ fontSize: 10, color: '#94a3b8' }}>norm: μ={m.norm_mean}, σ={m.norm_sd} · z={m.z_score} · {m.percentile}%ile</div>
                <span style={{ padding: '2px 8px', borderRadius: 8, fontSize: 10, fontWeight: 600, background: (sevColors[m.severity] || '#94a3b8') + '18', color: sevColors[m.severity] || '#475569' }}>{m.severity}</span>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
          ...
          {detail.recommendations && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginBottom: 4 }}>Recommendations</div>
              {detail.recommendations.map((r, i) => (
                <div key={i} style={{ fontSize: 11, color: '#0f172a', marginBottom: 4, paddingLeft: 10, borderLeft: '2px solid #c4b5fd' }}>{r}</div>
              ))}
            </div>
          )}
        </div>
      ...
      {trend && !trend.error && (
        <div style={{ ...card, borderLeft: '4px solid #10b981' }}>
          <h4 style={{ marginTop: 0, color: '#0f172a' }}>📈 12-month PE trajectory — {trend.patient_name || trend.patient_id}</h4>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
            {(trend.trajectory || []).map(t => (
              <div key={t.month} style={{ padding: '4px 8px', borderRadius: 6, fontSize: 10, background: (sevColors[t.severity] || '#94a3b8') + '18', color: sevColors[t.severity] || '#475569', textAlign: 'center', minWidth: 50 }}>
                <div style={{ fontWeight: 600 }}>{t.label}</div>
                <div style={{ fontSize: 14, fontWeight: 700 }}>{t.projected_pe} PE</div>
                <div>{t.severity}</div>
              </div>
            ))}
          </div>
        ...
        <div style={card}>
          ...
          {defs.psychometrics && (
            <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {Object.entries(defs.psychometrics).map(([k, v]) => (
                  <span key={k} style={{ padding: '2px 8px', borderRadius: 6, fontSize: 10, background: '#f5f3ff', color: '#0f172a' }}>
                    {k.replace(/_/g, ' ')}: <strong>{v}</strong>
                  </span>
                ))}
              </div>
...
function NBackDashboardPanel() {
  ...
  return (
    <div>
      <div style={{ ...card, borderLeft: '4px solid #0ea5e9' }}>
        <h3 style={{ marginTop: 0, color: '#0f172a' }}>🔢 N-Back Working Memory Test <span style={{ fontSize: 12, color: '#64748b' }}>— working memory: hit rate, false alarms, d-prime, reaction time</span></h3>
        <div style={{ fontSize: 12, color: '#475569', marginBottom: 12 }}>
          Monitor a stimulus stream and respond when the current item matches the one n trials back. Primary metric: d-prime (signal detection sensitivity). Core paradigm for temporal-lobe and prefrontal working-memory circuits in epilepsy.
        </div>
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 10 }}>
          {[['Patients', d.total_patients], ["Mean d'", d.mean_dprime], ['Mean HR', `${d.mean_hit_rate}%`], ['Mean FAR', `${d.mean_far}%`], ['Impairment', `${d.impairment_rate_pct}%`]].map(([k, v]) => (
            <div key={k} style={{ padding: '8px 14px', borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
              <div style={{ fontSize: 11, color: '#475569' }}>{k}</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>{v}</div>
            </div>
      ...
      <div style={card}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e5e7eb' }}>
                {['Patient', 'Age', 'Disease', 'Hit Rate', 'FA Rate', "d'", 'RT (ms)', 'Acc%', 'Severity'].map(h => (
                  <th key={h} style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 600, color: '#475569', fontSize: 11 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(d.patients || []).map(p => (
                <tr key={p.patient_id} onClick={() => setSelPid(p.patient_id === selPid ? null : p.patient_id)}
                    style={{ cursor: 'pointer', borderBottom: '1px solid #f1f5f9', background: p.patient_id === selPid ? '#eff6ff' : 'transparent' }}>
                  <td style={{ padding: '5px 8px', fontWeight: 600, color: '#0f172a' }}>{p.patient_name || p.patient_id}</td>
                  <td style={{ padding: '5px 8px', color: '#475569' }}>{p.age || '—'}</td>
                  <td style={{ padding: '5px 8px', color: '#475569' }}>{p.disease || '—'}</td>
                  <td style={{ padding: '5px 8px', fontWeight: 700 }}>{p.hit_rate}%</td>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace' }}>{p.false_alarm_rate}%</td>
                  <td style={{ padding: '5px 8px', fontWeight: 700, fontFamily: 'monospace' }}>{p.dprime}</td>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace' }}>{p.reaction_time_ms}</td>
                  <td style={{ padding: '5px 8px' }}>{p.accuracy}%</td>
                  <td style={{ padding: '5px 8px' }}>
                    <span style={{ padding: '2px 8px', borderRadius: 8, fontSize: 10, fontWeight: 600, background: (sevColors[p.severity] || '#94a3b8') + '18', color: sevColors[p.severity] || '#475569' }}>{p.severity}</span>
                  </td>
                </tr>
      ...
      {detail && !detail.error && (
        <div style={{ ...card, borderLeft: '4px solid #38bdf8' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 10, marginBottom: 12 }}>
            {[
              { label: 'Hit Rate', abbr: 'HR%', value: `${detail.hit_rate}%`, z: detail.hr_z_score, norm: '82 ± 10' },
              { label: 'False Alarm', abbr: 'FAR%', value: `${detail.false_alarm_rate}%`, z: detail.far_z_score, norm: '12 ± 8' },
              { label: "d-prime", abbr: "d'", value: detail.dprime, z: detail.dprime_z_score, norm: '2.8 ± 0.7' },
              { label: 'Reaction Time', abbr: 'RT', value: `${detail.reaction_time_ms} ms`, z: null, norm: '520 ± 80' },
              { label: 'Accuracy', abbr: 'Acc%', value: `${detail.accuracy}%`, z: null, norm: '85 ± 8' },
            ].map(m => (
              <div key={m.abbr} style={{ padding: 10, borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#475569' }}>{m.abbr}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: '#0f172a' }}>{m.value}</div>
                <div style={{ fontSize: 10, color: '#94a3b8' }}>norm: {m.norm}{m.z != null ? ` · z=${m.z}` : ''}</div>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
            <div style={{ padding: '6px 12px', borderRadius: 8, background: (sevColors[detail.severity] || '#94a3b8') + '18', border: `1px solid ${sevColors[detail.severity] || '#cbd5e1'}40` }}>
              <span style={{ fontWeight: 700, color: sevColors[detail.severity] || '#475569' }}>Overall: {detail.severity}</span>
              <span style={{ fontSize: 11, color: '#64748b', marginLeft: 8 }}>{detail.dprime_percentile}th percentile</span>
            </div>
          </div>
          {detail.contributing_factors && (
            <div style={{ marginBottom: 10 }}>
          ...
          {detail.clinical_recommendations && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginBottom: 4 }}>Recommendations</div>
              {detail.clinical_recommendations.map((r, i) => (
                <div key={i} style={{ fontSize: 11, color: '#0f172a', marginBottom: 4, paddingLeft: 10, borderLeft: '2px solid #7dd3fc' }}>{r}</div>
              ))}
            </div>
          )}
        </div>
      ...
      {trend && !trend.error && (
        <div style={{ ...card, borderLeft: '4px solid #10b981' }}>
          <h4 style={{ marginTop: 0, color: '#0f172a' }}>📈 12-month d' trajectory — {trend.patient_name || trend.patient_id}</h4>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
            {(trend.trajectory || []).map(t => (
              <div key={t.month} style={{ padding: '4px 8px', borderRadius: 6, fontSize: 10, background: (sevColors[t.severity] || '#94a3b8') + '18', color: sevColors[t.severity] || '#475569', textAlign: 'center', minWidth: 50 }}>
                <div style={{ fontWeight: 600 }}>{t.label}</div>
                <div style={{ fontSize: 14, fontWeight: 700 }}>{t.projected_dprime}</div>
                <div>{t.severity}</div>
              </div>
            ))}
          </div>
        ...
        <div style={card}>
          ...
          {defs.psychometrics && (
            <div style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {Object.entries(defs.psychometrics).map(([k, v]) => (
                  <span key={k} style={{ padding: '2px 8px', borderRadius: 6, fontSize: 10, background: '#eff6ff', color: '#0f172a' }}>
                    {k.replace(/_/g, ' ')}: <strong>{v}</strong>
                  </span>
                ))}
              </div>
          ...
          {defs.epilepsy_context?.aed_effects && (
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginBottom: 4 }}>AED cognitive impact</div>
              <div style={{ fontSize: 11, color: '#475569' }}>{defs.epilepsy_context.aed_effects.effect_summary}</div>
              {defs.epilepsy_context.aed_effects.high_burden_aeds && (
                <div style={{ marginTop: 4 }}>
                  <span style={{ fontSize: 10, fontWeight: 600, color: '#dc2626' }}>High burden: </span>
                  <span style={{ fontSize: 10, color: '#475569' }}>{defs.epilepsy_context.aed_effects.high_burden_aeds.join(', ')}</span>
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div>
      <div style={{ ...card, borderLeft: '4px solid #2563eb' }}>
        <h3 style={{ marginTop: 0, color: '#0f172a' }}>📝 Rey Auditory Verbal Learning Test <span style={{ fontSize: 12, color: '#64748b' }}>— verbal learning & memory: encoding, retention, recognition</span></h3>
        <div style={{ fontSize: 12, color: '#475569', marginBottom: 12 }}>
          15-word list learned over 5 trials → interference list → immediate recall → 20-min delayed recall → recognition. Gold-standard verbal memory test for temporal-lobe epilepsy — hippocampal dysfunction directly impairs delayed recall.
        </div>
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 10 }}>
          {[['Patients', d.total_patients], ['Mean DR', d.mean_delayed_recall], ['Mean T1–T5', d.mean_total_learning], ['Impairment', `${d.impairment_rate_pct}%`]].map(([k, v]) => (
            <div key={k} style={{ padding: '8px 14px', borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
              <div style={{ fontSize: 11, color: '#475569' }}>{k}</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>{v}</div>
            </div>
      ...
      <div style={card}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e5e7eb' }}>
                {['Patient', 'Age', 'Disease', 'T1', 'T5', 'T1–T5', 'DR', 'Recog', 'Forget%', 'z(DR)', 'Overall'].map(h => (
                  <th key={h} style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 600, color: '#475569', fontSize: 11 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(d.patients || []).map(p => (
                <tr key={p.patient_id} onClick={() => setSelPid(p.patient_id === selPid ? null : p.patient_id)}
                  ...
                  <td style={{ padding: '5px 8px', color: '#475569' }}>{p.age || '—'}</td>
                  <td style={{ padding: '5px 8px', color: '#475569' }}>{p.disease || '—'}</td>
                  <td style={{ padding: '5px 8px' }}>{p.trial_1}</td>
                  <td style={{ padding: '5px 8px' }}>{p.trial_5}</td>
                  <td style={{ padding: '5px 8px', fontWeight: 700 }}>{p.total_learning}</td>
                  <td style={{ padding: '5px 8px', fontWeight: 700, fontFamily: 'monospace' }}>{p.delayed_recall}</td>
                  <td style={{ padding: '5px 8px' }}>{p.recognition_hits}/15</td>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace' }}>{p.forgetting_rate_pct}%</td>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace' }}>{p.dr_z_score}</td>
                  <td style={{ padding: '5px 8px' }}>
                    <span style={{ padding: '2px 8px', borderRadius: 8, fontSize: 10, fontWeight: 600, background: (sevColors[p.severity] || '#94a3b8') + '18', color: sevColors[p.severity] || '#475569' }}>{p.severity}</span>
                  </td>
                </tr>
      ...
      {detail && !detail.error && (
        <div style={{ ...card, borderLeft: '4px solid #3b82f6' }}>
          <h4 style={{ marginTop: 0, color: '#0f172a' }}>📋 Detail — {detail.patient_name || detail.patient_id}</h4>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 10, marginBottom: 12 }}>
            {[['T1', detail.trial_1], ['T5', detail.trial_5], ['T1–T5', detail.total_learning], ['List B', detail.list_b], ['T6', detail.trial_6], ['DR', detail.delayed_recall], ['Recog', `${detail.recognition_hits}/15`], ['Forget%', `${detail.forgetting_rate}%`], ['Slope', detail.learning_slope]].map(([k, v]) => (
              <div key={k} style={{ padding: 10, borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#475569' }}>{k}</div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
            <div style={{ padding: '6px 12px', borderRadius: 8, background: (sevColors[detail.severity] || '#94a3b8') + '18', border: `1px solid ${sevColors[detail.severity] || '#cbd5e1'}40` }}>
              <span style={{ fontWeight: 700, color: sevColors[detail.severity] || '#475569' }}>Overall: {detail.severity}</span>
              <span style={{ fontSize: 11, marginLeft: 8, color: '#475569' }}>z(DR)={detail.dr_z_score} · {detail.dr_percentile}%ile</span>
            </div>
          </div>
          {detail.clinical_interpretation && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginBottom: 4 }}>Memory profile</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {Object.entries(detail.clinical_interpretation).map(([k, v]) => (
                  <span key={k} style={{ padding: '3px 8px', borderRadius: 6, fontSize: 10, background: String(v).includes('Impaired') ? '#fef2f2' : '#f0fdf4', border: '1px solid #e5e7eb' }}>
          ...
          {detail.clinical_recommendations && detail.clinical_recommendations.length > 0 && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginBottom: 4 }}>Recommendations</div>
              {detail.clinical_recommendations.map((r, i) => (
                <div key={i} style={{ fontSize: 11, color: '#0f172a', marginBottom: 4, paddingLeft: 10, borderLeft: '2px solid #93c5fd' }}>{r}</div>
              ))}
            </div>
          )}
        </div>
      ...
      {trend && !trend.error && (
        <div style={{ ...card, borderLeft: '4px solid #10b981' }}>
          <h4 style={{ marginTop: 0, color: '#0f172a' }}>📈 12-month DR trajectory — {trend.patient_name || trend.patient_id}</h4>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
            {(trend.trajectory || []).map(t => (
              <div key={t.month} style={{ padding: '4px 8px', borderRadius: 6, fontSize: 10, background: (sevColors[t.severity] || '#94a3b8') + '18', color: sevColors[t.severity] || '#475569', textAlign: 'center', minWidth: 50 }}>
                <div style={{ fontWeight: 600 }}>{t.label}</div>
                <div style={{ fontSize: 14, fontWeight: 700 }}>{t.projected_dr} DR</div>
                <div>{t.severity}</div>
              </div>
            ))}
          </div>
...
function VerbalFluencyDashboardPanel() {
  const [d, setD] = useState(null)
  const [detail, setDetail] = useState(null)
  const [defs, setDefs] = useState(null)
  const [trend, setTrend] = useState(null)
  ...
  return (
    <div>
      <div style={{ ...card, borderLeft: '4px solid #7c3aed' }}>
        <h3 style={{ marginTop: 0, color: '#0f172a' }}>🗣️ Verbal Fluency Test (FAS + Category) <span style={{ fontSize: 12, color: '#64748b' }}>— phonemic & semantic word generation, executive function</span></h3>
        <div style={{ fontSize: 12, color: '#475569', marginBottom: 12 }}>
          Phonemic fluency (F-A-S letters, 60s each) measures frontal-executive lexical retrieval. Semantic fluency (Animals, 60s) measures temporal-lobe semantic store. FAS/Animals dissociation helps lateralize seizure focus — frontal epilepsy impairs FAS &gt; Animals; temporal epilepsy impairs Animals &gt; FAS.
        </div>
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 10 }}>
          {[['Patients', d.total_patients], ['Mean FAS', d.mean_fas_total], ['Mean Animals', d.mean_animals], ['Impairment', `${d.impairment_rate_pct}%`]].map(([k, v]) => (
            <div key={k} style={{ padding: '8px 14px', borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
              <div style={{ fontSize: 11, color: '#475569' }}>{k}</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>{v}</div>
            </div>
      ...
      <div style={card}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e5e7eb' }}>
                {['Patient', 'Age', 'Disease', 'F', 'A', 'S', 'FAS', 'Animals', 'Switch', 'Errs', 'P/S Ratio', 'z(FAS)', 'Overall'].map(h => (
                  <th key={h} style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 600, color: '#475569', fontSize: 11 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(d.patients || []).map(p => (
                <tr key={p.patient_id} onClick={() => setSelPid(p.patient_id === selPid ? null : p.patient_id)}
                  ...
                  <td style={{ padding: '5px 8px', color: '#475569' }}>{p.age || '—'}</td>
                  <td style={{ padding: '5px 8px', color: '#475569' }}>{p.disease || '—'}</td>
                  <td style={{ padding: '5px 8px' }}>{p.f_score}</td>
                  <td style={{ padding: '5px 8px' }}>{p.a_score}</td>
                  <td style={{ padding: '5px 8px' }}>{p.s_score}</td>
                  <td style={{ padding: '5px 8px', fontWeight: 700 }}>{p.fas_total}</td>
                  <td style={{ padding: '5px 8px', fontWeight: 700, fontFamily: 'monospace' }}>{p.animals}</td>
                  <td style={{ padding: '5px 8px' }}>{p.switching_total}</td>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace' }}>{p.switching_errors}</td>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace' }}>{p.phonemic_semantic_ratio}</td>
                  <td style={{ padding: '5px 8px', fontFamily: 'monospace' }}>{p.fas_z_score}</td>
                  <td style={{ padding: '5px 8px' }}>
                    <span style={{ padding: '2px 8px', borderRadius: 8, fontSize: 10, fontWeight: 600, background: (sevColors[p.severity] || '#94a3b8') + '18', color: sevColors[p.severity] || '#475569' }}>{p.severity}</span>
                  </td>
                </tr>
      ...
      {detail && !detail.error && (
        <div style={{ ...card, borderLeft: '4px solid #8b5cf6' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 10, marginBottom: 12 }}>
            {[['F', detail.f_score], ['A', detail.a_score], ['S', detail.s_score], ['FAS Total', detail.fas_total], ['Animals', detail.animals], ['Switching', detail.switching_total], ['Errors', detail.switching_errors], ['Clustering', detail.clustering_coefficient], ['P/S Ratio', detail.phonemic_semantic_ratio]].map(([k, v]) => (
              <div key={k} style={{ padding: 10, borderRadius: 8, background: '#f8fafc', border: '1px solid #e5e7eb' }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: '#475569' }}>{k}</div>
                <div style={{ fontSize: 22, fontWeight: 700, color: '#0f172a' }}>{v}</div>
              </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
            <div style={{ padding: '6px 12px', borderRadius: 8, background: (sevColors[detail.severity] || '#94a3b8') + '18', border: `1px solid ${sevColors[detail.severity] || '#cbd5e1'}40` }}>
              <span style={{ fontWeight: 700, color: sevColors[detail.severity] || '#475569' }}>Overall: {detail.severity}</span>
              <span style={{ fontSize: 11, marginLeft: 8, color: '#475569' }}>z(FAS)={detail.fas_z_score} · {detail.fas_percentile}%ile</span>
            </div>
          </div>
          {detail.clinical_interpretation && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginBottom: 4 }}>Fluency profile</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {Object.entries(detail.clinical_interpretation).map(([k, v]) => (
                  <span key={k} style={{ padding: '3px 8px', borderRadius: 6, fontSize: 10, background: String(v).includes('Impaired') || String(v).includes('impaired') ? '#fef2f2' : '#f0fdf4', border: '1px solid #e5e7eb' }}>
                    <strong>{k.replace(/_/g, ' ')}:</strong> {v}
                  </span>
                ))}
              </div>
          ...
          {detail.clinical_recommendations && detail.clinical_recommendations.length > 0 && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginBottom: 4 }}>Recommendations</div>
              {detail.clinical_recommendations.map((r, i) => (
                <div key={i} style={{ fontSize: 11, color: '#0f172a', marginBottom: 4, paddingLeft: 10, borderLeft: '2px solid #a78bfa' }}>{r}</div>
              ))}
            </div>
          )}
        </div>
      ...
      {trend && !trend.error && (
        <div style={{ ...card, borderLeft: '4px solid #10b981' }}>
          <h4 style={{ marginTop: 0, color: '#0f172a' }}>📈 12-month FAS trajectory — {trend.patient_name || trend.patient_id}</h4>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
            {(trend.trajectory || []).map(t => (
              <div key={t.month} style={{ padding: '4px 8px', borderRadius: 6, fontSize: 10, background: (sevColors[t.severity] || '#94a3b8') + '18', color: sevColors[t.severity] || '#475569', textAlign: 'center', minWidth: 50 }}>
                <div style={{ fontWeight: 600 }}>{t.label}</div>
                <div style={{ fontSize: 14, fontWeight: 700 }}>{t.projected_fas} FAS</div>
                <div>{t.severity}</div>
              </div>
            ))}
          </div>
      ...
      {defs && (
        <div style={card}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569' }}>Purpose</div>
              <div style={{ fontSize: 12 }}>{defs.purpose}</div>
            </div>
          </div>
          <div style={{ fontSize: 11, color: '#475569', marginBottom: 8 }}>{defs.reference}</div>
          {defs.metrics && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginBottom: 4 }}>Metrics ({defs.metrics.length})</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {defs.metrics.map(m => (
                  <span key={m.id} style={{ padding: '2px 8px', borderRadius: 6, fontSize: 10, background: '#eff6ff', color: '#0f172a' }}>
                    <strong>{m.abbreviation}</strong>: {m.metric}
                  </span>
                ))}
              </div>
            </div>
          ...
          {defs.epilepsy_context && (
            <div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#475569', marginBottom: 4 }}>Epilepsy context</div>
              {defs.epilepsy_context.applications && defs.epilepsy_context.applications.map((a, i) => (
                <div key={i} style={{ fontSize: 11, color: '#0f172a', marginBottom: 2 }}>• {a}</div>
              ))}
              {defs.epilepsy_context.aed_effects && (
