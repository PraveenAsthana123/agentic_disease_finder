...
export default function PHQ9Dashboard() {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>Loading PHQ-9 data...</div>
  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ margin: 0, fontSize: 22, color: '#1e293b' }}>PHQ-9 Dashboard</h2>
        <p style={{ margin: '4px 0 0', fontSize: 13, color: '#64748b' }}>
          Patient Health Questionnaire-9 — depression screening & severity monitoring
        </p>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          ...
          <Card span={3}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
              ...
              <KPI label="Avg Score" value={fmt(overview.avg_score)} sub="range 0-27" />
              <KPI label="Item 9 Flag Rate" value={`${fmt(overview.item9_flag_rate_pct)}%`}
                color={overview.item9_flag_rate_pct > 20 ? '#ef4444' : overview.item9_flag_rate_pct > 5 ? '#f97316' : '#22c55e'} />
            </div>
          </Card>
          ...
          <Card title="Monthly Average Score & Moderate+ Rate">
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={breakdown.trend}>
                ...
                <YAxis yAxisId="score" domain={[0, 27]} />
                  ...
                  <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                    {hist.map((h, i) => (
                      <div key={i} style={{ fontSize: 11, padding: '4px 8px', background: '#fff', borderRadius: 6, border: '1px solid #e2e8f0' }}>
                        <div style={{ fontWeight: 600 }}>{h.score}/27 <SevBadge level={h.severity} /></div>
            ...
            <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
                  ...
                  <th style={{ textAlign: 'left', padding: '4px 8px', color: '#64748b', width: 120 }}>Domain</th>
              ...
              <tbody>
                {(defs.items || []).map((it, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f1f5f9', background: it.id === 'item9' ? '#fef2f2' : undefined }}>
