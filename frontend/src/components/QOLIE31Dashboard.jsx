...
const COLORS = ['#ef4444', '#eab308', '#84cc16', '#22c55e', '#8b5cf6']
...
export default function QOLIE31Dashboard() {
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>Loading QOLIE-31 data...</div>
  ...
  const domainData = overview?.domain_scores
    ? Object.entries(overview.domain_scores).map(([k, v]) => ({ domain: k, score: v }))
    : (breakdown?.domain_scores
      ? Object.entries(breakdown.domain_scores).map(([k, v]) => ({ domain: k, score: v }))
      : [])

  return (
    <div style={{ padding: '20px 24px', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ margin: 0, fontSize: 22, color: '#1e293b' }}>QOLIE-31 Dashboard</h2>
        <p style={{ margin: '4px 0 0', fontSize: 13, color: '#64748b' }}>
          Quality of Life in Epilepsy (31-item) — QoL domain scoring & monitoring
        </p>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          ...
          <Card span={3}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
              ...
              <KPI label="Avg Score" value={fmt(overview.avg_score)} sub="range 0-100" />
              <KPI label="% Poor/Fair" value={`${fmt(overview.pct_poor_fair)}%`}
                color={overview.pct_poor_fair > 50 ? '#ef4444' : overview.pct_poor_fair > 30 ? '#f97316' : '#22c55e'} />
            </div>
          </Card>
          ...
          <Card title="Domain Scores (avg)" span={2}>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={domainData} layout="vertical" margin={{ left: 140 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" domain={[0, 100]} />
                <YAxis type="category" dataKey="domain" width={130} tick={{ fontSize: 11 }} />
                <Tooltip />
                <Bar dataKey="score" fill="#3b82f6" radius={[0, 4, 4, 0]} name="Avg Score" />
              </BarChart>
            </ResponsiveContainer>
          </Card>

          {/* Active Alerts */}
          <Card title="Active Alerts (score < 50)" span={3}>
                ...
                <tbody>
                  {(overview.patient_summary || []).map((p, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '6px 8px', textAlign: 'center' }}>{p.latest_score}/100</td>
      ...
      {tab === 'breakdown' && breakdown && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
          {/* Domain Comparison */}
          <Card title="Domain Comparison (avg scores)" span={2}>
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={breakdown.domain_comparison || Object.entries(breakdown.domain_scores || {}).map(([k, v]) => ({ domain: k, score: v }))} layout="vertical" margin={{ left: 140 }}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" domain={[0, 100]} />
                <YAxis type="category" dataKey="domain" width={130} tick={{ fontSize: 11 }} />
                <Tooltip />
            ...
            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 8 }}>
              Higher scores indicate better quality of life (0-100 scale)
            </div>
          </Card>
          ...
          {/* Monthly Trend */}
          <Card title="Monthly Average Score & % Poor/Fair" span={2}>
            <ResponsiveContainer width="100%" height={250}>
              <LineChart data={breakdown.trend}>
                ...
                <Line yAxisId="pct" type="monotone" dataKey="pct_poor_fair" stroke="#f97316" strokeWidth={2} strokeDasharray="5 5" name="Poor/Fair %" dot />
              </LineChart>
            </ResponsiveContainer>
          </Card>

          {/* Per-Patient History */}
          <Card title="Per-Patient Assessment History" span={2}>
                  ...
                  <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                    {hist.map((h, i) => (
                      <div key={i} style={{ fontSize: 11, padding: '4px 8px', background: '#fff', borderRadius: 6, border: '1px solid #e2e8f0' }}>
                        <div style={{ fontWeight: 600 }}>{h.score}/100 <SevBadge level={h.severity} /></div>
      ...
      {tab === 'reference' && defs && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16 }}>
          <Card title={defs.title}>
            ...
            <h4 style={{ fontSize: 14, color: '#334155', margin: '16px 0 8px' }}>Domains</h4>
            <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
                  <th style={{ textAlign: 'left', padding: '4px 8px', color: '#64748b', width: 40 }}>#</th>
                  <th style={{ textAlign: 'left', padding: '4px 8px', color: '#64748b', width: 180 }}>Domain</th>
                  <th style={{ textAlign: 'left', padding: '4px 8px', color: '#64748b' }}>Description</th>
                </tr>
              </thead>
              <tbody>
                {(defs.domains || []).map((d, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    <td style={{ padding: '6px 8px', fontWeight: 600 }}>{i + 1}</td>
                    <td style={{ padding: '6px 8px', fontWeight: 600, color: '#334155' }}>{d.label}</td>
                    <td style={{ padding: '6px 8px', color: '#475569' }}>{d.description}</td>
                  </tr>
                ))}
              </tbody>
            </table>
