...
export default function EegAiStackDashboard() {
  ...
  if (!overview) return <div style={{ padding: 24, color: '#64748b' }}>Loading EEG AI Stack...</div>
  if (!overview.available) return <div style={{ padding: 24, color: '#f97316' }}>{overview.note}</div>

  const tabs = ['overview', 'by-layer', 'endpoints', 'edc-tools', 'definitions']
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 1280, margin: '0 auto', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      {tab === 'overview' && (
        <>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
            <Card><KPI label="Layers" value={s.total_layers} /></Card>
            <Card><KPI label="Total Tools" value={s.total_tools} /></Card>
            <Card><KPI label="Installed" value={s.installed} sub="import verified" /></Card>
            <Card><KPI label="Built" value={s.built} sub="live endpoints" /></Card>
            <Card><KPI label="External" value={s.external} sub="MATLAB/server" /></Card>
            <Card><KPI label="With Endpoints" value={s.tools_with_endpoints} /></Card>
            <Card><KPI label="EDC Tools" value={s.edc_tools} /></Card>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
            ...
            <Card title="Tools per Layer">
              <ResponsiveContainer width="100%" height={380}>
                <BarChart data={overview.layer_distribution} layout="vertical" margin={{ left: 160 }}>
          ...
          {overview.recommended_pipeline && (
            <Card title="Recommended Pipeline">
              <div style={{ fontSize: 13, color: '#1e293b', fontFamily: 'monospace', background: '#f8fafc', padding: 12, borderRadius: 6, lineHeight: 1.6 }}>
                {overview.recommended_pipeline}
              </div>
            </Card>
          )}
      ...
      {tab === 'by-layer' && breakdown && (
        <>
          {(breakdown.per_layer || []).map((layer, li) => (
            <Card key={li} title={layer.layer}>
              <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                {Object.entries(layer.status_counts || {}).map(([st, cnt]) => (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 10 }}>
                {(layer.tools || []).map((t, ti) => (
                  <div key={ti} style={{
                    border: '1px solid #e2e8f0', borderRadius: 6, padding: 10,
                    background: t.status === 'built' ? '#f0fdf4' : t.status === 'installed' ? '#eff6ff' : t.status === 'external' ? '#fff7ed' : '#fff'
                  }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <span style={{ fontWeight: 600, fontSize: 13, color: '#1e293b' }}>{t.name}</span>
                      <StatusBadge status={t.status} />
                    </div>
                    {t.use && <div style={{ fontSize: 11, color: '#64748b' }}>{t.use}</div>}
                    {t.note && <div style={{ fontSize: 10, color: '#8b5cf6', marginTop: 2 }}>{t.note}</div>}
                    ...
                    {t.endpoint && (!t.endpoints || !t.endpoints.length) && (
                      <div style={{ fontSize: 10, color: '#3b82f6', fontFamily: 'monospace', marginTop: 4 }}>{t.endpoint}</div>
                    )}
                    {t.dashboard && (
                      <div style={{ fontSize: 10, color: '#14b8a6', fontFamily: 'monospace', marginTop: 4 }}>{t.dashboard}</div>
                    )}
                  </div>
                ))}
              </div>
      ...
      {tab === 'edc-tools' && breakdown && (
        <Card title="EDC Assessment Tools">
          <p style={{ fontSize: 12, color: '#64748b', marginBottom: 12 }}>
            Electronic Data Capture and clinical assessment tools for the EEG AI platform.
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 12 }}>
            {(breakdown.edc_assessment_tools || []).map((t, i) => (
              <div key={i} style={{
                border: '1px solid #e2e8f0', borderRadius: 6, padding: 12,
                background: t.status === 'built' ? '#f0fdf4' : t.status === 'external' ? '#fff7ed' : '#fff'
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <span style={{ fontWeight: 600, fontSize: 14, color: '#1e293b' }}>{t.name}</span>
                  <StatusBadge status={t.status} />
                </div>
                <div style={{ fontSize: 12, color: '#64748b' }}>{t.use}</div>
                {t.endpoints && t.endpoints.length > 0 && (
