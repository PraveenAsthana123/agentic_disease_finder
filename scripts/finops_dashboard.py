...
def overview():
    ...
    total_7d = _safe(cur, """
        SELECT COALESCE(SUM(cost_usd), 0) FROM finops_costs
        WHERE cost_date >= date('now', '-7 days')""")
    ...
    total_requests_30d = _safe(cur, """
        ...
        WHERE cost_date >= date('now', '-30 days') AND requests > 0""")
    ...
    total_gpu_min_30d = _safe(cur, """
        ...
        WHERE cost_date >= date('now', '-30 days') AND gpu_minutes > 0""")

    unique_patients_30d = _safe(cur, """
        SELECT COUNT(DISTINCT patient_id) FROM finops_costs
        WHERE cost_date >= date('now', '-30 days') AND patient_id IS NOT NULL""")
    ...
    # Monthly budget (simulated: $150/month)
...
def breakdown():
    ...
    # Component-level spend
    component_spend = []
    for row in _safe_rows(cur, """
        SELECT component, ROUND(SUM(cost_usd), 2), SUM(requests),
               SUM(tokens_in + tokens_out)
        FROM finops_costs
        WHERE component IS NOT NULL AND cost_date >= date('now', '-30 days')
    ...
    # Per-patient cost (top 15)
    patient_costs = []
    for row in _safe_rows(cur, """
        SELECT patient_id, ROUND(SUM(cost_usd), 2), SUM(requests),
               SUM(tokens_in + tokens_out)
        FROM finops_costs
        WHERE patient_id IS NOT NULL AND cost_date >= date('now', '-30 days')
    ...
    last_week = _safe(cur, """
        SELECT COALESCE(SUM(cost_usd), 0) FROM finops_costs
        WHERE cost_date >= date('now', '-14 days') AND cost_date < date('now', '-7 days')""")
    week_change_pct = round((this_week - last_week) / last_week * 100, 1) if last_week else 0

    # Category daily breakdown (for stacked chart)
    category_daily = []
    for row in _safe_rows(cur, """
    ...
    for row in _safe_rows(cur, """
        ...
        FROM finops_costs
        WHERE category = 'cloud_infra' AND cost_date >= date('now', '-30 days')
        GROUP BY model_or_service ORDER BY SUM(cost_usd) DESC"""):
        cloud_costs.append({
