...
const COLOR  = '#004d40';  // deep teal — pancreatic/metabolic
const LIGHT  = '#e0f2f1';
const COLOR2 = '#b71c1c';  // fatal / absolute CI
const COLOR3 = '#0d47a1';  // biomarker / K-ATP
const COLOR4 = '#e65100';  // surgical / warning
const COLOR5 = '#1b5e20';  // treatable / diazoxide-responsive
const COLOR6 = '#4a148c';  // gene therapy / mecasermin
const COLOR7 = '#37474f';  // gene class / struct

const SUBGROUP_COLORS = {
  'K-ATP channel defects (ABCC8 · KCNJ11)':              '#0d47a1',
  'Glucose-sensing enzyme defects (GCK)':                 '#e65100',
  'Enzyme GOF / signalling defects (GLUD1 · HADH)':      '#7b1fa2',
  'Transcription factor defects (HNF4A)':                 '#00695c',
  'Unique trigger defects (SLC16A1 — EIHI)':             '#f57f17',
  'Insulin receptor defects (INSR)':                      '#b71c1c',
};
...
export default function CHIAtlasPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/chi-atlas/overview`).then(r => r.json()),
      fetch(`${API}/api/chi-atlas/breakdown`).then(r => r.json()),
      fetch(`${API}/api/chi-atlas/definitions`).then(r => r.json()),
    ]).then(([ov, bd, df]) => {
      setOverview(ov); setBreakdown(bd); setDefs(df); setLoading(false);
    }).catch(e => { setErr(e.message); setLoading(false); });
  }, []);

  if (loading) return <div className="p-4 text-center"><div className="spinner-border" style={{ color: COLOR }} /><p className="mt-2">Loading CHI-Atlas…</p></div>;
  ...
  return (
    <div className="container-fluid py-3">
      ...
      <div className="d-flex align-items-center mb-2 gap-3">
        ...
        <div>
          <h4 className="mb-0 fw-bold" style={{ color: COLOR }}>CHI-Atlas — Congenital Hyperinsulinism</h4>
      ...
      {tab === 'Overview' && (
        <div>
          <div className="row g-2 mb-3">
            ...
            <KPI label="Subgroups" value="6" color={COLOR3} />
            <KPI label="Diazoxide-Resp" value="GLUD1·HADH·HNF4A" color={COLOR5} />
            <KPI label="18F-DOPA PET" value="ABCC8·KCNJ11" color={COLOR3} />
            <KPI label="EIHI" value="SLC16A1" color={COLOR4} />
          </div>

          {/* Subgroups */}
          <div className="card shadow-sm mb-3">
            <div className="card-header fw-semibold" style={{ background: LIGHT, color: COLOR }}>
              CHI Mechanistic Subgroups — 8 Genes
            </div>
            <div className="card-body py-2">
              ...
              <div className="alert alert-info py-1 mb-0 mt-2" style={{ fontSize: '0.78rem' }}>
                <strong>Key principle:</strong> Congenital hyperinsulinism = unregulated insulin secretion
                despite hypoglycemia. Six mechanisms: K-ATP constitutive closure · GCK low glucose threshold ·
                GDH disinhibition · transcription factor excess · MCT1 pyruvate trigger · insulin receptor resistance.
              </div>
            </div>
          </div>
              ...
              <div key={g.gene} className="col-12 col-md-6 col-lg-3">
                <div className="card shadow-sm h-100" style={{ borderLeft: `4px solid ${GENE_COLORS[g.gene] || COLOR}` }}>
                  <div className="card-body py-2 px-3">
                    ...
                    <div className="small text-muted" style={{ fontSize: '0.7rem' }}>{g.chi_subgroup}</div>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="alert alert-warning py-2 mb-0" style={{ fontSize: '0.8rem' }}>
            <strong>NBS Status:</strong> {overview?.nbs_note}
          </div>
        </div>
      )}
      ...
      {tab === 'Gene Table' && breakdown?.genes && (
        <div>
            ...
            <table className="table table-bordered table-sm table-hover" style={{ fontSize: '0.79rem' }}>
              <thead style={{ background: COLOR, color: '#fff' }}>
                <tr>
                  ...
                  <th>N</th><th>Inheritance</th><th>Avg Dx (y)</th>
                  <th>Diaz-Resp%</th><th>Focal%</th><th>Phenotype (brief)</th>
                </tr>
              </thead>
              <tbody>
                {breakdown.genes.map(g => (
                  <tr key={g.gene}>
                    ...
                    <td style={{ maxWidth: 120, fontSize: '0.73rem' }}>{g.gene_class}</td>
                    ...
                    <td>
                      <span className="badge" style={{
                        background: g.pct_diazoxide_responsive >= 70 ? '#1b5e20' :
                                    g.pct_diazoxide_responsive >= 30 ? '#f57f17' : '#b71c1c',
                        fontSize: '0.68rem'
                      }}>{g.pct_diazoxide_responsive}%</span>
                    </td>
                    <td>{g.pct_focal_lesion}%</td>
          ...
          <p className="text-muted small">
            ...
            CHI = Congenital Hyperinsulinism · EIHI = Exercise-Induced HI · HHS = Hyperinsulinism-Hyperammonemia Syndrome
          </p>
        </div>
      )}
            ...
            <div key={g.gene} className="card shadow-sm mb-3" style={{ borderLeft: `5px solid ${GENE_COLORS[g.gene] || COLOR}` }}>
              <div className="card-header d-flex justify-content-between align-items-center"
                ...
                <div className="d-flex gap-1 flex-wrap">
                  ...
                  <span className="badge" style={{ background: COLOR7, fontSize: '0.65rem' }}>{g.chi_subgroup?.split('(')[0]?.trim()}</span>
                  <span className="badge" style={{
                    background: g.pct_diazoxide_responsive >= 70 ? '#1b5e20' :
                                g.pct_diazoxide_responsive >= 30 ? '#f57f17' : '#b71c1c',
                    fontSize: '0.65rem'
                  }}>Diaz {g.pct_diazoxide_responsive}%</span>
                  {g.pct_focal_lesion > 0 && (
                    <span className="badge" style={{ background: '#0d47a1', fontSize: '0.65rem' }}>
                      Focal {g.pct_focal_lesion}% (18F-DOPA PET)
                    </span>
                  )}
                </div>
              </div>
              <div className="card-body">
                ...
                <p className="small mb-1"><strong>Gene Therapy / Advanced Status:</strong> {g.gene_therapy_status}</p>
      ...
      {tab === 'Definitions' && defs && (
        <div>
          <div className="card shadow-sm mb-3">
            <div className="card-header fw-semibold" style={{ background: LIGHT, color: COLOR }}>
              CHI-Atlas Overview — {defs.chi_overview?.genes_in_atlas} Genes
            </div>
            <div className="card-body">
              <p className="small mb-1"><strong>Full Name:</strong> {defs.chi_overview?.full_name}</p>
              <p className="small mb-1"><strong>Collective Incidence:</strong> {defs.chi_overview?.collective_incidence}</p>
              <p className="small mb-0"><strong>NBS Note:</strong> {defs.chi_overview?.nbs_note}</p>
            </div>
          </div>
