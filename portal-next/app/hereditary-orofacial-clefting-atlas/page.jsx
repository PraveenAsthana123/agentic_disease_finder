...
const SLUG = 'hereditary-orofacial-clefting-atlas';
const TABS = ['Overview', 'Gene Table', 'Clinical Atlas', 'Definitions'];

const GENE_COLORS = {
  'IRF6':    '#b71c1c',  // deep red       — VWS1; lip pits PATHOGNOMONIC; most common hereditary CL/P
  'GRHL3':   '#c62828',  // red            — VWS2; downstream IRF6 pathway; PVRL1-negative VWS
  'PVRL1':   '#d32f2f',  // red            — CLPED1 AR; CL/P+hypodontia+ectodermal; Mediterranean founder
  'MSX1':    '#1565c0',  // deep blue      — OFC5; tooth agenesis #2 + CL/P; premolars; founder
  'TBX22':   '#6a1b9a',  // deep purple    — CPX XLR; ankyloglossia+cleft palate PATHOGNOMONIC
  'SATB2':   '#4a148c',  // dark purple    — Glass syndrome de novo; absent speech; severe ID
  'TP63':    '#e65100',  // deep orange    — EEC; ectrodactyly+ectodermal dysplasia+CLP TRIAD
  'COL11A1': '#2e7d32',  // dark green     — Marshall/Stickler2; cleft palate+high myopia+SNHL
};
...
export default function HereditaryOrofacialCleftingAtlasPage() {
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: 1100, margin: '0 auto', padding: '20px 16px' }}>
      <div style={{ marginBottom: 18 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1a1a2e', margin: '0 0 4px' }}>
          🧬 Hereditary Orofacial Clefting Atlas
        </h1>
        <p style={{ color: '#555', fontSize: 13, margin: 0 }}>
          Complete 8-Gene Orofacial Clefting &amp; Cleft Lip/Palate Genetics Reference · IRF6-GRHL3-PVRL1-MSX1-TBX22-SATB2-TP63-COL11A1 · 320 Patients · Seeds 3022-3029
        </p>
      </div>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 10, marginBottom: 18 }}>
            {[
              ...
              { label: 'Atlas',           value: 'OrofacialClefting' },
            ].map(({ label, value }) => (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr style={{ background: '#b71c1c', color: '#fff' }}>
                    {['Gene','Locus','n','Severe%','Mod%','Mild%','Mean IQ','SNHL%','Dental%','Ectrod%','Myopia%','Ankyl%','AbsSpeech%','Age Dx(mo)'].map(h => (
                ...
                <tbody>
                  {breakdown.genes?.map((g, i) => (
                    <tr key={g.gene}
                      ...
                      <td style={{ padding: '7px 10px', color: g.dental_pct > 50 ? '#e65100' : '#555' }}>{g.dental_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.ectrodactyly_pct > 0 ? '#c62828' : '#aaa', fontWeight: g.ectrodactyly_pct > 30 ? 700 : 400 }}>{g.ectrodactyly_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.high_myopia_pct > 0 ? '#2e7d32' : '#aaa' }}>{g.high_myopia_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.ankyloglossia_pct > 0 ? '#6a1b9a' : '#aaa' }}>{g.ankyloglossia_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.absent_speech_pct > 50 ? '#c62828' : '#aaa', fontWeight: g.absent_speech_pct > 50 ? 700 : 400 }}>{g.absent_speech_pct}%</td>
            ...
            <div key={g.gene} style={{ ...cardStyle, borderLeft: `4px solid ${GENE_COLORS[g.gene] || '#ccc'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {g.absent_speech_pct > 50 && <Badge text={`AbsSpeech ${g.absent_speech_pct}%`} color="#c62828" />}
                  {g.ectrodactyly_pct > 30 && <Badge text={`Ectrodactyly ${g.ectrodactyly_pct}%`} color="#e65100" />}
                  {g.snhl_pct > 0 && <Badge text={`SNHL ${g.snhl_pct}%`} color="#1565c0" />}
                  {g.high_myopia_pct > 0 && <Badge text={`HighMyopia ${g.high_myopia_pct}%`} color="#2e7d32" />}
                  {g.ankyloglossia_pct > 0 && <Badge text={`Ankyl ${g.ankyloglossia_pct}%`} color="#6a1b9a" />}
                  {g.dental_pct > 60 && <Badge text={`Dental ${g.dental_pct}%`} color="#e65100" />}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 6, marginTop: 8 }}>
                {[
                  ...
                  { label: 'Dental%', value: g.dental_pct + '%', color: g.dental_pct > 60 ? '#e65100' : '#555' },
      ...
      {tab === 'Definitions' && definitions && !loading && (
        <div>
          <div style={{ color: '#888', fontSize: 12, marginBottom: 12 }}>
            {definitions.count} clinical definition entries — orofacial clefting classification, VWS IRF6/GRHL3, TP63 EEC/AEC/RHS/LMS, Marshall/Stickler COL11A1 vs COL2A1, 8-gene algorithm
          </div>
