...
const COLOR  = '#00695c';   // dark teal — tRNA-Cys / L-strand / TENTH tRNA
const LIGHT  = '#e0f2f1';
const COLOR2 = '#004d40';   // deep teal — CPEO / myopathy
...
const COLOR5 = '#1a237e';   // deep indigo — CARS2 DDx / nuclear gene
...
function OverviewTab({ data }) {
  ...
  return (
    <>
      {/* Gene facts banner */}
      <SectionCard title="Gene Facts — MT-TC tRNA-Cys" borderColor={COLOR}>
      ...
      {/* L-strand NGS pitfall alert */}
      {data.l_strand_ngs_alert && (
        <div className="alert border-0 mb-4" style={{ backgroundColor: '#ffebee', borderLeft: `5px solid ${COLOR3}` }}>
          <strong style={{ color: COLOR3 }}>⚠ {data.l_strand_ngs_alert.alert_class}</strong>
          <p className="mb-0 small mt-1">{data.l_strand_ngs_alert.detail}</p>
        </div>
      )}

      {/* CARS2 DDx note */}
      {data.cars2_ddx_note && (
        <div className="alert border-0 mb-4" style={{ backgroundColor: '#e8eaf6', borderLeft: `5px solid ${COLOR5}` }}>
          <strong style={{ color: COLOR5 }}>🧬 {data.cars2_ddx_note.note_class}</strong>
          <p className="mb-0 small mt-1">{data.cars2_ddx_note.detail}</p>
        </div>
      )}
...
function VariantsTab({ data }) {
  ...
  return (
    <>
      <SectionCard title="Variant Breakdown — 40-Patient Cohort (seed-819)" borderColor={COLOR}>
        <div className="table-responsive">
          <table className="table table-sm table-hover small mb-0">
            <thead><tr>
              <th>Variant</th><th>n</th><th>% Cohort</th>
              <th>Avg Hetero</th><th>Avg CI%</th><th>Avg CIV%</th>
              <th>CPEO%</th><th>Myo%</th><th>Cardio%</th><th>Compound%</th>
            </tr></thead>
            <tbody>
              {vb.map((r, i) => (
                <tr key={i}>
                  ...
                  <td>{r.pct_compound_tc_ty_loss}%</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <SectionCard title="Management by Variant" borderColor={COLOR2}>
        <div className="table-responsive">
          <table className="table table-sm table-hover small mb-0">
            <thead><tr>
              <th>Variant</th><th>CPEO Risk</th><th>Cardio Risk</th><th>Key Action</th>
            </tr></thead>
            <tbody>
              {mgmt.map((r, i) => (
                <tr key={i}>
                  <td><code>{r.variant}</code></td>
                  <td><span className="badge" style={{ backgroundColor: r.cpeo_risk === 'High' ? COLOR3 : COLOR }}>{r.cpeo_risk}</span></td>
                  <td><span className="badge" style={{ backgroundColor: r.cardio_risk === 'Moderate' ? '#e65100' : r.cardio_risk === 'High' ? COLOR3 : COLOR4 }}>{r.cardio_risk}</span></td>
      ...
      <SectionCard title="Differential Diagnosis Table" borderColor={COLOR2}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small mb-0">
            ...
            <tbody>
              {ddx.map((r, i) => (
                <tr key={i} style={r.entity.startsWith('MT-TC') ? { backgroundColor: LIGHT, fontWeight: 600 } : {}}>
                  <td>{r.entity}</td>
                  <td>{r.inheritance}</td>
                  <td>{r.phenotype}</td>
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function MTTCPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/mttc/overview`).then(r => r.json()),
      fetch(`${API}/api/mttc/breakdown`).then(r => r.json()),
      fetch(`${API}/api/mttc/definitions`).then(r => r.json()),
    ])
  ...
  return (
    <div className="container-fluid py-4">
      ...
      <div className="mb-4 p-3 rounded" style={{ background: `linear-gradient(135deg, ${COLOR} 0%, ${COLOR2} 100%)`, color: '#fff' }}>
        <h4 className="fw-bold mb-1">🧬 MT-TC — tRNA-Cys (GCA Anticodon)</h4>
        <p className="mb-1 small opacity-75">
          Combined CI+CIV Deficiency · CPEO · Myopathy · L-strand NGS Pitfall — TENTH tRNA — rCRS 5761–5826
        </p>
        <div className="d-flex gap-3 flex-wrap small">
          <span className="badge bg-light text-dark">OMIM *590020</span>
          <span className="badge bg-light text-dark">L-strand — NGS Pitfall</span>
          <span className="badge bg-light text-dark">40-patient cohort seed-819</span>
          <span className="badge bg-light text-dark">THIRD of 4 consecutive L-strand tRNAs</span>
          <span className="badge bg-light text-dark">CARS2 DDx — Neonatal Multisystem OXPHOS</span>
        </div>
      </div>
