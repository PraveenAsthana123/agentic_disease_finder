...
const COLOR = '#1a237e';   // deep indigo — Q-module zinc-finger, unique CI structural mechanism
const LIGHT = '#e8eaf6';
  ...
  return (
    <>
      <div className="row g-2 mb-3">
        ...
        <KPI label="Also Known As" value={data.also_known_as || 'AQDQ'}   color={COLOR} />
        <KPI label="Size"          value={`${pt.size_kda} kDa`}           color={COLOR} />
        <KPI label="Fe-S Cluster"  value="None"                           color="#455a64" />
        <KPI label="Zinc Finger"   value="CXXC×2 Zn²⁺"                   color="#1565c0" />
        <KPI label="Module"        value="Q-module"                       color="#283593" />
        <KPI label="Inheritance"   value="AR biallelic"                   color={COLOR} />
        <KPI label="Chromosome"    value={data.chromosome}                color={COLOR} />
        <KPI label="OMIM Gene"     value={`*${data.omim_gene}`}           color="#3949ab" />
        <KPI label="OMIM Disease"  value={`#${data.omim_disease}`}        color="#3949ab" />
        <KPI label="Cohort N"      value={co.n}                           color={COLOR} />
        <KPI label="Mean CI Act"   value={`${co.ci_activity_mean_pct}%`} color="#e53935" />
      </div>

      <div className="alert mb-4" style={{ background: '#e8eaf6', borderLeft: '5px solid #1a237e' }}>
        <strong>Mechanistic Distinction:</strong> NDUFS6 is the only CI structural subunit with a dual
        CXXC tetracysteine zinc-finger (Zn²⁺ structural ion — NOT Fe-S). Loss abolishes Q-module
        assembly. Quinone-binding site integrity fails → isolated CI deficiency 5–20%.
      </div>

      <SectionCard title="Subunit Role — Q-Module Zinc-Finger (CXXC × 2 Tetracysteine / Structural Zn²⁺)">
        <p className="small mb-2" style={{ whiteSpace: 'pre-line' }}>{data.key_pathway_note}</p>
        <div className="row g-2">
          <div className="col-md-6">
            <div className="rounded p-2 small" style={{ background: '#e8eaf6' }}>
              <strong>Module:</strong> {pt.module}<br />
              <strong>Fe-S cluster:</strong> None (structural zinc-finger only)<br />
              <strong>Zinc finger:</strong> Dual CXXC tetracysteine (Zn²⁺ structural ion)<br />
              <strong>Function:</strong> {pt.function}
            </div>
          </div>
          <div className="col-md-6">
            <div className="rounded p-2 small" style={{ background: '#fff8e1' }}>
              <strong>BN-PAGE pattern:</strong> Q-module sub-assembly intermediates (zinc-finger collapse)<br />
              <em>vs NDUFS7/NDUFS8 (Fe-S direct relay block): cleaner absent CI</em><br />
              <em>vs NDUFB3 (PP-module): also cleaner absent CI (scaffolding loss)</em><br />
              <em>Overlaps NDUFS3/NDUFS4/NDUFS5 assembly-failure patterns</em>
            </div>
          </div>
        </div>
      </SectionCard>
      ...
      <SectionCard title="Clinical Feature Frequencies (40-patient cohort, seed-629)">
...
function PatientsTab({ data }) {
  ...
  return (
    <>
      <SectionCard title="CI Activity Distribution (40-patient cohort, seed-629)">
        <div className="row g-2">
...
COFACTORS    : Riboflavin B2 · CoQ10 (ubiquinol, quinone-site module) · Thiamine B1* · Biotin* · Succinate · Carnitine
               (* = MANDATORY empiric before genetics: rules out SLC19A3/BTD)
MECHANISM    : NDUFS6 CXXC dual zinc-finger → Q-module structural Zn²⁺ → quinone-site collapse
               Only CI subunit with a bona fide zinc-finger structural domain`}
        </div>
      </SectionCard>
    </>
  );
...
// ── Main page ──────────────────────────────────────────────────────────────
export default function NDUFS6Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/ndufs6/overview`).then(r => r.json()),
      fetch(`${API}/api/ndufs6/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ndufs6/definitions`).then(r => r.json()),
    ...
    <div className="container-fluid py-4">
      <div className="mb-4">
        <h2 className="fw-bold mb-1" style={{ color: COLOR }}>
          🧬 NDUFS6 — Leigh Syndrome Isolated Complex I Deficiency
        </h2>
        <p className="text-muted small mb-1">
          Q-Module Zinc-Finger Subunit · Dual CXXC Tetracysteine · Structural Zn²⁺ · AR Biallelic · 5p15.33
        </p>
        <p className="text-muted small mb-1">
          OMIM Gene *603848 · Disease Leigh Syndrome #256000 / CI Nuclear Type 3 #618225 · 40-patient cohort seed-629
        </p>
        <p className="small fw-semibold" style={{ color: '#1a237e' }}>
          ★ Only CI structural subunit with a bona fide dual-CXXC zinc-finger — structural Zn²⁺, not Fe-S
        </p>
