...
const COLOR = '#004d40';   // deep teal — AR / Q-module distal arm / LZ coiled-coil / no TM helix
const LIGHT = '#e0f2f1';
    ...
    <div>
      <SectionCard title="Gene & Disease Identity">
        <div className="row g-2 small">
          {[
            ['Gene', data.gene],
            ['Protein', 'NADH:Ubiquinone Oxidoreductase Subunit A6 (B14)'],
            ['Module', data.module],
...
function PatientsTab({ data }) {
  ...
  return (
    <div>
      ...
      <SectionCard title="Patient Cohort (40 patients, seed 659)">
...
// ── Root component ────────────────────────────────────────────────────────────
export default function NDUFA6Page() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/ndufa6/overview`).then(r => r.json()),
      fetch(`${API}/api/ndufa6/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ndufa6/definitions`).then(r => r.json()),
    ])
  ...
  return (
    <div className="container-fluid py-3">
      <div className="mb-3 p-3 rounded" style={{ background: COLOR, color: '#fff' }}>
        <h4 className="mb-1 fw-bold">🧬 NDUFA6 — Leigh Syndrome / Isolated Complex I Deficiency</h4>
        <div className="small opacity-75">
          B14 · Q-module Distal Arm · LZ Coiled-Coil · No TM Helix · AR · 22q13.2 ·
          OMIM Gene *602137 · Disease #256000
        </div>
      </div>

      {loading && <div className="alert alert-info">Loading NDUFA6 dashboard…</div>}
