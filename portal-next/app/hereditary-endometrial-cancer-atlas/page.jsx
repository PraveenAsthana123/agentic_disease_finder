...
const SLUG = 'hereditary-endometrial-cancer-atlas';
const TABS = ['Overview', 'Gene Table', 'Clinical Atlas', 'Definitions'];

const GENE_COLORS = {
  'MLH1':  '#00695c',  // deep teal        — Lynch Type 1; MMR; MSI-H; aspirin CAPP2
  'MSH6':  '#6a1b9a',  // deep purple      — Lynch Type 3; HIGHEST endometrial 40-71%; MSI-L pitfall
  'PTEN':  '#e65100',  // deep orange      — Cowden/PHTS; macrocephaly PATHOGNOMONIC; mTOR
  'POLE':  '#1565c0',  // deep blue        — ultra-hypermutated TMB>100 MSS; exceptional pembrolizumab
  'TP53':  '#b71c1c',  // deep red         — LFS; serous endometrial; AVOID RADIATION ABSOLUTELY
  'BRCA1': '#880e4f',  // deep magenta     — HBOC; serous-like; BSO age 40; olaparib
  'STK11': '#2e7d32',  // forest green     — Peutz-Jeghers; SCTAT PATHOGNOMONIC; OCP reduces risk
  'MSH2':  '#283593',  // dark indigo      — Lynch Type 2; Muir-Torre PATHOGNOMONIC; EPCAM deletion
};

const GENE_INFO = {
  'MLH1':  { full: 'Lynch Type 1 / Endometrial 40-50% / MSI-H / Aspirin CAPP2 50% ↓',          locus: '3p22.2',   size: '793 aa / 90 kDa',   inh: 'AD LOF' },
  'MSH6':  { full: 'Lynch Type 3 / HIGHEST Endometrial 40-71% / MSI-L 30% Pitfall',             locus: '2p16.3',   size: '1360 aa / 160 kDa', inh: 'AD LOF' },
  'PTEN':  { full: 'Cowden/PHTS / Endometrial 28-44% / Macrocephaly PATHOGNOMONIC / mTOR',      locus: '10q23.31', size: '403 aa / 47 kDa',   inh: 'AD LOF' },
  'POLE':  { full: 'PPAP / TMB>100 MSS / Exceptional Pembrolizumab / Complete Remissions',       locus: '12q24.33', size: '2286 aa / 261 kDa', inh: 'AD GOF Exo' },
  'TP53':  { full: 'LFS / Serous Endometrial / p53-Abnormal TCGA / AVOID RADIATION ABSOLUTELY', locus: '17p13.1',  size: '393 aa / 43 kDa',   inh: 'AD LOF' },
  'BRCA1': { full: 'HBOC / Serous-Like Endometrial 2-3× RR / BSO Age 40 / Olaparib',            locus: '17q21.31', size: '1863 aa / 220 kDa', inh: 'AD LOF' },
  'STK11': { full: 'Peutz-Jeghers / SCTAT PATHOGNOMONIC / Endometrial 9-12× RR / OCP ↓ Risk',   locus: '19p13.3',  size: '433 aa / 48 kDa',   inh: 'AD LOF' },
  'MSH2':  { full: 'Lynch Type 2 / Endometrial 40-60% / Muir-Torre PATHOGNOMONIC / EPCAM del',  locus: '2p21',     size: '934 aa / 105 kDa',  inh: 'AD LOF' },
};
...
export default function HereditaryEndometrialCancerAtlas() {
  ...
  return (
    <div style={{ fontFamily: 'system-ui,sans-serif', background: '#0a0a0a', minHeight: '100vh', color: '#e8e8e8', padding: 24 }}>
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg,#001a14 0%,#1a0033 60%,#1a0d00 100%)', borderRadius: 12, padding: '28px 32px', marginBottom: 24 }}>
        <div style={{ fontSize: 11, color: '#80cbc4', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8 }}>
          Hereditary Disease Atlas · Endometrial Cancer Predisposition · 8-Gene Reference
        </div>
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: '#fff' }}>
          🧬 Hereditary Endometrial Cancer Atlas
        </h1>
        <div style={{ marginTop: 10, color: '#b0bec5', fontSize: 13 }}>
          Complete 8-Gene Predisposition Reference · MLH1 · MSH6 · PTEN · POLE · TP53 · BRCA1 · STK11 · MSH2
        </div>
        ...
        <div style={{ marginTop: 10, fontSize: 11, color: '#ef9a9a', fontWeight: 600 }}>
          ⚠ CRITICAL PITFALLS: MSH6 MSI-L 30% false-negative on PCR — use IHC; POLE TMB&gt;100 but MSS — do NOT rely on MSI-PCR alone; TP53 LFS — AVOID RADIATION ABSOLUTELY
        </div>
        <div style={{ marginTop: 6, fontSize: 11, color: '#80cbc4' }}>
          320-patient aggregate · 8 × 40 seeds · seeds 3126-3133 · MSH6 HIGHEST Lynch endometrial 40-71% · POLE exceptional pembrolizumab complete remissions · PTEN Cowden macrocephaly PATHOGNOMONIC · Universal MMR IHC all endometrial biopsies · TCGA molecular classification: POLE-best &gt; MMRd &gt; NSMP &gt; p53-abnormal-worst
        </div>
      </div>
      ...
      {tab === 'Overview' && overview && (
        <div>
          ...
          {/* MSH6 MSI-L pitfall warning */}
          <div style={{ background: '#1a0033', border: '2px solid #6a1b9a', borderRadius: 8, padding: '14px 20px', marginBottom: 16 }}>
            <div style={{ color: '#ce93d8', fontWeight: 700, fontSize: 14, marginBottom: 8 }}>
              ⚠ MSH6 MSI-L PITFALL — IHC IS THE PRIMARY TEST
            </div>
            <ul style={{ margin: 0, padding: '0 0 0 18px' }}>
              <li style={{ fontSize: 12, color: '#e1bee7', marginBottom: 5 }}>
                <strong>MSH6 Lynch:</strong> MSI-L or MSS in 30% of MSH6-mutation carriers by PCR — MSI-PCR alone will miss 1 in 3 MSH6 cases
              </li>
              <li style={{ fontSize: 12, color: '#e1bee7', marginBottom: 5 }}>
                <strong>Protocol:</strong> Universal MMR IHC (MLH1/MSH2/MSH6/PMS2) on ALL endometrial biopsies — loss of MSH6 staining by IHC confirms Lynch even when PCR shows MSI-L/MSS
              </li>
              <li style={{ fontSize: 12, color: '#e1bee7' }}>
                <strong>POLE pitfall:</strong> POLE ultra-hypermutated tumours are MSS but TMB&gt;100 mut/Mb — do NOT rely on MSI-PCR; requires NGS-TMB; exceptional pembrolizumab response
              </li>
            </ul>
          </div>
      ...
      {tab === 'Clinical Atlas' && breakdown && (
        <div>
          <div style={{ background: '#1e1e1e', borderRadius: 8, padding: 20, marginBottom: 20 }}>
            <h3 style={{ margin: '0 0 16px', color: '#80cbc4', fontSize: 15 }}>Syndrome Summary — Hereditary Endometrial Cancer Predisposition</h3>
          ...
          {/* TCGA molecular classification box */}
          <div style={{ background: '#0d1b2a', border: '1px solid #1565c0', borderRadius: 8, padding: '14px 20px', marginBottom: 16 }}>
            <div style={{ color: '#90caf9', fontWeight: 700, fontSize: 14, marginBottom: 8 }}>
              TCGA Molecular Classification — Endometrial Carcinoma (Prognosis Order)
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 10 }}>
              {[
                { label: '1. POLE (BEST)', desc: 'Ultra-hypermutated · TMB>100 MSS · Exceptional immunotherapy response', color: '#1565c0' },
                { label: '2. MMR-Deficient', desc: 'MSI-H / IHC loss · Lynch or sporadic MLH1-methylation · Pembrolizumab', color: '#00695c' },
                { label: '3. NSMP', desc: 'No specific molecular profile · Intermediate prognosis · Hormone therapy', color: '#f57c00' },
                { label: '4. p53-Abnormal (WORST)', desc: 'Serous / serous-like · TP53 mutation · BRCA1 carriers · Worst prognosis', color: '#b71c1c' },
      ...
      {tab === 'Definitions' && definitions && (
        <div>
          <div style={{ marginBottom: 12, fontSize: 12, color: '#888' }}>
            {definitions.count} clinical definitions · MSH6 MSI-L pitfall · POLE pembrolizumab protocol · PTEN Cowden · Universal MMR IHC · STK11 PJS · BRCA1 TH+BSO · MLH1 methylation vs Lynch · hereditary endometrial differential guide
          </div>
