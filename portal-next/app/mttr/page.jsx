...
const COLOR  = '#37474f';   // blue-grey — tRNA-Arg / CPEO-dominant / H-strand ND3-ND4L
const LIGHT  = '#eceff1';
const COLOR2 = '#546e7a';   // medium blue-grey — CPEO / myopathy
const COLOR3 = '#b71c1c';   // dark red — absolute CIs / contraindications
const COLOR4 = '#1b5e20';   // dark green — biochemical fingerprint / OXPHOS
const COLOR5 = '#0d47a1';   // deep blue — RARS2 DDx / nuclear
...
function OverviewTab({ data }) {
  ...
  const rars2    = data.rars2_ddx_note || {};
  const boundary = data.mttr_nd3_nd4l_boundary_note || {};

  return (
    <>
      {/* Gene identity banner */}
      <SectionCard title="MT-TR — tRNA-Arg (UCG Anticodon) | 16th mt-tRNA | H-strand rCRS 10405–10469 | OMIM *590005">
      ...
      {/* ND3/ND4L boundary note */}
      <SectionCard title={boundary.note_class} borderColor={COLOR2}>
        <p className="small text-muted mb-0">{boundary.detail}</p>
      </SectionCard>

      {/* RARS2 DDx */}
      <SectionCard title={rars2.note_class} borderColor={COLOR5}>
        <p className="small text-muted mb-0">{rars2.detail}</p>
      </SectionCard>
...
function VariantsTab({ data }) {
  ...
  return (
    <>
      <SectionCard title="Per-Variant Breakdown (40-patient cohort, seed-829)">
        {vb.map(v => (
    ...
    <div>
      <div className="py-3 mb-4 rounded" style={{ background: `linear-gradient(135deg, ${COLOR} 0%, ${COLOR2} 100%)`, color: '#fff' }}>
        <div className="container-fluid px-4">
          <h4 className="mb-1 fw-bold">🧬 MT-TR — tRNA-Arg (UCG Anticodon)</h4>
          <p className="mb-1 small opacity-75">
            Combined CI+CIV Deficiency · CPEO + Myopathy · 16th mt-tRNA · H-strand rCRS 10405–10469 ·
            0nt gap MT-ND3 (5') + MT-ND4L (3') · RARS2 Nuclear DDx · KSS (common 4977bp deletion) ·
            OMIM *590005 · 40-patient cohort seed-829
          </p>
        </div>
      </div>
