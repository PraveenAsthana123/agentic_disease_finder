...
const COLOR  = '#1a237e';   // deep indigo — CIII late assembly / RISP chaperone
const LIGHT  = '#e8eaf6';
const COLOR2 = '#283593';
  ...
  return (
    <div>
      ...
      <div className="row mb-4">
        ...
        <KPI label="Infantile onset" value={`${s.infantile_onset_pct}%`} color={COLOR3} />
      ...
      <div className="row">
        ...
        <div className="col-md-6">
          ...
          <SectionCard title="🔑 LYRM7 vs UQCC3 — Critical Distinguisher" borderColor={COLOR4}>
            <ul className="small mb-0 ps-3">
              <li><strong>Immunoblot RISP:</strong> ABSENT in LYRM7 · PRESENT in UQCC3 — KEY differentiator</li>
              <li><strong>BN-PAGE CIII:</strong> Reduced (LYRM7) · Reduced (UQCC3) — similar pattern</li>
              <li><strong>Assembly step:</strong> LYRM7 = late (RISP chaperone) · UQCC3 = early (MT-CYB)</li>
              <li><strong>Chromosome:</strong> 5q33.1 (LYRM7) · 11q12.3 (UQCC3) — WES mandatory</li>
              <li><strong>Also DDx BCS1L:</strong> RISP absent in BOTH — but BCS1L has CIII precomplex</li>
            </ul>
          </SectionCard>
        </div>
      </div>
          ...
          {(data.key_clinical_alerts || []).map((a, i) => (
            <div key={i} className="col-md-6 mb-2">
              <div className="p-2 rounded small" style={{
                background: a.startsWith('🚫') ? '#fff3e0' : a.startsWith('⚠️') ? '#fff8e1' : '#e8eaf6',
            ...
            <tbody>
              {(data.patients || []).map((p, i) => (
                <tr key={i}>
                  ...
                  <td><span style={{ color: p.lactic_acid_mmolL > 8 ? COLOR3 : COLOR }}>{p.lactic_acid_mmolL}</span></td>
...
function VariantsTab({ data }) {
  ...
  return (
    <div>
      <SectionCard title="Pathogenic Variants in LYRM7">
        <div className="table-responsive">
      ...
      <div className="row">
        <div className="col-md-6">
          <SectionCard title="Biochemistry Distribution">
            ...
            <Bar label="CIII activity 15-25%" value={bio.ciii_15to25_pct || 0} color="#e53935" />
            <Bar label="CIII activity >25%" value={bio.ciii_above_25_pct || 0} color={COLOR} />
            <hr />
            <Bar label="Lactate >10 mM" value={bio.lactic_above_10_pct || 0} color={COLOR3} />
            <Bar label="Lactate 6-10 mM" value={bio.lactic_6_to10_pct || 0} color="#e53935" />
            <Bar label="Lactate <6 mM" value={bio.lactic_below_6_pct || 0} color={COLOR} />
          </SectionCard>
        </div>
        <div className="col-md-6">
          ...
          <SectionCard title="Immunoblot Pattern">
            {Object.entries(imm).map(([k, v], i) => (
              <div key={i} className="d-flex justify-content-between small py-1 border-bottom">
                ...
                <span style={{ color: v.includes('ABSENT') ? COLOR3 : COLOR }} className="fw-bold">{v.substring(0, 60)}</span>
              </div>
            ))}
          </SectionCard>
        </div>
  ...
  return (
    <div>
      <SectionCard title="Differential Diagnosis" borderColor={COLOR4}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered mb-0">
            <thead><tr><th>Condition</th><th>Distinguishing from LYRM7</th></tr></thead>
            <tbody>
              {ddx.map((d, i) => (
                <tr key={i}>
      ...
      <SectionCard title="CIII Assembly Cascade — LYRM7 Position" borderColor={COLOR2}>
        <div className="row text-center small">
          {[
            { step: '1a', label: 'UQCC1-UQCC2 heterodimerize', detail: 'Bind nascent MT-CYB (UQCC1 20q11.22)', color: '#1565c0' },
            { step: '2', label: 'UQCC3 cooperates', detail: '1-TM-Helix IMM (11q12.3)', color: '#2e7d32' },
            { step: '3', label: 'TTC19 stabilises', detail: 'TPR scaffold intermediate (17p12)', color: '#4a148c' },
            { step: '4', label: 'LYRM7 binds RISP', detail: 'LYR motif chaperones UQCRFS1 (5q33.1)', color: COLOR },
            { step: '5', label: 'BCS1L inserts RISP', detail: 'Rate-limiting Qo-site insertion (2q35)', color: COLOR3 },
            { step: '6', label: 'Mature CIII', detail: 'All subunits assembled; active bc1', color: '#37474f' },
          ].map((s, i) => (
        ...
        <div className="alert mt-3 mb-0 small" style={{ background: LIGHT, borderLeft: `4px solid ${COLOR}` }}>
          <strong>LYRM7 block effect:</strong> Step 4 fails → RISP (UQCRFS1) degraded in matrix →
          BCS1L (Step 5) has no RISP substrate → CIII assembly stalled at penultimate step.
          CIII core subunits (UQCRC1, UQCRC2) assembled but non-functional without RISP.
          Residual CIII activity ~15-35% (partial core retains some activity).
        </div>
      </SectionCard>
    </div>
  );
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function LYRM7Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/lyrm7/overview`).then(r => r.json()),
      fetch(`${API}/api/lyrm7/breakdown`).then(r => r.json()),
      fetch(`${API}/api/lyrm7/definitions`).then(r => r.json()),
    ...
    <div className="container-fluid py-4">
      <div className="mb-3" style={{ borderBottom: `3px solid ${COLOR}` }}>
        ...
        <p className="text-muted mb-2 small">
          OMIM Gene *615831 · Disease #615834 · AR biallelic · 5q33.1 · 40-patient cohort (seed 723)
        </p>
