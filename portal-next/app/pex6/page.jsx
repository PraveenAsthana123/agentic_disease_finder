...
const ACCENT2 = '#b71c1c';   // dark red — ABSOLUTE/HIGH RISK CI / VPA / VGB
...
const ACCENT5 = '#4a148c';   // deep purple — molecular biology / PEX6 AAA-ATPase D2 mechanism
const ACCENT6 = '#00695c';   // teal — DHA supplementation / R860W / NBS / supportive care
    ...
    <div className="card mb-2 shadow-sm" style={{ borderLeft: `4px solid ${color}` }}>
      <div className="card-body py-2 px-3">
        <div className="d-flex justify-content-between align-items-start mb-1">
          ...
          <Badge text={level?.split('(')[0]?.trim().split(' ').slice(0, 2).join(' ')} color={color} />
        </div>
...
function OverviewTab({ data }) {
  ...
  return (
    <div>
      ...
      />
      <Alert
        text="🚨 VGB (Vigabatrin) — HIGH RISK: ZSD retinopathy (universal ZS/NALD; 25–40% IRD) + VGB irreversible VF loss = additive blindness. ACTH (Level A) preferred for infantile spasms. AVOID VGB in IRD entirely."
        variant="danger"
      />
      <Alert
      ...
      />
      <Alert
        text={`ℹ PEX6 = second most common PBD-ZSD cause (~10% of all ZSD; PEX1+PEX6 ≈ 75% together). p.Arg860Trp (R860W) = most common European hypomorphic allele (~18% of PEX6 alleles). PEX6 cohorts show MORE IRD (45%) vs PEX1 (30–35%) — R860W provides more residual PEX5 recycling than PEX1-G843D. Plasmalogens (RBC) LOW — KEY DISTINCTION from ABCD1 (plasmalogens NORMAL). Biochemically identical to PEX1-ZSD; only gene panel distinguishes.`}
        variant="info"
      />
      ...
      <SectionCard title="Disease Summary — PEX6 / Zellweger Spectrum Disorder (ZSD)" borderColor={ACCENT}>
      ...
      <div className="row">
        <div className="col-md-6">
          <SectionCard title="Clinical Profile" borderColor={ACCENT4}>
            ...
            <PctBar label="NALD (intermediate — R860W/null)" pct={d.nald_pct} color={ACCENT3} />
            <PctBar label="IRD (attenuated — R860W/R860W)" pct={d.ird_pct} color={ACCENT} />
      ...
      <SectionCard title="Key Pharmacological Distinctions" borderColor={ACCENT2}>
        ...
        <Alert text="PHT/CBZ/OXC — RELATIVE CI (hepatic + DHA depletion via CYP3A4). NOT adrenal crisis mechanism (unlike ABCD1). IV LEV replaces fosphenytoin in SE." variant="warning" />
        <Alert text="DHA supplementation Level B (NALD/IRD). ACTH Level A for IS. LEV first-line all forms. Phytol-restricted diet (IRD). No ERT / No HSCT / Lorenzo's Oil ineffective." variant="info" />
      </SectionCard>
...
function PatientsTab({ data }) {
  ...
  const ETIOL_COLORS = {
    ...
    'Atypical / Late-Onset ZSD (PEX6)': ACCENT6,
  };
  const getColor = (name) => {
  ...
  return (
    <div>
      <Alert
        text="ℹ ZSD SPECTRUM (PEX6): R860W/R860W → IRD (attenuated, 45%) · R860W/null → NALD-like (32%) · null/null → ZS (severe, 18%) · Atypical (5%). PEX6 cohorts show MORE IRD vs PEX1 cohorts because p.Arg860Trp provides more residual PEX5 recycling activity than PEX1-p.Gly843Asp. Genotype PARTIALLY predictive (like PEX1, unlike ABCD1)."
        variant="info"
      />
      <h6 className="fw-bold mb-3" style={{ color: ACCENT }}>PEX6-ZSD Phenotypic Classes — 4 Classes (40 Patients)</h6>
      {etiologies?.map((e, i) => (
...
function TreatmentsTab({ data }) {
  ...
  return (
    <>
      <Alert
        text="ℹ LEV = first-line AED (all ZSD forms). DHA supplementation Level B (NALD/IRD). ACTH Level A for infantile spasms (preferred over VGB due to retinopathy). Phytol-restricted diet (IRD + Atypical). Cholic acid experimental. No ERT / No HSCT / Lorenzo's Oil ineffective."
        variant="info"
      />
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function PEX6Page() {
  ...
  useEffect(() => {
    if (tab >= 1 && tab <= 3) {
      fetch(`${API}/api/pex6/breakdown`).then(r => r.json()).then(setBreakdown).catch(() => {});
    }
    if (tab === 4) {
      fetch(`${API}/api/pex6/definitions`).then(r => r.json()).then(setDefinitions).catch(() => {});
    }
  }, [tab]);
    ...
    <div className="container-fluid py-4" style={{ maxWidth: 1200 }}>
      <div className="mb-4">
        <h2 className="fw-bold mb-1" style={{ color: ACCENT }}>
          PEX6 Epilepsy — Zellweger Spectrum Disorder (ZSD)
        </h2>
        <p className="text-muted small mb-2">
          PEX6 (6p21.1) · AAA-ATPase D2 motor subunit · PEX1–PEX6 heterodimer · PEX26-anchored ·
          AR biallelic LOF · p.Arg860Trp (R860W) European hypomorphic · ~10% of all ZSD ·
          R860W/R860W → IRD 45% · R860W/null → NALD 32% · null/null → ZS 18% ·
