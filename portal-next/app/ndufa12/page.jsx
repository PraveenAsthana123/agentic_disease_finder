...
const COLOR = '#01579b';   // dark blue — N-module/Q-module interface, peripheral arm assembly-swap
const LIGHT = '#e3f2fd';
  ...
  return (
    <>
      <div className="row g-2 mb-3">
        ...
        <KPI label="Also Known As" value={data.also_known_as || 'B17.2 / N/Q Interface'} color={COLOR} />
        <KPI label="Size"          value={`${pt.size_kda} kDa`}                         color={COLOR} />
        <KPI label="Fe-S Cluster"  value="None"                                         color="#455a64" />
        <KPI label="Module"        value="N/Q Interface"                                color="#01579b" />
        <KPI label="Swap"          value="NDUFAF2→NDUFA12"                              color="#0277bd" />
        <KPI label="Inheritance"   value="AR biallelic"                                 color={COLOR} />
        <KPI label="Chromosome"    value={data.chromosome}                              color={COLOR} />
        <KPI label="OMIM Gene"     value={`*${data.omim_gene}`}                         color="#0288d1" />
        <KPI label="OMIM Disease"  value={`#${data.omim_disease}`}                      color="#0288d1" />
      ...
      <SectionCard title="Clinical Feature Frequencies (40-patient cohort, seed-633)">
        ...
        <div className="table-responsive">
          <table className="table table-sm table-bordered small">
            <thead style={{ background: LIGHT }}>
              <tr>
                <th>Feature</th><th>NDUFA12</th><th>Comparator</th><th>Significance</th>
              </tr>
            </thead>
            <tbody>
      ...
      <SectionCard title="Mutation Distribution">
        {Object.entries(md).map(([k, v]) => (
          <div key={k} className="d-flex justify-content-between small mb-1">
            ...
            <span className="badge" style={{ background: '#0277bd' }}>{v}</span>
          </div>
        ))}
      </SectionCard>
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function NDUFA12Page() {
  ...
  useEffect(() => {
    fetch(`${API}/api/ndufa12/overview`)
      .then(r => r.json()).then(setOverview).catch(e => setError(e.message));
    fetch(`${API}/api/ndufa12/breakdown`)
      .then(r => r.json()).then(setBreakdown).catch(() => {});
    fetch(`${API}/api/ndufa12/definitions`)
    ...
    <div className="container-fluid py-3">
      <div className="mb-3" style={{ borderBottom: `3px solid ${COLOR}` }}>
        ...
        <p className="text-muted small mb-1">
          Isolated Complex I Deficiency · N-Module/Q-Module Interface · B17.2 · NDUFAF2-Paralog Assembly-Swap ·
          OMIM *614846 / #256000 / #618234 · 12q22 · AR
        </p>
