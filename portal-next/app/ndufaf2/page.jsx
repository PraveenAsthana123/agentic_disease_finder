...
const COLOR = '#1a237e';   // deep indigo — CI Assembly Factor / assembly-swap chaperone
const LIGHT = '#e8eaf6';
  ...
  return (
    <>
      <div className="row g-3 mb-4">
        ...
        <KPI label="Also known as"     value="NDUFA12L"                   color={COLOR} />
      ...
      <SectionCard title="🔄 Key Pathway Note — Assembly-Factor Swap (NDUFAF2 → NDUFA12) / CI Assembly Factor 2 / NDUFA12L">
      ...
      <SectionCard title="📊 Feature Frequencies (40-patient cohort, seed-673)">
...
function PatientsTab({ data }) {
  ...
  return (
    <>
      ...
      <SectionCard title="🗂 Patient Cohort (40 patients, seed-673)">
      ...
      <SectionCard title="🔬 Technical Details">
        <div className="row g-2">
          {[
            ...
            ['OMIM Disease (Leigh)', `#${data.omim_disease}`],
            ['OMIM Disease (MC1DN25)', `#${data.omim_disease_specific}`],
            ['Chromosome', data.chromosome],
            ['Inheritance', data.inheritance_detail],
            ['Protein (precursor)', `${data.protein_size_precursor_aa} aa`],
            ['Protein (mature)', `${data.protein_size_mature_aa} aa`],
            ['Protein size', `${data.protein_size_kda} kDa`],
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function NDUFAF2Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/ndufaf2/overview`).then(r => r.json()),
      fetch(`${API}/api/ndufaf2/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ndufaf2/definitions`).then(r => r.json()),
  ...
  return (
    <div className="container-fluid py-4">
      <div className="mb-4 p-3 rounded" style={{ background: COLOR, color: '#fff' }}>
        <h4 className="mb-1 fw-bold">&#x1f9ec; NDUFAF2 — Leigh Syndrome Isolated Complex I Deficiency</h4>
        <div className="small opacity-90">
          CI-Leigh · CI Assembly Factor 2 / NDUFA12L · Assembly-Factor-Swap Chaperone ·
          12.5 kDa · 137aa precursor / 107aa mature · 5q12.1 · OMIM Gene *609653 · AR biallelic · Isolated CI 5–20% · CII/CIII/CIV NORMAL
        </div>
      </div>
