...
const GENE_COLORS = {
  HBB:    '#b71c1c',   // deep red — sickle haemoglobin / beta-globin
  HBA1:   '#c62828',   // medium red — alpha-globin
  G6PD:   '#e65100',   // deep orange — G6PD enzymopathy
  PKLR:   '#f57f17',   // amber — pyruvate kinase
  ANK1:   '#1565c0',   // deep blue — ankyrin / HS
  SPTA1:  '#283593',   // indigo — spectrin / HPP
  SLC4A1: '#1b5e20',   // deep green — band 3 / SAO
  PIEZO1: '#4a148c',   // deep purple — mechanosensitive / xerocytosis
};
...
function Loading() {
  return (
    <div className="text-center py-5">
      ...
      <p className="mt-3 text-muted">Loading hemoglobinopathy atlas…</p>
    </div>
  );
}
...
function OverviewTab({ data }) {
  ...
  return (
    <div>
      {/* Hero */}
      <div className="rounded-3 p-4 mb-4 text-white" style={{ background: 'linear-gradient(135deg,#b71c1c,#4a148c)' }}>
        <h3 className="mb-1">{ov.atlas_name || 'Hemoglobinopathy Atlas'}</h3>
      ...
      <div className="row mt-2">
        <div className="col-md-6">
          <div className="card border-0 shadow-sm mb-3">
            <div className="card-header fw-semibold" style={{ background: '#ffebee' }}>Clinical Rates (Aggregate)</div>
            <div className="card-body">
              <BarRow label="ESRD Rate"          pct={agg.esrd_pct}               color="#b71c1c" />
              <BarRow label="Hypertension"        pct={agg.hypertension_pct}       color="#c62828" />
              ...
              <BarRow label="Surveillance OK"     pct={agg.surveillance_adherent_pct} color="#1565c0" />
            </div>
          </div>
        </div>
        <div className="col-md-6">
          ...
          <div className="card border-0 shadow-sm mb-3">
            <div className="card-header fw-semibold" style={{ background: '#ffebee' }}>Severity Distribution</div>
            <div className="card-body">
      ...
      {ov.disease_category_breakdown && (
        <div className="card border-0 shadow-sm mb-3">
          <div className="card-header fw-semibold" style={{ background: '#ffebee' }}>Disease Category Distribution</div>
          <div className="card-body">
            {Object.entries(ov.disease_category_breakdown).map(([cat, pct], i) => (
              <BarRow key={i} label={cat} pct={pct} color={Object.values(GENE_COLORS)[i * 2 % 8]} />
            ))}
          </div>
        </div>
      )}
...
function GeneTableTab({ data }) {
  ...
  return (
    <div>
      <h5 className="mb-3">8-Gene Hereditary Red Cell Disorder Reference Table</h5>
          ...
          <tbody>
            {genes.map((g) => (
              <tr key={g.gene}>
                ...
                <td className="text-center" style={{ color: g.cohort_stats?.drug_error_pct > 25 ? '#b71c1c' : '#1b5e20' }}>
                  {g.cohort_stats?.drug_error_pct}%
                </td>
                <td className="text-center" style={{ color: g.cohort_stats?.dx_delayed_pct > 50 ? '#b71c1c' : '#1b5e20' }}>
    ...
    <div>
      ...
      {gene && (
        <div>
          ...
          <div className="card border-0 shadow-sm mb-3">
            <div className="card-header fw-semibold">Haematology Pattern</div>
            <div className="card-body small">
              <div><strong>GFR / Renal:</strong> {gene.gfr_pattern}</div>
            ...
            <div className="card-body">
              <div className="row">
                <div className="col-md-6">
                  ...
                  <BarRow label="Hypertension"    pct={gene.cohort_stats?.htn_pct}           color="#c62828" />
...
function DefinitionsTab({ data }) {
  ...
  return (
    <div>
      <h5 className="mb-3">Clinical Definitions — Hereditary Red Cell Disorder Terms</h5>
      {defs.map((d, i) => (
        <div key={i} className="card border-0 shadow-sm mb-3">
          <div className="card-header fw-bold" style={{ background: '#ffebee' }}>
            {d.term}
...
/* ── Main page ─────────────────────────────────────────────────────────── */
export default function HemoglobinopathyAtlasPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/hemoglobinopathy-atlas/overview`).then(r => r.json()),
      fetch(`${API}/api/hemoglobinopathy-atlas/breakdown`).then(r => r.json()),
      fetch(`${API}/api/hemoglobinopathy-atlas/definitions`).then(r => r.json()),
    ])
      .then(([ov, bd, df]) => {
        setOverview(ov);
        setBreakdown(bd);
