...
const COLOR = '#1a237e';   // deep indigo — AR / N-module peripheral / B14.5a / NDUFS1-contact / no TM helix
const LIGHT = '#e8eaf6';
    ...
    <div>
      <SectionCard title="Gene & Disease Identity">
        <div className="row g-2 small">
          {[
            ['Gene', data.gene],
            ['Protein', 'NADH:Ubiquinone Oxidoreductase Subunit A7 (B14.5a)'],
            ['Module', data.module],
...
function PatientsTab({ data }) {
  ...
  return (
    <div>
      ...
      <SectionCard title="Patient Cohort (40 patients, seed 661)">
...
// ── Root component ────────────────────────────────────────────────────────────
export default function NDUFA7Page() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/ndufa7/overview`).then(r => r.json()),
      fetch(`${API}/api/ndufa7/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ndufa7/definitions`).then(r => r.json()),
    ])
  ...
  return (
    <div className="container-fluid py-3">
      <div className="mb-3 p-3 rounded" style={{ background: COLOR, color: '#fff' }}>
        <h4 className="mb-1 fw-bold">🧬 NDUFA7 — Leigh Syndrome / Isolated Complex I Deficiency</h4>
        <div className="small opacity-75">
          B14.5a · N-module Peripheral Stabiliser · NDUFS1 Contact · No TM Helix · AR · 19p13.3 ·
          OMIM Gene *601796 · Disease #256000
        </div>
      </div>

      {loading && <div className="alert alert-info">Loading NDUFA7 dashboard…</div>}
