...
const COLOR  = '#4a148c';   // deep purple — tRNA-Tyr / L-strand / ELEVENTH tRNA
const LIGHT  = '#f3e5f5';
...
const COLOR5 = '#1a237e';   // deep indigo — YARS2 DDx / nuclear gene
...
function OverviewTab({ data }) {
  ...
  return (
    <>
      {/* Gene facts banner */}
      <SectionCard title="Gene Facts — MT-TY tRNA-Tyr" borderColor={COLOR}>
      ...
      {data.yars2_ddx_note && (
        <div className="alert border-0 mb-4" style={{ backgroundColor: '#e8eaf6', borderLeft: `5px solid ${COLOR5}` }}>
          <strong style={{ color: COLOR5 }}>🧬 {data.yars2_ddx_note.note_class}</strong>
          <p className="mb-0 small mt-1">{data.yars2_ddx_note.detail}</p>
        </div>
      )}
...
function VariantsTab({ data }) {
  ...
  return (
    <>
      <SectionCard title="Variant Breakdown — 40-Patient Cohort (seed-821)" borderColor={COLOR}>
        <div className="table-responsive">
          <table className="table table-sm table-hover small mb-0">
            <thead><tr>
      ...
      <SectionCard title="Differential Diagnosis Table" borderColor={COLOR2}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small mb-0">
            ...
            <tbody>
              {ddx.map((r, i) => (
                <tr key={i} style={r.entity.startsWith('MT-TY') ? { backgroundColor: LIGHT, fontWeight: 600 } : {}}>
                  <td>{r.entity}</td>
                  <td>{r.inheritance}</td>
                  <td>{r.phenotype}</td>
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function MTTYPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/mtty/overview`).then(r => r.json()),
      fetch(`${API}/api/mtty/breakdown`).then(r => r.json()),
      fetch(`${API}/api/mtty/definitions`).then(r => r.json()),
    ])
  ...
  return (
    <div className="container-fluid py-4">
      ...
      <div className="mb-4 p-3 rounded" style={{ background: `linear-gradient(135deg, ${COLOR} 0%, ${COLOR2} 100%)`, color: '#fff' }}>
        <h4 className="fw-bold mb-1">🧬 MT-TY — tRNA-Tyr (GUA Anticodon)</h4>
        <p className="mb-1 small opacity-75">
          Combined CI+CIV Deficiency · CPEO · Myopathy · L-strand NGS Pitfall — ELEVENTH tRNA — rCRS 5826–5891
        </p>
        <div className="d-flex gap-3 flex-wrap small">
          <span className="badge bg-light text-dark">OMIM *590100</span>
          <span className="badge bg-light text-dark">L-strand — NGS Pitfall</span>
          <span className="badge bg-light text-dark">40-patient cohort seed-821</span>
          <span className="badge bg-light text-dark">FOURTH &amp; FINAL of 4 consecutive L-strand tRNAs</span>
          <span className="badge bg-light text-dark">YARS2 DDx — MLASA2 (Sideroblastic Anemia)</span>
        </div>
      </div>
