...
const ACCENT  = '#1a237e';   // deep indigo — PCC complex / CT domain / PCCB
...
const ACCENT6 = '#1b5e20';   // dark green — KEY NEGATIVES / absent methylmalonate
...
export default function PCCBPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/pccb/overview`).then(r => r.json()),
      fetch(`${API}/api/pccb/breakdown`).then(r => r.json()),
      fetch(`${API}/api/pccb/definitions`).then(r => r.json()),
    ]).then(([o, b, d]) => { setOv(o); setBk(b); setDf(d); })
      .catch(e => setErr(e.message));
  }, []);

  if (err) return <div className="alert alert-danger m-3">Error: {err}</div>;
  if (!ov)  return <div className="text-center p-5 text-muted">Loading PCCB Dashboard…</div>;
    ...
    <div className="container-fluid px-3 py-3">
      ...
      <div className="mb-3">
        <h4 className="fw-bold mb-1" style={{ color: ACCENT }}>
          🧬 PCCB Epilepsy — Propionic Acidemia Type B (Propionyl-CoA Carboxylase Beta Subunit Deficiency)
        </h4>
        <div className="text-muted small">
          PCCB · 3q22.3 · AR · OMIM *232050 / #606054 · 559 aa · CT (Carboxyl Transferase) domain ·
          (αβ)₆ dodecamer with PCCA · Step 2: carboxybiotin-PCCA → propionyl-CoA → D-methylmalonyl-CoA ·
          NBS: C3 (propionylcarnitine) elevated IDENTICAL to PCCA · Methylcitrate PATHOGNOMONIC ·
      ...
      {tab === 0 && (
        <div>
          <Alert
            variant="danger"
            text="⚠️ PROPIONIC ACIDEMIA TYPE B — ACUTE DECOMPENSATION: Hyperammonemia (NH₃ > 300 µmol/L = EMERGENT), metabolic acidosis + ketoacidosis, hypoglycemia. FIRST response: IV glucose GIR 8-12 + STOP PROTEIN + ammonia scavengers. VPA ABSOLUTE CI — valproyl-CoA directly inhibits PCCB CT domain + depletes carnitine: FATAL. Fasting = EXTREME HAZARD. Always give IV glucose in intercurrent illness. Biotin does NOT help (PCCB CT domain is the defective enzyme — not a biotin disorder)."
          />
          ...
          <Section title="Gene & Disease Overview" color={ACCENT}>
            <div className="row g-2 mb-2">
              {[
                ['Gene',          ov.gene || 'PCCB'],
                ['Full Name',     ov.full_name || 'Propionyl-CoA Carboxylase Beta Subunit'],
                ['Chromosome',    ov.chromosome || '3q22.3'],
                ['Inheritance',   ov.inheritance || 'AR'],
                ['OMIM Gene',     ov.omim_gene || '*232050'],
                ['OMIM Disease',  ov.omim_disease || '#606054'],
                ['Protein',       ov.protein_size || '559 aa; CT domain'],
                ['Prevalence',    ov.prevalence || '1:100,000–150,000 (EU)'],
                ['NBS Primary',   ov.nbs_primary || 'C3 (propionylcarnitine) elevated — identical to PCCA'],
          ...
          {/* PCC Pathway */}
          <Section title="PCC Two-Step Reaction Mechanism (PCCB performs Step 2 — CT domain)" color={ACCENT8}>
            <div className="row g-2">
              {(ov.pcc_pathway || []).map((s, i) => (
                <div key={i} className="col-12 col-md-6">
                  <div className="card border-0 shadow-sm h-100">
                    <div className="card-body py-2 px-3">
                      <div className="fw-bold small" style={{ color: i === 1 ? ACCENT2 : ACCENT8 }}>{s.step}</div>
                      ...
                      <div className="mt-1 small" style={{ color: ACCENT2 }}>
                        <strong>PCCB LOF consequence:</strong> {s.loss_when_PCCB_mutant}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="table-responsive">
                <table className="table table-sm table-bordered" style={{ fontSize: 12 }}>
                  <thead className="table-dark">
                    <tr>
                      <th>Feature</th>
                      <th style={{ color: '#ce93d8' }}>PCCA (comparator)</th>
                      <th style={{ color: '#80cbc4' }}>PCCB (this disease)</th>
                    </tr>
                  </thead>
                  <tbody>
      ...
      {tab === 1 && bk && (
        <div>
          ...
          {/* Key Variants */}
          <Section title="Key Pathogenic Variants in PCCB" color={ACCENT7}>
            <div className="table-responsive">
              <table className="table table-sm table-hover" style={{ fontSize: 12 }}>
                <thead className="table-dark">
      ...
      {tab === 2 && bk && (
        <div>
          {/* Seizure Types */}
          <Section title="Seizure Types in Propionic Acidemia Type B" color={ACCENT2}>
            <div className="row g-2">
      ...
      {tab === 3 && bk && (
        <div>
          <Alert
            variant="info"
            text="ℹ️ PCCB TREATMENT KEY: Biochemically identical to PCCA — same treatments apply. L-Carnitine (LEVEL A) — conjugates propionyl-CoA to C3 (propionylcarnitine) for renal excretion; secondary carnitine depletion is a major PA complication. Liver transplant (LEVEL B) corrects ~75% hepatic PCC but does NOT prevent cardiac complications (extra-hepatic PCCB remains deficient — myocardium, skeletal muscle). Biotin is NOT effective in PA (PCCB CT domain is the defective enzyme — not a biotin recycling defect)."
          />
          <div className="row g-2">
      ...
      {tab === 4 && df && (
        <div>
          {/* Gene Card */}
          <Section title="Gene Card — PCCB" color={ACCENT7}>
            <div className="table-responsive">
              <table className="table table-sm table-bordered" style={{ fontSize: 12 }}>
                <tbody>
