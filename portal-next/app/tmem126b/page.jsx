...
const COLOR = '#006064';   // dark teal — TMEM126B / 2-TM integral IMM protein / MCIA tetramer
const LIGHT = '#e0f7fa';
  ...
  return (
    <>
      ...
      <div className="alert mb-4" style={{ background: '#ffebee', borderLeft: '4px solid #c62828' }}>
        <strong>🔴 NO Riboflavin Response — Critical DDx vs ACAD9</strong> — TMEM126B has NO FAD-binding domain.
        High-dose riboflavin does NOT rescue TMEM126B deficiency. If MCIA-type CI deficiency shows riboflavin
        response: ACAD9 is the diagnosis. No response → consider TMEM126B / ECSIT / NDUFAF1 (after excluding ACAD9).
      </div>

      <div className="alert mb-4" style={{ background: LIGHT, borderLeft: `4px solid ${COLOR}` }}>
        <strong>🟦 TMEM126B — Terminal Integral IMM Member of the MCIA Tetramer</strong> — TMEM126B is the 4th
        and final member recruited into the MCIA complex (via ECSIT). With 2 transmembrane helices, it is the
        ONLY integral membrane protein in the MCIA tetramer (ACAD9, NDUFAF1, ECSIT are all soluble/peripheral).
        Its TM helices anchor the MCIA scaffold to the IMM during ND2/ND5 membrane arm biogenesis.
      </div>
      ...
      <SectionCard title="🔄 Key Pathway Note — MCIA Complex / 2-TM Integral IMM / No Riboflavin Response">
      ...
      {mc.tetramer && (
        <SectionCard title="⚙️ MCIA Tetramer Assembly Order">
          ...
          <p className="small text-muted mt-2 mb-0">{mc.tmem126b_unique_position}</p>
        </SectionCard>
      )}
      ...
      <SectionCard title="📊 Feature Frequencies (40-patient cohort, seed-681)">
...
function MCIATab({ data }) {
  ...
  const stepColors = ['#1b5e20', '#2e7d32', '#1565c0', '#c62828'];
    ...
    <>
      <div className="alert mb-4" style={{ background: LIGHT, borderLeft: `4px solid ${COLOR}` }}>
        ...
        <span className="small text-muted">
          TMEM126B joins last as the 4th member, recruited by ECSIT. It is the ONLY integral membrane protein
          in the MCIA tetramer (2 TM helices — IMM anchor). Loss of TMEM126B: ACAD9-NDUFAF1-ECSIT ternary
          still intact, but the tetramer cannot complete → ND2/ND5 biogenesis stalled.
        </span>
      </div>
              ...
              <div className="flex-grow-1">
                ...
                <div className="mb-1">
                  <span className="badge" style={{
                    background: step.status_in_tmem126b_deficiency.startsWith('INTACT') ? '#2e7d32' : '#c62828',
                    fontSize: '0.7rem'
                  }}>
                    {step.status_in_tmem126b_deficiency}
                  </span>
                </div>
          ...
          <table className="table table-sm small">
            ...
            <tbody>
              {[
                ...
                { member: 'NDUFAF1',  step: '2nd — CIA30 binary',      chr: '15q11.2-q13', ribo: 'None (0%)',        feat: 'First CI assembly factor ever discovered (Kuffner 1998, Neurospora)' },
                ...
                { member: 'TMEM126B', step: '4th — integral IMM',      chr: '11q14.1',     ribo: 'None (0%)',        feat: '2 TM helices — ONLY integral membrane MCIA member; IMM anchor; recruited by ECSIT' },
              ].map(r => (
                <tr key={r.member} style={{ background: r.member === 'TMEM126B' ? LIGHT : undefined }}>
                  <td className="fw-bold">
                    {r.member}
                    {r.member === 'TMEM126B' && (
                      <span className="badge ms-1" style={{ background: COLOR, fontSize: '0.6rem' }}>YOU ARE HERE</span>
                    )}
                  </td>
                  <td>{r.step}</td>
                  <td className="font-monospace">{r.chr}</td>
                  <td>
                    <span className="badge" style={{ background: r.ribo.includes('None') ? '#c62828' : '#1565c0', fontSize: '0.65rem' }}>
                      {r.ribo}
                    </span>
                  </td>
      ...
      <SectionCard title="🏗️ TMEM126B Structural Distinction — 2-TM Integral IMM" borderColor={COLOR}>
        <div className="alert mb-3" style={{ background: LIGHT, borderLeft: `4px solid ${COLOR}` }}>
          <strong>Only MCIA member with transmembrane helices.</strong> ACAD9, NDUFAF1, and ECSIT are all
          soluble or peripherally membrane-associated proteins. TMEM126B spans the inner mitochondrial
          membrane with TM1 and TM2, providing the integral IMM anchor for the MCIA scaffold during
          ND2/ND5 proton-pump subunit insertion.
        </div>
        <div className="table-responsive">
          <table className="table table-sm small">
            <thead><tr><th>Member</th><th>Topology</th><th>TM Helices</th><th>IMM Anchor</th></tr></thead>
            <tbody>
              {[
                { m: 'ACAD9',    topo: 'Soluble / peripheral matrix', tm: 0, anchor: '—' },
                { m: 'NDUFAF1',  topo: 'Soluble / peripheral matrix', tm: 0, anchor: '—' },
                { m: 'ECSIT',    topo: 'Soluble / peripheral matrix', tm: 0, anchor: '—' },
                { m: 'TMEM126B', topo: '2-TM integral IMM protein',   tm: 2, anchor: '✅ YES — unique' },
              ].map(r => (
                <tr key={r.m} style={{ background: r.m === 'TMEM126B' ? LIGHT : undefined }}>
                  <td className="fw-bold">{r.m}</td>
                  <td>{r.topo}</td>
                  <td className="text-center"><span className="badge" style={{ background: r.tm > 0 ? COLOR : '#9e9e9e' }}>{r.tm}</span></td>
                  <td style={{ color: r.tm > 0 ? COLOR : undefined }}>{r.anchor}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function TMEM126BPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/tmem126b/overview`).then(r => r.json()),
      fetch(`${API}/api/tmem126b/breakdown`).then(r => r.json()),
      fetch(`${API}/api/tmem126b/definitions`).then(r => r.json()),
  ...
  return (
    <div className="container-fluid py-3">
      <div className="d-flex align-items-center mb-3 flex-wrap gap-2">
        <h4 className="fw-bold mb-0" style={{ color: COLOR }}>🧬 TMEM126B — Complex I Deficiency</h4>
        ...
        <span className="badge ms-1" style={{ background: '#1565c0' }}>2-TM Integral IMM</span>
        <span className="badge ms-1" style={{ background: '#6a1b9a' }}>Terminal MCIA Member</span>
        <span className="badge ms-1 bg-secondary">11q14.1</span>
        <span className="badge ms-1 bg-secondary">OMIM *615533</span>
