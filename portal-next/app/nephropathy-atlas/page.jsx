...
const GENE_COLORS = {
  PKD1:    '#1565c0',   // deep blue — polycystic
  PKD2:    '#0288d1',   // medium blue — polycystin-2
  COL4A5:  '#6a1b9a',   // deep purple — Alport XL
  COL4A3:  '#4a148c',   // darker purple — Alport AR
  NPHS1:   '#1b5e20',   // deep green — nephrin/slit diaphragm
  NPHS2:   '#2e7d32',   // medium green — podocin/FSGS
  UMOD:    '#e65100',   // deep orange — uromodulin/gout
  HNF1B:   '#880e4f',   // dark magenta — HNF1B/MODY5
};
...
function Loading() {
  return (
    <div className="text-center py-5">
      ...
      <p className="mt-3 text-muted">Loading nephropathy atlas…</p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="rounded-3 p-4 mb-4 text-white" style={{ background: 'linear-gradient(135deg,#1565c0,#6a1b9a)' }}>
        <h3 className="mb-1">{ov.atlas_name || 'Nephropathy Atlas'}</h3>
        ...
        <div className="col-md-6">
          <div className="card border-0 shadow-sm mb-3">
            ...
            <div className="card-body">
              <BarRow label="ESRD Rate"          pct={agg.esrd_pct}             color="#1565c0" />
              <BarRow label="Hypertension"        pct={agg.hypertension_pct}     color="#6a1b9a" />
              <BarRow label="Transplant Rate"     pct={agg.transplant_rate_pct}  color="#1b5e20" />
              ...
              <BarRow label="Surveillance OK"     pct={agg.surveillance_adherent_pct} color="#2e7d32" />
            </div>
          </div>
        </div>
        <div className="col-md-6">
              ...
              {Object.entries(pathways).map(([key, pathway]) => (
                <div key={key} className="d-flex align-items-start mb-2">
                  <span className="badge me-2 flex-shrink-0"
                    style={{ background: GENE_COLORS[key] || '#6c757d', fontSize: '0.72rem', minWidth: 70 }}>
                    {key}
                  </span>
          ...
          <div className="card border-0 shadow-sm mb-3">
            ...
            <div className="card-body">
              <BarRow label="Mild"     pct={agg.severity_mild_pct}     color="#2e7d32" />
      ...
      {ov.disease_category_breakdown && (
        <div className="card border-0 shadow-sm mb-3">
          <div className="card-header fw-semibold" style={{ background: '#e3f2fd' }}>Disease Category Distribution</div>
          <div className="card-body">
...
function GeneTableTab({ data }) {
  ...
  return (
    <div>
      <h5 className="mb-3">8-Gene Hereditary Nephropathy Reference Table</h5>
            ...
            {genes.map((g) => (
              <tr key={g.gene}>
                ...
                <td className="text-center fw-bold" style={{ color: '#b71c1c' }}>
                  {g.cohort_stats?.esrd_pct}%
                </td>
                <td className="text-center">
                  {g.cohort_stats?.transplant_pct}%
                </td>
          ...
          <div key={g.gene} className="col-md-6 mb-3">
            <div className="card border-0 shadow-sm h-100">
              <div className="card-header fw-semibold small"
                style={{ background: GENE_COLORS[g.gene] + '22', borderLeft: `4px solid ${GENE_COLORS[g.gene]}` }}>
                <span className="badge me-2" style={{ background: GENE_COLORS[g.gene] }}>{g.gene}</span>
                {g.protein}
              </div>
              <div className="card-body small">
                <div className="mb-1"><strong>Hallmark:</strong> {g.hallmark}</div>
                <div className="mb-1"><strong>Treatment Alert:</strong> <span className="text-danger">{g.treatment_alert?.substring(0, 120)}…</span></div>
                <div className="mb-2"><strong>GFR Pattern:</strong> {g.gfr_pattern}</div>
                <BarRow label="ESRD"     pct={g.cohort_stats?.esrd_pct}       color={GENE_COLORS[g.gene]} />
                <BarRow label="Severe"   pct={g.cohort_stats?.severity?.Severe} color="#b71c1c" />
                <BarRow label="Adherent" pct={g.cohort_stats?.adherent_pct}   color="#2e7d32" />
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="d-flex flex-wrap gap-2 mb-4">
        {genes.map(g => (
          <button
            ...
            style={selected === g.gene ? { background: GENE_COLORS[g.gene], borderColor: GENE_COLORS[g.gene] } : {}}
        ...
        <div>
          <div className="card border-0 shadow mb-3">
            <div className="card-header text-white fw-bold"
              style={{ background: GENE_COLORS[gene.gene] }}>
            ...
            <div className="card-body">
              <div className="row mb-3">
                ...
                <div className="col-md-4">
                  <small className="text-muted">OMIM</small>
                  <div className="fw-bold">Gene {gene.omim_gene} · Disease {gene.omim_disease}</div>
                </div>
              </div>
              <div className="mb-2">
          ...
          <div className="card border-warning border-2 mb-3">
            <div className="card-header bg-warning fw-bold">Treatment Alert</div>
          ...
          <div className="card border-0 shadow-sm mb-3">
            <div className="card-header fw-semibold">GFR / Proteinuria Pattern</div>
            <div className="card-body small">
              <div><strong>GFR:</strong> {gene.gfr_pattern}</div>
          ...
          <div className="card border-0 shadow-sm mb-3">
            ...
            <div className="card-body">
              <div className="row">
                <div className="col-md-6">
                  <BarRow label="ESRD"            pct={gene.cohort_stats?.esrd_pct}       color={GENE_COLORS[gene.gene]} />
                  <BarRow label="Hypertension"    pct={gene.cohort_stats?.htn_pct}        color="#6a1b9a" />
                  <BarRow label="Transplant"      pct={gene.cohort_stats?.transplant_pct} color="#1b5e20" />
                </div>
                <div className="col-md-6">
                  <BarRow label="Drug Error"      pct={gene.cohort_stats?.drug_error_pct} color="#b71c1c" />
                  <BarRow label="Delayed Dx"      pct={gene.cohort_stats?.dx_delayed_pct} color="#e65100" />
...
function DefinitionsTab({ data }) {
  ...
  return (
    <div>
      <h5 className="mb-3">Clinical Definitions — Hereditary Nephropathy Terms</h5>
      {defs.map((d, i) => (
...
/* ── Main page ─────────────────────────────────────────────────────────── */
export default function NephropathyAtlasPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/nephropathy-atlas/overview`).then(r => r.json()),
      fetch(`${API}/api/nephropathy-atlas/breakdown`).then(r => r.json()),
      fetch(`${API}/api/nephropathy-atlas/definitions`).then(r => r.json()),
    ])
      .then(([ov, bd, df]) => {
        setOverview(ov);
        setBreakdown(bd);
  ...
  return (
    <div className="container-fluid py-4">
      <div className="mb-3">
        <h2 className="mb-0">
          <span style={{ color: '#1565c0' }}>🫘</span> Nephropathy-Atlas
        </h2>
        <p className="text-muted small mb-0">
          Complete 8-Gene Hereditary Nephropathy Atlas —
