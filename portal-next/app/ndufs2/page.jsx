...
const COLOR = '#01579b';   // dark blue — Q-module / N2 / ubiquinone-binding theme
const LIGHT = '#e3f2fd';
...
function OverviewTab({ data }) {
  ...
  return (
    <div>
      ...
      <SectionCard title="NDUFS2 = N2 Fe-S — Terminal Q-Module Electron Carrier to Ubiquinone" borderColor="#01579b">
        <Alert variant="info" text="NDUFS2 (PSST/49 kDa) harbours the N2 [4Fe-4S] cluster — the TERMINAL and ONLY Fe-S cluster in the Q-module, directly transferring electrons to ubiquinone. All upstream Fe-S clusters (N1b, N4, N5 at NDUFS1; N6a/N6b) funnel electrons into N2. Loss of NDUFS2 blocks the FINAL step → complete isolated CI deficiency. Biochemical fingerprint: CI 5–20%, CII/CIII/CIV NORMAL." />
      </SectionCard>

      <SectionCard title="NO Peripheral Neuropathy — KEY DDx vs NDUFS1 (~50% neuropathy)" borderColor="#2e7d32">
        <Alert variant="success" text="NDUFS2-CI-Leigh does NOT show peripheral neuropathy. This distinguishes it from NDUFS1 (50% neuropathy — the only common CI-Leigh gene with significant neuropathy). Absence of neuropathy + isolated CI deficiency helps narrow the differential to NDUFS2, NDUFS4, or NDUFV1, with olfactory bulb MRI and white matter changes further differentiating those." />
      </SectionCard>

      <SectionCard title={`KPIs — 40-patient NDUFS2 cohort (seed-613)`}>
      ...
      <SectionCard title="Feature Frequencies (40-patient cohort, seed-613)">
...
function PatientsTab({ data }) {
  ...
  return (
    <div>
      ...
      <SectionCard title="Patient Cohort (40 patients, seed-613)">
...
function TreatmentsTab({ overview, breakdown }) {
  ...
  return (
    <div>
      <SectionCard title="CI-Leigh Differential Diagnosis (NDUFS2 vs Series)" borderColor="#6a1b9a">
        <div className="table-responsive">
          <table className="table table-sm small">
            <thead>
              <tr>
                <th>Gene</th><th>Module</th><th>Distinguishing Feature</th><th>MRI Extra</th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                ['NDUFS2',  'Q-module (N2/PSST/49kDa)', 'NO neuropathy, NO olfactory, NO leukodystrophy',  'None specific'],
              ].map(([g, m, d, mr]) => (
                <tr key={g} style={{ background: g === 'NDUFS2' ? LIGHT : '' }}>
                  <td className="fw-bold" style={{ color: g === 'NDUFS2' ? COLOR : '#333' }}>{g}</td>
...
// ── Main page ────────────────────────────────────────────────────────────────
export default function NDUFS2Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/ndufs2/overview`).then(r => r.json()),
      fetch(`${API}/api/ndufs2/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ndufs2/definitions`).then(r => r.json()),
    ]).then(([ov, bk, df]) => {
    ...
    <div className="container-fluid py-3">
      <div className="mb-3">
        ...
        <div className="text-muted small">
          CI-Leigh / Q-Module 49 kDa PSST Subunit / N2 [4Fe-4S] Terminal Fe-S Cluster to Ubiquinone · 1q23.3 · OMIM *602985 · AR Biallelic · 40-patient cohort seed-613
        </div>
      </div>
