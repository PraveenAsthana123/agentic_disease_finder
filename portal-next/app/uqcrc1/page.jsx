...
const COLOR  = '#1a237e';   // deep indigo — UQCRC1 / core scaffold protein 1 (largest)
const LIGHT  = '#e8eaf6';
const COLOR2 = '#0277bd';
    ...
    <div>
      ...
      <div className="row">
        ...
        <div className="col-md-6">
          ...
          <SectionCard title="🔑 UQCRC1 — Critical DDx vs UQCRC2 / CYC1 / LYRM7 / BCS1L" borderColor={COLOR4}>
            <ul className="small mb-0 ps-3">
              <li><strong>vs UQCRC2:</strong> BN-PAGE/immunoblot <span style={{ color: COLOR3 }}>IDENTICAL</span> — WES mandatory (UQCRC1 = 3p21.31 vs UQCRC2 = 16p12.2); no reliable bedside distinguisher</li>
              <li><strong>vs CYC1:</strong> Both absent CIII; <span style={{ color: COLOR }}>No cataracts in UQCRC1</span> (cataracts ~35% in CYC1) — KEY bedside clue</li>
              <li><strong>UQCRC2 (Core2):</strong> <span style={{ color: COLOR3 }}>ABSENT</span> in UQCRC1 deficiency · <span style={{ color: COLOR }}>PRESERVED</span> in LYRM7/BCS1L — definitive distinguisher</li>
              <li><strong>BN-PAGE CIII:</strong> Absent (UQCRC1) · Reduced 15-35% (LYRM7) · Precomplex (BCS1L)</li>
              <li><strong>Mechanism:</strong> UQCRC1 = core scaffold loss (entire holocomplex absent) vs LYRM7 = chaperone (core preserved)</li>
            </ul>
          </SectionCard>
        </div>
      </div>
...
function VariantsTab({ data }) {
  ...
  return (
    <div>
      <SectionCard title="Pathogenic Variants in UQCRC1">
        <div className="table-responsive">
      ...
      <div className="row">
        <div className="col-md-6">
          <SectionCard title="Biochemistry Distribution">
            ...
            <Bar label="CIII activity 5-10%" value={bio.ciii_5_to10_pct || 0} color="#e53935" />
            ...
            <Bar label="Lactate 8-15 mM" value={bio.lactic_8_to_15_pct || 0} color="#e53935" />
...
function DDxTab({ data }) {
  ...
  const tx   = data.safe_treatments || [];
  const ddx  = data.ddx_pearls || [];

  return (
    <div>
      <SectionCard title="Differential Diagnosis Pearls" borderColor={COLOR4}>
        <ul className="small ps-3 mb-0">
          {ddx.map((d, i) => (
            <li key={i} className="mb-2 pb-1 border-bottom">{d}</li>
          ))}
        </ul>
      </SectionCard>
      ...
      <div className="row">
        ...
        <div className="col-md-6">
          <SectionCard title="✅ Safe Treatments & Recommendations" borderColor={COLOR}>
            {tx.map((t, i) => (
      ...
      <SectionCard title="CIII Core Scaffold — UQCRC1 Position" borderColor={COLOR2}>
        <div className="row text-center small">
          {[
            { step: '1', label: 'UQCC1-UQCC2 scaffold', detail: 'Bind nascent MT-CYB (20q11.22 / 6p21.2)', color: '#1565c0' },
            { step: '2', label: 'UQCC3 cooperates', detail: '1-TM-Helix IMM anchor (11q12.3)', color: '#2e7d32' },
            { step: '3', label: 'UQCRC1 + UQCRC2', detail: 'Core scaffold heterodimer (3p21.31 / 16p12.2)', color: COLOR },
        ...
        <div className="alert mt-3 mb-0 small" style={{ background: LIGHT, borderLeft: `4px solid ${COLOR3}` }}>
          <strong>UQCRC1 block effect:</strong> UQCRC1 (LARGEST core subunit) absent → UQCRC2 (Core2) secondarily absent →
          ENTIRE CIII holocomplex destabilised → ALL subunits (UQCRC1, UQCRC2, RISP, CYC1, MT-CYB reduced) absent on immunoblot →
          BN-PAGE: CIII2 and supercomplexes completely absent.
          Biochemically indistinguishable from UQCRC2 and CYC1 deficiency — WES mandatory.
          No cataracts in UQCRC1 (cataracts present in CYC1 ~35%) — KEY bedside DDx vs CYC1.
          No bedside DDx vs UQCRC2 — gene-level WES (3p21.31 vs 16p12.2) required.
        </div>
      </SectionCard>
    </div>
  );
          ...
          <SectionCard title="Protein Details">
            <table className="table table-sm mb-0">
              <tbody>
                ...
                <tr><td><strong>TM helices</strong></td><td>{p.tm_helices} (soluble peripheral IMM protein)</td></tr>
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function UQCRC1Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/uqcrc1/overview`).then(r => r.json()),
      fetch(`${API}/api/uqcrc1/breakdown`).then(r => r.json()),
      fetch(`${API}/api/uqcrc1/definitions`).then(r => r.json()),
    ...
    <div className="container-fluid py-4">
      <div className="mb-3" style={{ borderBottom: `3px solid ${COLOR}` }}>
        <h4 className="fw-bold" style={{ color: COLOR }}>
          🧬 UQCRC1 — Complex III Deficiency, Nuclear Type 5 (CIII-D5)
        </h4>
        <p className="text-muted mb-2 small">
          OMIM Gene *191328 · Disease #615160 · AR biallelic · 3p21.31 · 40-patient cohort (seed 729)
        </p>
