...
const TABS = ['Overview', 'Patients & Deletions', 'Treatments & DDx', 'Definitions'];
const COLOR = '#1565c0';   // deep blue — KSS / mtDNA deletion / cardiac-ophthalmic
const LIGHT = '#e3f2fd';
    ...
    <div>
      <SectionCard title="Gene & Disease Identity">
        <div className="row g-2 small">
          {[
            ['Gene/Cause', data.gene],
            ['Protein(s) Lost', data.protein],
      ...
      <SectionCard title="Mechanism — Why Large Deletion Causes Multi-Complex OXPHOS Failure">
        <p className="small text-muted mb-0">{data.mechanism}</p>
      </SectionCard>

      <SectionCard title="mtDNA Deletion Pattern & Tissue Heteroplasmy">
...
// ── Tab 2: Patients & Deletions ───────────────────────────────────────────────
function PatientsTab({ data }) {
  if (!data) return <Spinner />;
  return (
    <div>
      <SectionCard title="Patient Cohort (40 patients · seed-589 · KSS single large mtDNA deletion)">
        <div className="table-responsive">
          <table className="table table-sm table-hover small">
            <thead className="table-light">
              <tr>
                <th>ID</th><th>Deletion</th><th>Δ-het%</th><th>CHB</th>
                <th>Pacemaker</th><th>Sex</th><th>Age Dx</th><th>Features</th><th>Treatments</th><th>Alerts</th>
              </tr>
            </thead>
            <tbody>
              {(data.patients || []).map(p => (
                <tr key={p.id} style={p.alerts !== 'None' ? { background: '#fff3e0' } : {}}>
                  ...
                  <td className="small" style={{ maxWidth: 180, wordBreak: 'break-word' }}>{p.deletion}</td>
                  <td>
                    <span style={{
                      color: p.het >= 80 ? '#b71c1c' : p.het >= 70 ? '#e65100' : '#558b2f',
                      fontWeight: 600,
                    }}>{p.het}%</span>
                  </td>
                  <td>
                    <span style={{ color: p.chb ? '#c62828' : '#555', fontWeight: p.chb ? 700 : 400 }}>
                      {p.chb ? '🚨 YES' : 'No'}
                    </span>
                  </td>
                  <td>
                    <span style={{ color: p.pacemaker ? '#1b5e20' : p.chb ? '#c62828' : '#555', fontWeight: 600 }}>
                      {p.pacemaker ? '✓ PM' : p.chb ? '⚠ PENDING' : '—'}
                    </span>
                  </td>
                  <td>{p.sex}</td>
                  <td>{p.age_dx}y</td>
      ...
      <SectionCard title="Clinical Feature Frequencies (Cohort)">
        {Object.entries(data.feature_frequencies || {}).map(([feat, pct]) => (
          <Bar
            ...
            color={
              pct === 100 ? COLOR :
              feat.toLowerCase().includes('heart') || feat.toLowerCase().includes('chb') ? '#c62828' :
              feat.toLowerCase().includes('pacemaker') ? '#1b5e20' :
              feat.toLowerCase().includes('diabet') ? '#e65100' :
              COLOR
            }
          />
        ))}
...
export default function KSSPage() {
  ...
  useEffect(() => {
    fetch(`${API}/api/kss/overview`)
      .then(r => r.json()).then(setOverview).catch(e => setError(e.message));
  }, []);

  useEffect(() => {
    if (tab === 1) fetch(`${API}/api/kss/breakdown`).then(r => r.json()).then(setBreakdown).catch(() => {});
    if (tab === 2) fetch(`${API}/api/kss/breakdown`).then(r => r.json()).then(setBreakdown).catch(() => {});
    if (tab === 3) fetch(`${API}/api/kss/definitions`).then(r => r.json()).then(setDefinitions).catch(() => {});
  }, [tab]);
      ...
      <div className="d-flex align-items-center mb-3 gap-3">
        ...
        <div>
          ...
          <div className="text-muted small">
            mtDNA single large deletion · 4977 bp common deletion (m.8483_13459del) · OMIM #530000 ·
            Sporadic de novo · PEO + Pigmentary RP + Onset &lt;20y TRIAD · Complete Heart Block 57% (Pacemaker MANDATORY) ·
            VPA / Aminoglycosides / Metformin / Linezolid ABSOLUTE CI
          </div>
        </div>
      </div>
