...
const SLUG = 'hereditary-medulloblastoma-predisposition-atlas';
...
const GENE_INFO = {
  PTCH1:  { full: 'Patched-1 Receptor',                   locus: '9q22.32',  size: '1447aa', inh: 'AD LOF' },
  SUFU:   { full: 'Suppressor of Fused Homologue',         locus: '10q24.32', size: '484aa',  inh: 'AD LOF' },
  ...
  CREBBP: { full: 'CREB-Binding Protein (CBP/KAT3A)',      locus: '16p13.3',  size: '2441aa', inh: 'AD LOF' },
  EP300:  { full: 'E1A-Binding Protein p300 (KAT3B)',      locus: '22q13.2',  size: '2414aa', inh: 'AD LOF' },
  BRCA2:  { full: 'Breast Cancer Type 2 Susceptibility',   locus: '13q12.3',  size: '3418aa', inh: 'AD LOF / AR FA-D1' },
  PALB2:  { full: 'Partner and Localiser of BRCA2',        locus: '16p12.2',  size: '1186aa', inh: 'AD LOF / AR FA-N' },
};
...
export default function HereditaryMedulloblastomaPredispositionAtlasPage() {
  ...
  if (loading) return (
    <div style={{ padding: 48, textAlign: 'center', color: '#6b7280' }}>
      Loading Hereditary Medulloblastoma Predisposition Atlas…
    </div>
  );
  if (error) return (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
          ...
          <div>
            ...
            <div style={{ color: '#6b7280', fontSize: 14, marginTop: 4 }}>
              Complete 8-Gene Reference · PTCH1 · SUFU · TP53 · APC · CREBBP · EP300 · BRCA2 · PALB2 · Seeds 3302–3309
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <Badge label="SHH Subgroup" color="#10b981" />
          <Badge label="WNT Subgroup" color="#3b82f6" />
          <Badge label="Fanconi Anaemia" color="#ef4444" />
          <Badge label="AVOID RADIATION: PTCH1 SUFU TP53" color="#f59e0b" />
          <Badge label="AVOID ALKYLATING: BRCA2 PALB2 FA" color="#8b5cf6" />
      ...
      {tab === 'Overview' && overview && (
        <div>
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 20, marginBottom: 24 }}>
            ...
            {overview.gene_summaries?.map(g => (
              <GeneBar key={g.gene} gene={g.gene} value={`${g.mean_age_dx}yr`} max={15} />
            ))}
          </div>
          ...
          <div style={{ background: '#fffbeb', border: '1px solid #fbbf24', borderRadius: 12, padding: 16 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 20, color: '#78350f', fontSize: 13, lineHeight: 1.7 }}>
              <li><strong>PTCH1 Gorlin:</strong> AVOID RADIATION ABSOLUTELY — each Gray induces ~10 new BCCs. SHH-MB 25–30% germline in SHH subgroup.</li>
              <li><strong>SUFU:</strong> HIGHEST lifetime MB risk 50–60%. Vismodegib RESISTANT (distal to SMO). AVOID radiation + vismodegib in children.</li>
              <li><strong>TP53 LFS:</strong> SHH-MB anaplastic + chromothripsis PATHOGNOMONIC LFS. AVOID RADIATION ABSOLUTELY. WBMRI Toronto annually.</li>
              <li><strong>APC Turcot Type 2:</strong> WNT-MB monosomy 6 — BEST prognosis (&gt;90% 5yr survival). Nuclear β-catenin IHC PATHOGNOMONIC.</li>
              <li><strong>CREBBP/EP300 RTS:</strong> WNT-MB excellent prognosis. Broad thumbs PATHOGNOMONIC. Sequence BOTH genes — cannot distinguish clinically.</li>
              <li><strong>BRCA2 FA-D1:</strong> Desmoplastic MB PATHOGNOMONIC. DEB/MMC FIRST. AVOID alkylating agents. SIBLING DONOR EXCLUSION MANDATORY.</li>
              <li><strong>PALB2 FA-N:</strong> Brain tumours in biallelic FA-N. DEB/MMC PATHOGNOMONIC. AVOID alkylating. Monoallelic: breast 53%, olaparib FDA.</li>
            </ul>
          </div>
        </div>
      )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#f9fafb' }}>
                {['Gene', 'Full Name', 'Locus', 'Size', 'Inheritance', 'Syndrome', 'MB Subgroup', 'Key Risk'].map(h => (
            ...
            <tbody>
              {[
                { gene: 'PTCH1',  syndrome: 'Gorlin / NBCCS',         subgroup: 'SHH',    risk: 'MB 5% lifetime · BCC 90% · jaw keratocysts' },
                { gene: 'SUFU',   syndrome: 'Gorlin-like SHH distal',  subgroup: 'SHH',    risk: 'MB HIGHEST 50–60% lifetime' },
                { gene: 'TP53',   syndrome: 'Li-Fraumeni Syndrome',    subgroup: 'SHH',    risk: 'MB anaplastic + sarcoma + CPC + ACC' },
                { gene: 'APC',    syndrome: 'FAP / Turcot Type 2',     subgroup: 'WNT',    risk: 'WNT-MB 0.5–1% · CRC 100% untreated' },
                { gene: 'CREBBP', syndrome: 'Rubinstein-Taybi RTS1',   subgroup: 'WNT',    risk: 'WNT-MB · ID 100% · lymphoma' },
                { gene: 'EP300',  syndrome: 'Rubinstein-Taybi RTS2',   subgroup: 'WNT',    risk: 'WNT-MB · ID milder vs RTS1' },
                { gene: 'BRCA2',  syndrome: 'HBOC / Fanconi FA-D1',   subgroup: 'SHH FA', risk: 'Desmoplastic MB + HBL + AML + Wilms PATHOGNOMONIC' },
                { gene: 'PALB2',  syndrome: 'HBOC2 / Fanconi FA-N',   subgroup: 'SHH FA', risk: 'Brain tumours + breast 53% · olaparib' },
              ].map((row, i) => (
                <tr key={row.gene} style={{ background: i % 2 === 0 ? '#fff' : '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                  ...
                  <td style={{ padding: '10px 12px' }}>
                    <Badge label={row.subgroup} color={row.subgroup === 'WNT' ? '#3b82f6' : row.subgroup === 'SHH FA' ? '#ef4444' : '#10b981'} />
                  </td>
          ...
          {breakdown.breakdown?.map(g => (
            <div key={g.gene} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  <Badge label={`CR ${g.cr_pct}%`} color={g.cr_pct >= 85 ? '#10b981' : g.cr_pct >= 70 ? '#f59e0b' : '#ef4444'} />
