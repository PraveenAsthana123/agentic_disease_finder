...
// Deep amber — CIV subunit, NOT CI despite NDUFA name; visual distinction from CI-Leigh green/indigo
const COLOR = '#bf360c';
const LIGHT = '#fbe9e7';
...
function OverviewTab({ data }) {
  ...
  return (
    <div>
      <Alert variant="danger" text="⚠️ CRITICAL NAMING TRAP: NDUFA4 (MLRQ) is a Complex IV (CIV) subunit — NOT Complex I — despite the 'NDUFA' family name. Deficiency → isolated CIV deficiency (5–25% residual CIV), CI/CII/CIII NORMAL. WES 'CI panel' flag is a DDx pitfall — biochemical complex fingerprint is mandatory." />

      <SectionCard title="Gene & Disease Identity">
        <div className="row g-2 small">
          {[
            ['Gene', data.gene],
            ['Also known as', 'MLRQ / ML-INSERT'],
            ['Module / Complex', 'Complex IV (CIV / COX) — NOT Complex I'],
            ['Size', data.protein_size],
            ['Chromosome', data.chromosome],
            ['OMIM Gene', `*${data.omim_gene}`],
            ['OMIM Disease', `#${data.omim_disease} (Leigh)`],
            ['Inheritance', data.inheritance],
            ['Disease', data.disease],
          ].map(([l, v]) => (
            <div className="col-md-6" key={l}>
              <span className="fw-semibold">{l}:</span>{' '}
      ...
      <h6 className="fw-bold mb-3" style={{ color: COLOR }}>
        Cohort KPIs — {data.cohort_n} patients (seed {data.seed})
      </h6>
      <div className="row mb-4">
        ...
        <KPI label="Mean CIV %"     value={`${k.mean_civ_pct}%`}        color="#006064" />
      </div>

      <Alert variant="warning" text="🧬 NDUFA4 (7p21.3) vs NDUFA5 (7q32.1): both on chromosome 7 — arm (p vs q) is the mandatory WES differentiation pivot. NDUFA4 (7p) → CIV deficiency; NDUFA5 (7q) → CI deficiency. Biochemical complex fingerprint resolves before WES report." />
        ...
        <div className="row">
          {(data.triggers || []).map(t => (
            <div className="col-md-6" key={t.trigger}>
              <Bar label={t.trigger} value={t.pct} color="#b71c1c" />
            </div>
          ))}
        </div>
      </SectionCard>
      ...
      <SectionCard title="Key Molecular Concepts">
        {(data.key_concepts || []).map(c => (
          <Alert
            key={c.concept}
            variant={c.concept.includes('CIV') || c.concept.includes('trap') || c.concept.includes('NDUFA4') ? 'danger' : 'info'}
            text={<><strong>{c.concept}:</strong> {c.detail}</>}
          />
        ))}
      </SectionCard>
...
function PatientsTab({ data }) {
  ...
  return (
    <div>
        ...
        {(data.variants || []).map(v => (
          <div key={v.variant} className="mb-3 p-3 rounded" style={{ background: LIGHT }}>
            ...
            <div className="small text-muted mt-1">
              <strong>Structural:</strong> {v.structural_impact}
            </div>
      ...
      <SectionCard title="Patient Cohort (40 patients, seed 656)">
        <div className="table-responsive">
          <table className="table table-sm table-hover small">
            <thead>
              <tr style={{ background: COLOR, color: '#fff' }}>
                <th>ID</th><th>Phenotype</th><th>Onset (mo)</th>
                <th>Variant</th><th>CIV%</th>
...
function TreatmentsTab({ data }) {
  ...
  return (
    <div>
      <Alert variant="danger" text="🚨 PROPOFOL ABSOLUTE CONTRAINDICATION: Propofol directly inhibits CIV (haem a3-CuB site) in addition to PRIS. In NDUFA4-CIV-Leigh, propofol causes catastrophic dual CIV inhibition. Use SEVOFLURANE for all anaesthesia." />
      ...
      <SectionCard title="Treatments (Evidence Level)">
        {(data.treatments || []).map(t => (
          <div key={t.name} className="mb-2 p-2 rounded" style={{ background: LIGHT }}>
            ...
            <div className="small text-muted mt-1">{t.notes}</div>
          </div>
        ))}
      </SectionCard>
...
// ── Root component ────────────────────────────────────────────────────────────
export default function NDUFA4Page() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/ndufa4/overview`).then(r => r.json()),
      fetch(`${API}/api/ndufa4/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ndufa4/definitions`).then(r => r.json()),
    ])
    ...
    <div className="container-fluid py-3">
      <div className="mb-3 p-3 rounded" style={{ background: COLOR, color: '#fff' }}>
        <h4 className="mb-1 fw-bold">
          🧬 NDUFA4 — Leigh Syndrome / Isolated Complex IV Deficiency (COXPD20)
        </h4>
        <div className="small opacity-75">
          MLRQ / ML-INSERT · CIV structural subunit (NOT CI despite NDUFA name) ·
          COX2-CuA IMS contact · Single TM helix · AR · 7p21.3 ·
          OMIM Gene *602137 · Disease #256000 · Propofol ABSOLUTE CI
        </div>
      </div>

      {loading && <div className="alert alert-info">Loading NDUFA4 dashboard…</div>}
