...
export default function RasmussenDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/rasmussen/overview`).then(r => r.json()),
      fetch(`${API}/api/rasmussen/breakdown`).then(r => r.json()),
      fetch(`${API}/api/rasmussen/definitions`).then(r => r.json()),
    ]).then(([ov, br, df]) => {
      setOverview(ov);
      setBreakdown(br);
      setDefinitions(df);
  ...
  if (loading) return <div className="container py-5 text-center text-muted">Loading Rasmussen&#39;s Encephalitis Dashboard…</div>;
  ...
  const CTRL_BADGE = {
    'Drug-resistant (EPC active)': 'danger',
    'Partial control': 'warning',
    'Seizure-free (post-surgical)': 'success',
  };
  const PHASE_BADGE = {
    'Prodrome': 'secondary',
    'Rapid Deterioration': 'danger',
    'Plateau (Hemiplegia)': 'warning',
    'Post-surgical Recovery': 'info',
  ...
  return (
    <div className="container-fluid py-3" style={{ maxWidth: 1200 }}>
      <div className="mb-3">
        <h2 className="fw-bold mb-0" style={{ color: '#7c3aed' }}>🧠 Rasmussen&#39;s Encephalitis</h2>
        <div className="text-muted small">
          Progressive unilateral autoimmune encephalitis · EPC · T-cell mediated ·
          Hemispherectomy definitive (Engel I 52–65%) ·{' '}
      ...
      {tab === 0 && (
        <>
          <div className="row g-2 mb-4">
            ...
            <KPI label="EPC Active" value={`${overview.epc_active_n} (${overview.epc_active_pct}%)`} color="#dc2626" />
            <KPI label="Post-Surgical" value={`${overview.post_surgical_n} (${overview.post_surgical_pct}%)`} color="#16a34a" />
              ...
              <table className="table table-sm table-hover table-bordered">
                <thead className="table-light sticky-top">
                  <tr>
                    ...
                    <Th k="EPC" label="EPC" />
                    <Th k="hemiplegia_side" label="Hemiplegia" />
                  </tr>
                </thead>
                <tbody>
                  {sorted.map(p => (
                    <tr key={p.id}>
                      ...
                      <td className="text-center">{p.EPC ? '⚡' : '—'}</td>
                      <td className="text-center">
                        <span className={`badge text-bg-${p.hemiplegia_side === 'Left' ? 'primary' : 'info'}`}>
                          {p.hemiplegia_side}
                        </span>
                      </td>
                    </tr>
      ...
      {tab === 2 && (
        <>
          <SectionCard title="⚡ Seizure Types in Rasmussen's Encephalitis" borderColor="#7c3aed">
            {(breakdown.seizure_types || []).map((st, i) => (
              <div key={i} className="mb-4 border-bottom pb-3">
                <div className="d-flex align-items-center mb-2 flex-wrap gap-2">
                  ...
                  <span className="badge" style={{ backgroundColor: '#7c3aed' }}>{st.freq_pct}% of RE</span>
      ...
      {tab === 3 && (
        <>
          <div className="alert alert-danger mb-3">
            <strong>⛔ Key Safety Rules in Rasmussen&#39;s Encephalitis:</strong>
            <ul className="mb-0 mt-1">
              <li><strong>AEDs alone</strong> — do NOT halt disease progression; immunotherapy + surgical referral mandatory in all cases</li>
              <li><strong>EPC</strong> — AED-resistant by definition; EPC &gt;2 hours = emergent IV methylprednisolone</li>
              <li><strong>Natalizumab</strong> — JC antibody index BEFORE initiation + every 6 months (PML risk)</li>
              <li><strong>Rituximab</strong> — hepatitis B screen (HBsAg + anti-HBc) MANDATORY before first dose</li>
            </ul>
          </div>

          <div className="alert alert-success mb-3">
            <strong>✅ Definitive Treatment: Hemispherectomy / Hemispherotomy</strong> — Engel I seizure-free 52–65%.
            EPC resolves in &gt;90% of post-surgical cases. Language fMRI + Wada test mandatory for dominant-hemisphere RE.
            Surgical timing: disease plateau (established hemiplegia + 3 stable serial MRIs).
          </div>
                ...
                <h2 className="accordion-header">
                  <button className="accordion-button collapsed fw-bold" type="button"
                    ...
                    {tx.contraindication_note && (
                      <span className="badge bg-warning text-dark ms-2" style={{ fontSize: 10, maxWidth: 240, whiteSpace: 'normal', textAlign: 'left' }}>
          ...
          <SectionCard title="🩺 Monitoring Protocol" borderColor="#7c3aed">
          ...
          <SectionCard title="📅 Rasmussen&#39;s Encephalitis Lifecycle Management" borderColor="#2563eb">
      ...
      {tab === 4 && (
        <>
          <SectionCard title="⛔ Contraindications in Rasmussen&#39;s Encephalitis" borderColor="#dc2626">
            <div className="table-responsive">
              <table className="table table-sm table-bordered">
                <thead className="table-light">
          ...
          <SectionCard title="📖 Glossary — Rasmussen&#39;s Encephalitis Concepts" borderColor="#7c3aed">
