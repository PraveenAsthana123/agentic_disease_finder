...
const GENE_COLORS = {
  BRCA1:  '#7b1fa2',  // deep purple — high-risk, triple-negative, RING/BRCT
  BRCA2:  '#1565c0',  // deep blue — high-risk, ER+, pancreatic/prostate
  PALB2:  '#00695c',  // dark teal — high-moderate, BRCA1-BRCA2 bridge
  CHEK2:  '#e65100',  // deep orange — moderate, ER+, NO RRSO
  ATM:    '#4e342e',  // dark brown — moderate, radiation sensitivity
  RAD51C: '#c62828',  // deep red — ovarian dominant, FANCO
  RAD51D: '#880e4f',  // deep pink — ovarian dominant, FANCS
  BRIP1:  '#37474f',  // dark slate — ovarian moderate, G4 helicase, FANCJ
};

const GENE_DISEASE = {
  BRCA1:  'HBOC-1 AD — 70% Breast 46% Ovarian — Triple-Negative Dominant — Olaparib/Talazoparib — BSO Age 35–40',
  BRCA2:  'HBOC-2 AD — 69% Breast 25% Ovarian — ER+ Dominant — Male Breast 5–7% — Pancreatic/Prostate — Olaparib/Niraparib',
  PALB2:  'HBOC-PALB2 AD — 35–58% Breast 5% Ovarian — BRCA1-BRCA2 Bridge — PARPi Sensitive — TBCRC048',
  CHEK2:  'Moderate-Risk HBOC AD — 25–35% Breast — c.1100delC European — NO RRSO — NO PARPi — ER+ Standard',
  ATM:    'Moderate-Risk HBOC AD-Heterozygous — 25–33% Breast — Radiation Sensitivity — Biallelic = A-T',
  RAD51C: 'Ovarian-Dominant HBOC AD — 10–14% Ovarian 3% Breast — FANCO — HGSC — PARPi/Platinum Sensitive',
  RAD51D: 'Ovarian-Dominant HBOC AD — 10–13% Ovarian 3% Breast — FANCS — HGSC — PARPi/Platinum Sensitive',
  BRIP1:  'Ovarian-Moderate HBOC AD — 5–10% Ovarian 1–2% Breast — FANCJ G4 Helicase — BRCA1-BRCT Binding',
};
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: '2rem' }}>
        ...
        <KPI label="BRCA1 triple-neg %" value={`${s.brca1_triple_negative_pct}%`} color="#7b1fa2" />
        <KPI label="BRCA1 BSO done %" value={`${s.brca1_bso_completed_pct}%`} color="#7b1fa2" />
        <KPI label="BRCA1 PARPi %" value={`${s.brca1_parp_inhibitor_pct}%`} color="#7b1fa2" />
        <KPI label="BRCA2 ER+ %" value={`${s.brca2_er_positive_pct}%`} color="#1565c0" />
        <KPI label="BRCA2 male breast %" value={`${s.brca2_male_breast_pct}%`} color="#1565c0" />
        <KPI label="CHEK2 RRSO incorrect %" value={`${s.chek2_rrso_incorrectly_offered_pct}%`} color="#e65100" />
        <KPI label="RAD51C HGSC %" value={`${s.rad51c_ovarian_hgsc_pct}%`} color="#c62828" />
        <KPI label="RAD51C platinum resp %" value={`${s.rad51c_platinum_response_pct}%`} color="#c62828" />
...
function GeneTableTab({ data }) {
  ...
  return (
    <div>
      <h3 style={{ color: '#e2e8f0', marginBottom: '1rem' }}>HBOC Gene Reference Table</h3>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr style={{ background: '#1e293b' }}>
              {['Gene', 'Locus', 'aa', 'kDa', 'OMIM Gene', 'OMIM Disease', 'Inheritance', 'N Patients', 'Key Class'].map(h => (
          ...
          <tbody>
            {genes.map(g => (
              <tr key={g.gene} style={{ borderBottom: '1px solid #1e293b' }}>
                ...
                <td style={{ padding: '0.5rem 0.8rem', color: '#e2e8f0' }}>{g.n_patients}</td>
                <td style={{ padding: '0.5rem 0.8rem', color: '#64748b', maxWidth: 180, fontSize: 10 }}>{g.gene_class.slice(0, 50)}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function DefinitionsTab({ data }) {
  ...
  return (
    <div>
      <h3 style={{ color: '#e2e8f0', marginBottom: '1rem' }}>HBOC Clinical Concepts</h3>
...
export default function HBOCAtlasPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/hereditary-hboc-atlas/overview`).then(r => r.json()),
      fetch(`${API}/api/hereditary-hboc-atlas/breakdown`).then(r => r.json()),
      fetch(`${API}/api/hereditary-hboc-atlas/definitions`).then(r => r.json()),
    ]).then(([ov, bk, df]) => {
      setOverview(ov);
      setBreakdown(bk);
      setDefinitions(df);
    ...
    <div style={{ padding: '1.5rem 2rem', background: '#0f172a', minHeight: '100vh', fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ color: '#e2e8f0', fontSize: 22, fontWeight: 800, marginBottom: 4 }}>
          🧬 Hereditary HBOC Atlas
        </h1>
        <p style={{ color: '#64748b', fontSize: 12 }}>
          Complete 8-Gene Hereditary Breast &amp; Ovarian Cancer Reference ·
          BRCA1 · BRCA2 · PALB2 · CHEK2 · ATM · RAD51C · RAD51D · BRIP1 ·
          320 patients (8×40, seeds 1630–1637)
        </p>
      </div>
