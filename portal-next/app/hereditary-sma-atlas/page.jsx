...
const GENE_COLORS = {
  SMN1:    '#1565c0',  // deep blue — 3 FDA treatments, NBS mandatory
  AR:      '#880e4f',  // dark pink — Kennedy SBMA CAG repeat X-linked
  BICD2:   '#4a148c',  // deep purple — SMALED2 DDH lower extremity
  DYNC1H1: '#e65100',  // deep orange — SMALED1 CNS malformation subgroup
  VAPB:    '#1b5e20',  // dark green — ALS8 P56S Brazil founder
  SETX:    '#006064',  // dark teal — ALS4 juvenile NO bulbar decades survival
  SIGMAR1: '#b71c1c',  // deep red — ALS16 distal MAM junction
  PLEKHG5: '#37474f',  // dark slate — DSMA4 NF-kB autophagy pure LMN
};

const GENE_DISEASE = {
  SMN1:    'AR SMA 5q — Survival Motor Neuron 1 — 95% Homozygous Deletion — SMN2 Copy Number Modifier — Nusinersen FDA2016 + Onasemnogene FDA2019 + Risdiplam FDA2020 — NBS Mandatory 40+ Countries',
  AR:      'X-linked Recessive Kennedy SBMA — Androgen Receptor CAG Repeat >36 — PolyQ Toxic GOF — Gynaecomastia 70% + Sensory Neuropathy + Bulbar — Leuprolide FUTILE Trial Negative — Males Predominantly',
  BICD2:   'AD SMALED2 — Bicaudal-D2 Cargo Adaptor — Lower Extremity Predominant Congenital — DDH Screen Mandatory — Intelligence NORMAL — De Novo Common 60% — Slowly Progressive',
  DYNC1H1: 'AD SMALED1 — Dynein Heavy Chain — Lower Extremity + CNS Malformation Subgroup 25% — Brain MRI Mandatory — Pachygyria/Lissencephaly — Foot Deformities 75% — Cognition Variable',
  VAPB:    'AD ALS8/SMA-Proximal — VAMP-Associated Protein B — P56S Brazil Founder 85% — Late Onset 40-60y — Fasciculations PROMINENT — Slowly Progressive Decades — ER-Mitochondria MAM Tethering',
  SETX:    'AD ALS4 Juvenile — Senataxin RNA-DNA Helicase — NO Bulbar Involvement KEY Distinguisher — Onset 10-25y — Slowly Progressive Decades Survival — AOA2 Biallelic AR — AFP Elevated AOA2',
  SIGMAR1: 'AR ALS16/Distal SMA — Sigma-1 Receptor MAM Chaperone — Juvenile Onset Distal Weakness — Sensory Neuropathy 70% — Vocal Cord Paresis 20% — ER-Mitochondria Calcium — No Treatment',
  PLEKHG5: 'AR DSMA4 — RhoGEF NF-kB Autophagy — Pure LMN Only No Sensory — Distal SMA Type 4 — Vocal Cord Paresis 30% — Autophagy Axon Terminal — Slowly Progressive — NF-kB Pathway',
};
...
function OverviewTab({ data }) {
  ...
  return (
    <div>
      ...
      {top_alerts.map((a, i) => <Alert key={i} text={a} color={['#1565c0','#880e4f','#4a148c','#e65100','#1b5e20','#006064','#b71c1c','#37474f'][i % 8]} />)}
...
function GeneTableTab({ data }) {
  ...
  return (
    <div>
      <h3 style={{ color: '#e2e8f0', marginBottom: 12 }}>Gene Reference Table — All 8 SMA/MND Genes</h3>
          ...
          <div>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                <thead>
                  <tr style={{ background: '#0f172a' }}>
                    {['ID','Onset Age','Dx Delay(mo)','Bulbar Onset','FTD'].map(h => (
                ...
                <tbody>
                  {gene.sample_patients.map((p, i) => (
                    <tr key={p.patient_id} style={{ background: i % 2 === 0 ? '#1e293b' : '#162032' }}>
                      ...
                      <td style={{ padding: '6px 8px', color: p.ftd ? '#ef4444' : '#64748b' }}>
                        {p.ftd ? 'Yes' : 'No'}
                      </td>
                    </tr>
                  ))}
...
export default function SMAAtlasPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/hereditary-sma-atlas/overview`).then(r => r.json()),
      fetch(`${API}/api/hereditary-sma-atlas/breakdown`).then(r => r.json()),
      fetch(`${API}/api/hereditary-sma-atlas/definitions`).then(r => r.json()),
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ color: '#f1f5f9', fontSize: 22, fontWeight: 700, marginBottom: 4 }}>
            🧬 Hereditary SMA / Spinal Muscular Atrophy Atlas
          </h1>
          <p style={{ color: '#64748b', fontSize: 13 }}>
            Complete 8-Gene Hereditary SMA Atlas — SMN1 / AR / BICD2 / DYNC1H1 / VAPB / SETX / SIGMAR1 / PLEKHG5 — 320 Patients (8×40, Seeds 1718–1725)
          </p>
        </div>
