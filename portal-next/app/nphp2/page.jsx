...
// NPHP2 colour scheme — indigo-teal-crimson-olive (infantile renal; situs inversus; CHF; Wnt)
const ACCENT  = '#283593';   // deep indigo — INVS / NPHP2 / IFT-zone scaffold
const ACCENT2 = '#00695c';   // dark teal — transplant / ESRD treatment / curative
const ACCENT3 = '#1a237e';   // dark navy — genetics / Wnt switch mechanism
const ACCENT4 = '#b71c1c';   // deep red — ESRD progression / anaemia / infantile urgency
const ACCENT5 = '#4e342e';   // deep brown — situs inversus / laterality defect (unique NPHP2)
const ACCENT6 = '#37474f';   // dark slate — epidemiology / consanguinity / AR genetics
const ACCENT7 = '#e65100';   // deep orange — ARPKD mimic / misdiagnosis alert
const ACCENT8 = '#558b2f';   // dark olive — CHF / hepatic fibrosis / liver surveillance
...
export default function Nphp2Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/nphp2/overview`).then(r => r.json()),
      fetch(`${API}/api/nphp2/breakdown`).then(r => r.json()),
      fetch(`${API}/api/nphp2/definitions`).then(r => r.json()),
    ])
  ...
  if (!ov)  return <div className="container py-4 text-muted">Loading NPHP2 dashboard…</div>;
    ...
    <div className="container-fluid py-3">
      ...
      <div className="mb-3">
        <h4 className="fw-bold mb-0" style={{ color: ACCENT }}>
          🧬 Nephronophthisis Type 2 (NPHP2 — Infantile NPHP; INVS / Inversin)
        </h4>
        <div className="text-muted small">
          INVS · 9q31.1 · 1065 aa IFT-zone boundary scaffold · Wnt canonical-to-non-canonical switch · AR Biallelic LOF · OMIM #602088
          &nbsp;|&nbsp; Cohort: {_COHORT_SIZE} patients (seed-343) · 3 endpoints verified
        </div>
        <div className="mt-1">
          <Badge text="TZ Ciliopathy (Infantile)" color={ACCENT} />
          <Badge text="ESRD Median 3yr" color={ACCENT4} />
          <Badge text="Situs Inversus ~35%" color={ACCENT5} />
          <Badge text="CHF ~55%" color={ACCENT8} />
          <Badge text="Transplant EXCELLENT" color={ACCENT2} />
          <Badge text="No Retinal (no SLS)" color={ACCENT6} />
          <Badge text="AR · ~1/100,000" color={ACCENT6} />
        </div>
      </div>
      ...
      {tab === 0 && (
        <div>
          ...
          <div className="row mb-3">
            ...
            <KPI label="% Kidneys Enlarged" value={kpis.pct_kidneys_enlarged ? `${kpis.pct_kidneys_enlarged}%` : '—'} color={ACCENT7} />
            <KPI label="Mean Hgb (g/dL)" value={kpis.mean_hgb_g_dl ?? '—'} color={ACCENT4} />
            <KPI label="% ARPKD Misdiag." value={kpis.pct_arpkd_misdiagnosis ? `${kpis.pct_arpkd_misdiagnosis}%` : '—'} color={ACCENT7} />
          </div>

          {/* Alerts */}
          <Section title="⚠️ Clinical Alerts" color={ACCENT4}>
            {ov.alerts && Object.entries(ov.alerts).map(([k, v]) => (
              <Alert key={k} color={k === 'ARPKD_mimic' || k === 'situs_inversus_clue' ? ACCENT7 : ACCENT4}>
          ...
          {/* Key Facts */}
          <Section title="🔬 Key Facts — NPHP2 Clinical & Molecular" color={ACCENT}>
          ...
          <Section title="🫘 NPHP2 vs NPHP1 — Key Differences" color={ACCENT2}>
            <div className="table-responsive">
              <table className="table table-sm table-bordered small mb-0">
                <thead style={{ background: ACCENT + '15' }}>
                  <tr>
                    <th>Feature</th>
                    <th style={{ color: ACCENT }}>NPHP2 (INVS) — THIS DISEASE</th>
                    <th style={{ color: ACCENT4 }}>NPHP1 (NPHP1)</th>
                  </tr>
                </thead>
                <tbody>
                  <tr><td className="fw-bold">Onset type</td><td style={{ color: ACCENT }}>INFANTILE</td><td>Juvenile</td></tr>
                  <tr><td className="fw-bold">ESRD median</td><td style={{ color: ACCENT4 }}>3 yr</td><td>13 yr</td></tr>
                  <tr><td className="fw-bold">Kidneys (infancy)</td><td style={{ color: ACCENT7 }}>ENLARGED (mimics ARPKD)</td><td>Normal/Small</td></tr>
                  <tr><td className="fw-bold">Situs inversus</td><td style={{ color: ACCENT5 }}>30–50% (UNIQUE)</td><td>Absent</td></tr>
                  <tr><td className="fw-bold">CHF (liver)</td><td style={{ color: ACCENT8 }}>~55%</td><td>Absent</td></tr>
                  <tr><td className="fw-bold">Retinal (SLS)</td><td>None</td><td>~12% Senior-Løken</td></tr>
                  <tr><td className="fw-bold">Genetics</td><td>Heterogeneous, no founder</td><td>290kb deletion 80%</td></tr>
                  <tr><td className="fw-bold">Transplant</td><td style={{ color: ACCENT2 }}>EXCELLENT, no recurrence</td><td>EXCELLENT, no recurrence</td></tr>
                </tbody>
              </table>
            </div>
          </Section>
                ...
                <tbody>
                  {(ov.patients || []).map(p => (
                    <tr key={p.id}>
                      ...
                      <td style={{ color: p.chf_grade === 'Absent' ? ACCENT6 : ACCENT8 }}>
        ...
        <div>
          <div className="row">
            <div className="col-md-6">
              <Section title="🫘 Kidney Size Distribution" color={ACCENT7}>
              ...
              <Section title="📊 CKD Stage at Current Visit" color={ACCENT4}>
                {Object.entries(bk.ckd_stage_at_diagnosis_now || {}).map(([k, v]) => (
          ...
          <div className="row">
            <div className="col-md-6">
              <Section title="🧬 INVS Allele Distribution" color={ACCENT3}>
      ...
      {tab === 2 && df && (
        <div>
          <div className="row">
            <div className="col-md-6">
              <Section title="🧬 Genetic Architecture (INVS/NPHP2)" color={ACCENT3}>
                ...
                <table className="table table-sm table-bordered small mb-0">
                  ...
                  <tbody>
                    {Object.entries(df.nphp_comparison || {}).map(([k, v]) => (
                      <tr key={k} style={k.includes('NPHP2') ? { background: ACCENT + '15' } : {}}>
                        <td className="fw-bold" style={{ color: k.includes('NPHP2') ? ACCENT : ACCENT6, whiteSpace: 'nowrap' }}>{k}</td>
