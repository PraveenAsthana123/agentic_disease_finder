...
const ACCENT  = '#1a237e';   // deep indigo — GABA-A / inhibitory neurotransmission
const ACCENT2 = '#b71c1c';   // dark red — CI / danger / NCSE / absent acceleration
const ACCENT3 = '#e65100';   // deep orange — fever / alerts / thresholds
const ACCENT4 = '#1b5e20';   // deep green — seizure freedom / safe
const ACCENT5 = '#4a148c';   // purple — DEE spectrum / mechanisms / Dravet-like
...
function OverviewTab({ ov }) {
  ...
  return (
    <div>
      ...
      <div className="row g-2 mb-3">
        ...
        <div className="col-12 col-md-6">
          <div className="alert py-2 mb-0" style={{ fontSize: 12, backgroundColor: '#fff3e0', borderColor: ACCENT3, color: '#bf360c' }}>
            ⚠️ <strong>CBZ/OXC/PHT:</strong> {ov.cbz_oxc_alert}
          </div>
        </div>
      </div>
      ...
      <div className="row g-2 mb-3">
        <div className="col-12 col-md-6">
          <div className="alert py-2 mb-0" style={{ fontSize: 12, backgroundColor: '#e8f5e9', borderColor: ACCENT4, color: '#1b5e20' }}>
            💊 <strong>VPA:</strong> {ov.vpa_alert}
          </div>
        </div>
        <div className="col-12 col-md-6">
      ...
      <div className="alert py-2 mb-3" style={{ fontSize: 12, backgroundColor: '#fce4ec', borderColor: ACCENT2, color: '#880e4f' }}>
        ⛔ <strong>LTG WARNING:</strong> {ov.ltg_alert}
      </div>

      <SectionCard title="🧬 GABRB2 Gene & GABA-A β2 Subunit" borderColor={ACCENT}>
      ...
      <SectionCard title={`📊 Cohort KPIs (n=${ov.n_patients})`} borderColor={ACCENT4}>
        <div className="row g-2">
          <KPI label="GEFS+ %" value={`${ov.gefs_pct}%`} color={ACCENT} />
          <KPI label="Seizure-free" value={`${ov.seizure_free_pct}%`} color={ACCENT4} />
          <KPI label="DRE %" value={`${ov.drug_resistant_pct}%`} color={ACCENT2} />
          <KPI label="Dravet-like" value={`${ov.dravet_like_pct}%`} color={ACCENT5} />
          <KPI label="Absence %" value={`${ov.absence_pct}%`} color={ACCENT} />
          <KPI label="Myoclonic %" value={`${ov.myoclonic_pct}%`} color={ACCENT3} />
          <KPI label="On VPA" value={`${ov.on_vpa_pct}%`} color={ACCENT4} />
          <KPI label="On ETX" value={`${ov.on_etx_pct}%`} color={ACCENT} />
          ...
          <KPI label="Photosensitive" value={`${ov.photosensitive_pct}%`} color={ACCENT3} />
        </div>
      </SectionCard>
...
function PatientsTab({ bk }) {
  ...
  return (
    <div>
      <SectionCard title="📊 Cohort Summary" borderColor={ACCENT}>
        <div className="row g-2">
          {[
            ...
            ['On VPA', `${summary.on_vpa_pct}%`, ACCENT4],
            ['On ETX', `${summary.on_etx_pct}%`, ACCENT],
      ...
      <SectionCard title="🧬 Etiology Distribution (5 Classes — GABRB2)" borderColor={ACCENT5}>
        ...
        <div className="table-responsive">
          <table className="table table-sm table-hover" style={{ fontSize: 11 }}>
            <thead className="table-light">
              <tr>
                <th>ID</th><th>Name</th><th>Age(y)</th><th>Onset(y)</th>
                <th>Etiology</th><th>DRE</th><th>Sz-Free</th>
                <th>Absence</th><th>Myocl.</th><th>VPA</th><th>ETX</th><th>POLG</th>
              </tr>
            </thead>
            <tbody>
              {(patients_sample || []).map(p => (
                <tr key={p.id}>
                  ...
                  <td>{p.absence ? '✓' : '—'}</td>
                  <td>{p.myoclonic ? <span style={{ color: ACCENT3 }}>✓</span> : '—'}</td>
                  <td>{p.on_vpa ? '✓' : '—'}</td>
                  <td>{p.on_etx ? '✓' : '—'}</td>
...
function SeizuresTab({ bk }) {
  ...
  return (
    <div>
      <SectionCard title="⚡ Seizure Types (GABRB2 — GEFS+ / CAE / DEE spectrum)" borderColor={ACCENT}>
      ...
      <SectionCard title="🌡️ Seizure Triggers (GABRB2)" borderColor={ACCENT3}>
...
function TreatmentsTab({ bk }) {
  ...
  return (
    <div>
      <SectionCard title="💊 Treatments (GABRB2 — Evidence-Based)" borderColor={ACCENT4}>
          ...
          <div key={i} className="mb-3 p-2 border rounded" style={{ borderLeft: `4px solid ${ACCENT4}` }}>
            ...
            {t.gabrb2_note && (
              <div className="alert py-1 mt-1 mb-0" style={{ fontSize: 12, backgroundColor: '#e8f5e9', borderColor: ACCENT4, color: '#1b5e20' }}>
                🧬 <strong>GABRB2-specific:</strong> {t.gabrb2_note}
              </div>
            )}
          </div>
        ))}
      ...
      <SectionCard title="🚫 Contraindications (GABRB2)" borderColor={ACCENT2}>
      ...
      <SectionCard title="📋 Monitoring Protocol (GABRB2)" borderColor={ACCENT5}>
        <div className="table-responsive">
          <table className="table table-sm mb-0" style={{ fontSize: 12 }}>
            <thead className="table-light">
      ...
      <SectionCard title="📅 Disease Lifecycle (GABRB2 — 6 Windows)" borderColor={ACCENT}>
        {(lifecycle || []).map((l, i) => (
          <div key={i} className="mb-2 p-2 border rounded">
...
function DefinitionsTab({ def }) {
  ...
  return (
    <div>
      <SectionCard title="📖 Key Concepts (GABRB2 / GABA-A β2 / GEFS+ / CAE / DEE)" borderColor={ACCENT}>
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function GABRB2Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/gabrb2/overview`).then(r => r.json()),
      fetch(`${API}/api/gabrb2/breakdown`).then(r => r.json()),
      fetch(`${API}/api/gabrb2/definitions`).then(r => r.json()),
    ...
    <div className="container-fluid py-3">
      <div className="mb-3">
        <h4 className="fw-bold mb-0" style={{ color: ACCENT }}>
          🧠 GABRB2 Epilepsy (GEFS+ / CAE / DEE-Dravet-like / GABA-A β2 Subunit / 5q34)
        </h4>
        <div className="text-muted small">
          GABRB2 — GABA-A Receptor β2 Subunit · Phasic Synaptic Inhibition ·
          5q34 · GEFS+ / Childhood Absence / Dravet-like DEE / LGS-DEE ·
          VPA first-line (POLG mandatory) · AVOID CBZ/OXC/PHT/LTG (absence + myoclonic aggravation)
        </div>
      </div>
