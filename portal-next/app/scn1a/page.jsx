...
const ACCENT  = '#1a237e';   // deep indigo — NaV1.1 / SCN1A / Dravet
...
const ACCENT5 = '#4a148c';   // purple — Dravet / DEE / encephalopathy
...
export default function SCN1APage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/scn1a/overview`).then(r => r.json()),
      fetch(`${API}/api/scn1a/breakdown`).then(r => r.json()),
      fetch(`${API}/api/scn1a/definitions`).then(r => r.json()),
    ])
      .then(([o, b, d]) => { setOv(o); setBk(b); setDf(d); setLoading(false); })
      .catch(e => { setError(e.message); setLoading(false); });
  }, []);

  if (loading) return <div className="p-5 text-center text-muted">Loading SCN1A dashboard…</div>;
  ...
  return (
    <div className="container-fluid py-3">
      ...
      <div className="p-3 rounded mb-3 text-white" style={{ background: ACCENT }}>
        <h3 className="mb-0">🧬 SCN1A Epilepsy</h3>
        <div className="small opacity-75 mt-1">
          Dravet Syndrome (SMEI / DEE6) · GEFS+ · NaV1.1 Channelopathy · 2q24.3
        </div>
        ...
        <div>
          ...
          <Section title={`Cohort KPIs (n=${ov.n_patients})`}>
            <div className="row g-2">
              ...
              <KPI label="On Stiripentol %" value={`${ov.on_stiripentol_pct}%`} color={ACCENT5} />
              <KPI label="On Fenfluramine %" value={`${ov.on_fenfluramine_pct}%`} color={ACCENT3} />
              <KPI label="On Cannabidiol %" value={`${ov.on_cbd_pct}%`} color={ACCENT} />
              <KPI label="On KD %" value={`${ov.on_kd_pct}%`} color={ACCENT3} />
              <KPI label="SUDEP Device %" value={`${ov.sudep_device_pct}%`} color={ACCENT5} />
              <KPI label="NaV Blocker Exp." value={ov.navblocker_exposure_n} color={ACCENT2} />
            </div>
          </Section>

          <Section title="Critical Alerts">
            <AlertBox color={ACCENT2} title="⛔ CBZ / OXC / PHT — ABSOLUTE CI in Dravet Syndrome" body={ov.cbl_alert} />
            <AlertBox color={ACCENT2} title="⛔ LTG (Lamotrigine) — ABSOLUTE CI in Dravet; HIGH RISK in GEFS+ with myoclonic component" body={ov.ltg_alert} />
            <AlertBox color={ACCENT2} title="⛔ TIAGABINE — ABSOLUTE CI (GAT-1 block → NCSE)" body={ov.tiagabine_alert} />
            <AlertBox color={ACCENT3} title="🔬 POLG MANDATORY before VPA" body={ov.polg_alert} />
            <AlertBox color={ACCENT3} title="💊 Fenfluramine (Fintepla) — REMS + Echocardiogram mandatory" body={ov.fenfluramine_alert} />
            <AlertBox color={ACCENT5} title="💀 SUDEP Risk Elevated in Dravet" body={ov.sudep_alert} />
          </Section>
              ...
              <table className="table table-sm table-bordered small">
                <thead style={{ background: ACCENT, color: 'white' }}>
                  <tr>
                    ...
                    <th>Sz-Free</th><th>DRE</th><th>VPA</th><th>POLG</th>
                    <th>VPA w/o POLG</th><th>STP</th><th>FEN</th><th>CBD</th><th>KD</th><th>SUDEP Dev</th><th>CSWS</th>
                  </tr>
                </thead>
                <tbody>
      ...
      {tab === 2 && (
        <div>
          <Section title="Seizure Types (6 types)">
            {bk.seizure_types.map((st, i) => (
              <div key={i} className="card mb-3 shadow-sm">
                <div className="card-header fw-bold" style={{ background: ACCENT5 + '18' }}>
                  {st.type} — <span style={{ color: ACCENT5 }}>{st.pct}% of patients</span>
                </div>
                <div className="card-body small">
          ...
          <Section title="Seizure Triggers (8 triggers)">
            {bk.triggers.map((t, i) => (
              <div key={i} className="card mb-3 shadow-sm">
                <div className="card-header fw-bold" style={{ background: ACCENT3 + '18' }}>
                  {t.trigger} — <span style={{ color: ACCENT3 }}>{t.pct}%</span>
                </div>
                <div className="card-body small">
                  <p className="mb-1"><strong>Threshold:</strong> {t.threshold}</p>
                  <p className="mb-0"><strong>Management:</strong> {t.mgmt}</p>
                </div>
              </div>
            ))}
          </Section>
        ...
        <div>
          <Section title="Contraindications">
            {bk.contraindications.map((ci, i) => (
              <div key={i} className="p-3 mb-3 rounded" style={{ background: ACCENT2 + '12', border: `2px solid ${ACCENT2}` }}>
                <strong style={{ color: ACCENT2 }}>⛔ {ci.drug}</strong>
                <span className="ms-2 badge" style={{ background: ACCENT2 }}>{ci.risk}</span>
                <p className="small mt-2 mb-1">{ci.reason}</p>
                <p className="small mb-0"><strong>Alternative:</strong> {ci.alternative}</p>
              </div>
            ))}
          </Section>

          <Section title="Treatments (7 drugs)">
            {bk.treatments.map((t, i) => (
              <div key={i} className="card mb-3 shadow-sm">
                <div className="card-header fw-bold" style={{ background: ACCENT4 + '18' }}>
                  {t.drug}
                  <span className="ms-2 badge" style={{ background: ACCENT4 }}>{t.evidence}</span>
                </div>
                <div className="card-body small">
                  <p className="mb-1"><strong>Indication:</strong> {t.indication}</p>
                  <p className="mb-1"><strong>Efficacy:</strong> {t.efficacy}</p>
                  <div className="p-2 rounded" style={{ background: ACCENT + '10' }}>
                    <strong>SCN1A-specific note:</strong> {t.scn1a_note}
                  </div>
                </div>
              </div>
            ))}
      ...
      {tab === 4 && (
        <div>
          <Section title="Key Concepts (14 terms)">
            {df.concepts.map((c, i) => (
              <div key={i} className="mb-3 p-2 rounded" style={{ background: ACCENT + '08', border: `1px solid ${ACCENT}22` }}>
