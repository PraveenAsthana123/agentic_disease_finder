...
const COLOR  = '#006064';   // dark teal — tRNA-Ser(UCN) / L-strand isolated / SNHL-dominant
const LIGHT  = '#e0f7fa';
const COLOR2 = '#00838f';   // medium teal — CPEO / myopathy
...
const COLOR5 = '#1a237e';   // deep indigo — SARS2 DDx / dual-gene boundary
...
function OverviewTab({ data }) {
  ...
  return (
    <>
      {/* Gene facts banner */}
      <SectionCard title="Gene Facts — MT-TS1 tRNA-Ser(UCN)" borderColor={COLOR}>
      ...
      {/* Dual-gene overlap note */}
      {data.dual_gene_overlap_note && (
        <div className="alert border-0 mb-4" style={{ backgroundColor: '#fff3e0', borderLeft: `5px solid #e65100` }}>
          <strong style={{ color: '#e65100' }}>🔬 {data.dual_gene_overlap_note.note_class}</strong>
          <p className="mb-0 small mt-1">{data.dual_gene_overlap_note.detail}</p>
        </div>
      )}
      ...
      {data.sars2_ddx_note && (
        <div className="alert border-0 mb-4" style={{ backgroundColor: '#e8eaf6', borderLeft: `5px solid ${COLOR5}` }}>
          <strong style={{ color: COLOR5 }}>🧬 {data.sars2_ddx_note.note_class}</strong>
          <p className="mb-0 small mt-1">{data.sars2_ddx_note.detail}</p>
        </div>
      )}
...
function VariantsTab({ data }) {
  ...
  return (
    <>
      <SectionCard title="Variant Breakdown — 40-Patient Cohort (seed-823)" borderColor={COLOR}>
        <div className="table-responsive">
          <table className="table table-sm table-hover small mb-0">
            <thead><tr>
              <th>Variant</th><th>n</th><th>% Cohort</th>
              <th>Avg Hetero</th><th>Avg CI%</th><th>Avg CIV%</th>
              <th>SNHL%</th><th>CPEO%</th><th>Myo%</th><th>Cardio%</th>
            </tr></thead>
            <tbody>
              {vb.map((r, i) => (
                <tr key={i}>
      ...
      <SectionCard title="Management by Variant" borderColor={COLOR2}>
        <div className="table-responsive">
          <table className="table table-sm table-hover small mb-0">
            <thead><tr>
              <th>Variant</th><th>SNHL Risk</th><th>CPEO Risk</th><th>Key Action</th>
            </tr></thead>
            <tbody>
              {mgmt.map((r, i) => (
                <tr key={i}>
      ...
      <SectionCard title="Differential Diagnosis Table" borderColor={COLOR2}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small mb-0">
            ...
            <tbody>
              {ddx.map((r, i) => (
                <tr key={i} style={r.entity && r.entity.startsWith('MT-TS1') ? { backgroundColor: LIGHT, fontWeight: 600 } : {}}>
                  <td>{r.entity}</td>
                  <td>{r.inheritance}</td>
                  <td>{r.phenotype}</td>
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function MTTS1Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/mtts1/overview`).then(r => r.json()),
      fetch(`${API}/api/mtts1/breakdown`).then(r => r.json()),
      fetch(`${API}/api/mtts1/definitions`).then(r => r.json()),
    ])
  ...
  return (
    <div className="container-fluid py-4">
      ...
      <div className="mb-4 p-3 rounded" style={{ background: `linear-gradient(135deg, ${COLOR} 0%, ${COLOR2} 100%)`, color: '#fff' }}>
        <h4 className="fw-bold mb-1">🧬 MT-TS1 — tRNA-Ser(UCN) (UGA Anticodon)</h4>
        <p className="mb-1 small opacity-75">
          SNHL-Dominant · Combined CI+CIV Deficiency · L-strand ISOLATED NGS Pitfall — TWELFTH tRNA — rCRS 7445–7516
        </p>
        <div className="d-flex gap-3 flex-wrap small">
          <span className="badge bg-light text-dark">OMIM *590080</span>
          <span className="badge bg-light text-dark">L-strand ISOLATED — NGS Pitfall</span>
          <span className="badge bg-light text-dark">40-patient cohort seed-823</span>
          <span className="badge bg-light text-dark">m.7445A&gt;G Dual-Gene MT-CO1+MT-TS1</span>
          <span className="badge bg-light text-dark">SARS2 DDx — HUPRA Syndrome</span>
        </div>
      </div>
