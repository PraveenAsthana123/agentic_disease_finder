  ...
  return (
    <div>
      <div className="alert alert-info py-2 small mb-3" style={{ borderLeft: `4px solid ${COLOR}` }}>
        <strong>GLRA1 (5q33.1) — Glycine Receptor α1 · Hyperekplexia Type 1:</strong>{' '}
        GLRA1 is the <strong>most common genetic cause of hyperekplexia (~70% of families)</strong>.{' '}
        Hyperekplexia is <strong>NON-EPILEPTIC</strong> — EEG is normal during events.{' '}
        Arg271 (TM2 gate residue) is the hotspot — dominant-negative impairs glycine-gated Cl⁻ conductance.{' '}
        <strong>First-line: Clonazepam + Forward-Flexion Manoeuvre (Vigevano).</strong>{' '}
        <span className="text-danger fw-bold">
          ABSOLUTE SAFETY: no discharge without forward-flexion carer training.{' '}
          POLG mandatory before VPA. Metabolic screen (exclude NKH, biotinidase) at diagnosis.
        </span>
      </div>
    ...
    <div>
      ...
      <div className="row g-3 mb-4">
        {[
          ...
          { label: 'Metabolic Screened', val: `${summary.metabolic_screened_pct}%`, color: '#0d6efd' },
          { label: 'Video-EEG Done', val: `${summary.video_eeg_done_pct}%`, color: COLOR },
        ].map((s, i) => (
            ...
            <table className="table table-sm table-striped small mb-0">
              <thead>
                <tr style={{ background: LIGHT }}>
                  ...
                  <th>Apnoea</th><th>Rigid</th><th>Falls</th><th>EpilSz</th>
                  <th>CLZ</th><th>Trained</th><th>Nose+</th><th>MetScr</th>
                </tr>
              </thead>
              <tbody>
                {patients.map((p, i) => (
                  <tr key={i}>
                    ...
                    <td>{p.metabolic_screened ? '✓' : <span className="text-warning">!</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div>
      <div className="alert alert-warning py-2 small mb-3">
        <strong>⚡ Critical Distinction:</strong> Hyperekplexia events are <strong>NON-EPILEPTIC</strong> —
        normal EEG during events. Forward-flexion manoeuvre terminates apnoea; AEDs do not.
        Video-EEG mandatory to classify each event type.
      </div>
  ...
  return (
    <div>
      <div className="alert alert-success py-2 small mb-3">
        ...
        Hyperekplexia events are NOT epileptic — do not treat with phenytoin or carbamazepine.
        Natural history: 70-80% of patients can reduce/stop CLZ by late childhood (GlyR maturation).
      </div>
            ...
            <div className="card shadow-sm h-100">
              ...
              <div className="card-body small">
                ...
                <div className="alert alert-info py-1 px-2 mb-0 small">
                  <strong>GLRA1 Note:</strong> {t.glra1_note?.slice(0, 200)}…
                </div>
              </div>
            </div>
          </div>
...
export default function GLRA1Page() {
  ...
  useEffect(() => {
    fetch(`${API}/api/glra1/overview`).then(r => r.json()).then(setOverview).catch(() => {});
  }, []);

  useEffect(() => {
    if (tab === 'Patients & Events' && !breakdown)
      fetch(`${API}/api/glra1/breakdown`).then(r => r.json()).then(setBreakdown).catch(() => {});
    if (tab === 'Event Types & Triggers' && !breakdown)
      fetch(`${API}/api/glra1/breakdown`).then(r => r.json()).then(setBreakdown).catch(() => {});
    if (tab === 'Treatments' && !breakdown)
      fetch(`${API}/api/glra1/breakdown`).then(r => r.json()).then(setBreakdown).catch(() => {});
    if (tab === 'Definitions' && !definitions)
      fetch(`${API}/api/glra1/definitions`).then(r => r.json()).then(setDefinitions).catch(() => {});
  }, [tab]);
      ...
      <div className="d-flex align-items-center mb-3 gap-3">
        <div>
          ...
          <div className="text-muted small">
            Hyperekplexia Type 1 (HYPER1) · OMIM Gene 138491 · OMIM Disease 149400 · 5q33.1 ·
            Glycine Receptor α1 · ~70% of Genetic Hyperekplexia · 40-patient cohort seed-495
          </div>
        </div>
