...
const COLOR = '#1b5e20';   // deep green — MCIA complex / riboflavin-responsive
const LIGHT = '#e8f5e9';
  ...
  return (
    <>
      <div className="row g-3 mb-4">
        ...
        <KPI label="Riboflavin Resp."  value={`~${co.riboflavin_responders_pct || 55}%`} color="#2e7d32" />
      ...
      <div className="alert mb-4" style={{ background: '#e8f5e9', borderLeft: '4px solid #2e7d32' }}>
        <strong>🟢 Riboflavin-Responsive (~50–60%)</strong> — ACAD9 is the ONLY CI assembly factor
        with established riboflavin (B2) responsiveness. Trial riboflavin 100–300 mg/day in all newly diagnosed patients.
        Monitor CI activity at 3 months. Level B evidence.
      </div>
      ...
      <SectionCard title="🔄 Key Pathway Note — MCIA Complex / ACAD9 / Riboflavin Responsiveness">
      ...
      <SectionCard title="🔬 Biochemical Fingerprint">
        {Object.entries(bf).map(([k, v]) => (
          <div key={k} className="d-flex justify-content-between small mb-1 flex-wrap">
            ...
            <span className={v.toString().includes('NORMAL') ? 'text-success' : 'text-danger fw-bold'}>{v}</span>
          </div>
        ))}
      </SectionCard>

      <SectionCard title="📊 Feature Frequencies (40-patient cohort, seed-675)">
        {Object.entries(ff).map(([k, v]) => (
          <Bar key={k} label={k.replace(/_/g, ' ')} value={v}
            color={k === 'exercise_intolerance' || k === 'skeletal_myopathy' ? '#1b5e20' : COLOR} />
        ))}
      </SectionCard>
    </>
  );
  ...
  return (
    <>
      <div className="row g-3 mb-4">
        ...
        <KPI label="Riboflavin responders"   value={`${data.riboflavin_responders} (${data.riboflavin_responder_pct}%)`} color="#2e7d32" />
          ...
          <SectionCard title="📈 Outcome Distribution">
            {Object.entries(out).sort((a,b) => b[1]-a[1]).map(([k,v]) => (
              <div key={k} className="d-flex justify-content-between small mb-2">
                <span>{k}</span>
                <span className="badge" style={{ background: k.includes('riboflavin') ? '#2e7d32' : COLOR }}>{v}</span>
              </div>
            ))}
          </SectionCard>
      ...
      <SectionCard title="🗂 Patient Cohort (40 patients, seed-675)">
            ...
            <tbody>
              {pts.map(p => (
                <tr key={p.id}>
                  ...
                  <td><span className={`badge ${p.riboflavin_responder ? 'bg-success' : 'bg-secondary'}`}>{p.riboflavin_responder ? 'Yes' : 'No'}</span></td>
...
function TreatmentsTab({ data }) {
  ...
  return (
    <>
      <div className="alert mb-4" style={{ background: '#e8f5e9', borderLeft: '4px solid #2e7d32' }}>
        <strong>🟢 Riboflavin (B2) — Level B Evidence (UNIQUE to ACAD9)</strong><br/>
        100–300 mg/day in 3 divided doses. ~50–60% respond. Monitor CI activity at 3 months. Continue indefinitely if any response.
        The ONLY CI assembly factor with established riboflavin responsiveness.
      </div>
      ...
      <SectionCard title="🔬 Technical Details">
        <div className="row g-2">
          {[
            ...
            ['FAD-binding domain', String(data.fad_binding)],
            ...
            ['Urine organic acids', String(data.urine_organic_acids_normal) + ' (normal — DDx vs ETFDH/MADD)'],
            ['Riboflavin response rate', `~${data.riboflavin_response_rate_pct}%`],
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function ACAD9Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/acad9/overview`).then(r => r.json()),
      fetch(`${API}/api/acad9/breakdown`).then(r => r.json()),
      fetch(`${API}/api/acad9/definitions`).then(r => r.json()),
  ...
  return (
    <div className="container-fluid py-4">
      <div className="mb-4 p-3 rounded" style={{ background: COLOR, color: '#fff' }}>
        <h4 className="mb-1 fw-bold">&#x1f9ec; ACAD9 — Complex I Deficiency (MCIA Complex / Riboflavin-Responsive)</h4>
        <div className="small opacity-90">
          CI Deficiency · MCIA Complex Scaffold (ACAD9 + NDUFAF1 + ECSIT + TMEM126B) ·
          621aa · 70 kDa · FAD-binding · 3q21.3 · OMIM Gene *611103 · AR biallelic ·
          Isolated CI 10–30% · CII/CIII/CIV NORMAL · Riboflavin-responsive ~50–60%
        </div>
      </div>
