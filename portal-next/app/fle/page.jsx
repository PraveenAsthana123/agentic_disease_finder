  ...
  return (
    <div>
      <div className="alert alert-info py-2 small mb-3">
        <strong>Frontal Lobe Epilepsy (FLE):</strong> Second most common <strong>focal epilepsy</strong> (~20–30% of focal epilepsies).
        Hallmark: <em>hypermotor seizures — explosive onset, bilateral asymmetric limb thrashing, vocalization, nocturnal clustering</em>.
        EEG: low-voltage fast activity (LVFA) or frontal spike. Drug resistance ~30–40%.
        <strong> Surgical cure: 50–60% seizure-free (Jeha 2007 / Englot 2011)</strong> — FCD most common structural cause; refer after ≥2 AED failures.
      </div>
          ...
          <div className="card h-100 shadow-sm">
            ...
            <div className="card-body">
              {(data.etiology_distribution || []).map(e => (
                <Bar key={e.etiology} label={`${e.etiology} (${e.pct}%)`} value={e.count} max={maxEtiol} color="#7c3aed" />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="row mt-2">
        <div className="col-md-6">
          <div className="card shadow-sm">
            <div className="card-header fw-bold">Seizure Triggers (% of FLE patients affected)</div>
            <div className="card-body">
          ...
          <div className="card shadow-sm">
            ...
            <div className="card-body">
              {(data.seizure_types || []).map((st, i) => {
                const colors = ['#1d4ed8', '#7c3aed', '#dc2626', '#0891b2'];
                return <PctBar key={st.type} label={`${st.type} (${st.prevalence_pct}%)`} pct={st.prevalence_pct} color={colors[i % colors.length]} />;
              })}
            </div>
          </div>
        </div>
...
function PatientsTab({ data }) {
  ...
  const etiolColors = ['#7c3aed', '#1d4ed8', '#0891b2', '#16a34a', '#f59e0b', '#dc2626'];
        ...
        <table className="table table-sm table-hover">
          <thead className="table-dark">
            <tr>
              ...
              <th>Etiology</th><th>Onset Semiology</th><th>AED Regimen</th><th>Control</th>
              <th>Nocturnal</th><th>Catamenial</th><th>Post-Surgical</th><th>Yrs on AED</th>
            </tr>
          </thead>
          <tbody>
            {sorted.map(p => (
              <tr key={p.patient_id}>
                ...
                <td><small className="text-muted">{p.onset_semiology}</small></td>
                ...
                <td>{p.nocturnal_predilection ? '🌙 Yes' : '—'}</td>
...
function SeizureSurgeryTab({ data }) {
  ...
  return (
    <div>
      <h6 className="fw-bold mb-3">Seizure Types in FLE</h6>
      <div className="row mb-4">
        {(data.seizure_types || []).map((st, i) => (
          <div key={st.type} className="col-md-6 mb-3">
            <div className="card h-100 shadow-sm" style={{ borderLeft: `4px solid ${typeColors[i % typeColors.length]}` }}>
              ...
              <div className="card-body small">
                ...
                <div className="mb-1"><strong>Nocturnal:</strong> {st.nocturnal ? '🌙 Yes — confirm with overnight PSG-EEG' : 'Daytime predominant'}</div>
      ...
      <div className="alert alert-success py-2 small">
        <strong>✂️ SURGICAL PATHWAY — FLE (Level A Evidence: Jeha 2007 / Englot 2011)</strong>
        <ul className="mb-0 mt-1">
          <li><strong>Trigger:</strong> ≥2 adequate AED failures (ILAE DRE definition 2010) → Level 4 epilepsy centre referral</li>
          <li><strong>Pre-surgical workup:</strong> Video-EEG (seizure capture) + 3T MRI epilepsy protocol (FCD thin cuts) + FDG-PET + ictal SPECT + neuropsychology</li>
          <li><strong>If MRI-negative or multi-focal:</strong> SEEG (stereo-EEG) depth electrodes → delineate epileptogenic zone; consider MEG + 7T MRI for FCD type I</li>
          <li><strong>FCD outcome:</strong> 50–60% Engel Class I post-frontal resection (Jeha 2007 Epilepsia); lower than MTLE-HS due to diffuse dysplasia</li>
          <li><strong>Lesional FLE:</strong> tumors/cavernomas → 70–80% seizure-free if complete resection</li>
          <li><strong>ADNFLE:</strong> surgery generally NOT recommended (genetic bilateral; prefer pharmacological management)</li>
        </ul>
      </div>
    </div>
  );
...
function TreatmentsTab({ data }) {
  ...
  return (
    <div>
      <h6 className="fw-bold mb-3">AED Therapies & Surgical Options for FLE</h6>
      <div className="row mb-4">
            ...
            <div key={tx.drug} className="col-md-6 mb-3">
              <div className="card h-100 shadow-sm" style={{ borderLeft: `4px solid ${colors[i % colors.length]}` }}>
                <div className="card-header fw-bold small d-flex justify-content-between">
                  <span>{tx.drug}</span>
                  <span className="badge bg-secondary small">{tx.evidence_level || tx.type}</span>
                </div>
                <div className="card-body small">
                  ...
                  <div className="text-muted fst-italic">{tx.evidence_note || tx.fda_status}</div>
                </div>
              </div>
            </div>
          );
            ...
            <div key={lc.age_window} className="col-md-6 mb-3">
              <div className="card h-100 shadow-sm" style={{ borderLeft: `4px solid ${colors[i % colors.length]}` }}>
                ...
                <div className="card-body small">
                  ...
                  <div className="mb-1"><strong>FLE indicators:</strong> {lc.fle_indicators || lc.tle_indicators}</div>
...
export default function FLEDashboard() {
  ...
  useEffect(() => {
    fetch(`${API}/api/fle/overview`).then(r => r.json()).then(setOverview).catch(() => setError('Backend unreachable'));
  }, []);

  useEffect(() => {
    if (tab >= 1 && tab <= 3 && !breakdown) {
      fetch(`${API}/api/fle/breakdown`).then(r => r.json()).then(setBreakdown).catch(() => {});
    }
    if (tab === 4 && !definitions) {
      fetch(`${API}/api/fle/definitions`).then(r => r.json()).then(setDefinitions).catch(() => {});
    }
  }, [tab]);
  ...
  return (
    <div className="container-fluid py-3">
      <div className="d-flex align-items-center mb-3">
        ...
        <div>
          <h4 className="mb-0 fw-bold">Frontal Lobe Epilepsy (FLE)</h4>
          <small className="text-muted">
            2nd Most Common Focal Epilepsy · ICD-10: G40.109 / G40.119 · ~20–30% of focal epilepsies ·
            Drug resistance ~30–40% · Surgery: 50–60% seizure-free (Jeha 2007 / Englot 2011)
          </small>
        </div>
      </div>
