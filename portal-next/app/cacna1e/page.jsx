...
const COLOR = '#880e4f'; // deep crimson — R-type Cav2.3 DEE69 identity (distinct from CACNA1A purple, CACNA1G teal, CACNA1H amber, CACNA1I violet)
const DANGER = '#b71c1c'; // dark red — CI / danger
const SUCCESS = '#1b5e20'; // dark green — success / spasm freedom
  ...
  return (
    <div>
      <div className="alert alert-info py-2 small mb-3 border" style={{ borderColor: COLOR, borderLeftWidth: 5 }}>
        <strong>🧬 CACNA1E (1q25.3) — Cav2.3 R-type HVA Ca²⁺ Channel — DEE69 — GOF de novo:</strong>{' '}
        CACNA1E encodes Cav2.3 (α1E), the sole <strong>R-type</strong> (Resistant to nimodipine/ω-conotoxin/ω-agatoxin) HVA Ca²⁺ channel.{' '}
        GOF → hyperpolarised V1/2 activation shift + slowed inactivation → <strong>enlarged window current</strong> → excess postsynaptic Ca²⁺{' '}
        → excitotoxic DEE. <strong>Cav2 subfamily:</strong> Cav2.1/CACNA1A (P/Q-type DEE42) · Cav2.2/CACNA1B (N-type) · Cav2.3/CACNA1E (R-type DEE69).{' '}
        CACNA1E is <strong>DISTINCT from T-type GGE</strong> (CACNA1G/H/I): R-type GOF → severe DEE69 (not absence/GGE).{' '}
        Helbig 2019 Nat Genet: 30 de novo GOF probands.{' '}
        <span className="text-danger fw-bold">
          NO approved R-type precision blocker. ACTH Level A for West syndrome (UKISS 2004).{' '}
          ABSOLUTE CI: TGB (NCSE) · VPA+POLG1 (Alpers). HIGH RISK: VGB long-term (VFD). KD HIGH PRIORITY ≥2 AED failures.
        </span>
      </div>
      ...
      <div className="row g-2 mb-4">
        ...
        <KPI label="Movement Disorder" value={kpis.movement_disorder_n} color={WARN} />
        ...
        <div className="col-md-6">
          <SectionCard title="Seizure Type Prevalence">
            {(data.seizure_summary || []).map(s => (
              <Bar key={s.type} label={s.type.split(' (')[0].substring(0, 38)} value={s.pct} max={100} />
            ))}
          </SectionCard>
        </div>
        <div className="col-md-6">
        ...
        {etiologies.map(e => (
          <div key={e.category} className="mb-3 border rounded p-2">
            ...
            <div className="small mt-1">
              <span className="badge text-bg-secondary me-1">Onset: {e.onset_age_years < 1 ? `${Math.round(e.onset_age_years * 12)}M` : `${e.onset_age_years}Y`}</span>
          ...
          <table className="table table-sm table-hover small">
            <thead>
              <tr>
                ...
                <th>ACTH</th><th>Spasms</th><th>GTCS</th><th>KD</th>
              </tr>
            </thead>
            <tbody>
              {patients.map(p => (
...
function TreatmentsTab({ data }) {
  ...
  const levelColor = (level) => {
    ...
    if (level.includes('Level B')) return '#880e4f';
  ...
  return (
    <div>
      <div className="alert small py-2 mb-3" style={{ backgroundColor: '#fce4ec', borderLeft: `5px solid ${DANGER}` }}>
        <strong>⚠️ NO APPROVED R-TYPE PRECISION BLOCKER for CACNA1E DEE69:</strong> Unlike CACNA1G/H/I (T-type GOF → ETX Level A) or
        CACNA1A (P/Q-type → lamotrigine/acetazolamide for EA2), CACNA1E R-type Cav2.3 GOF has NO approved precision Ca²⁺ channel blocker.
        SNX-482 (spider toxin) blocks R-type selectively but has no human clinical use. Management is <strong>symptomatic seizure control</strong>.{' '}
        <strong>KD is HIGHEST PRIORITY</strong> after ≥2 AED failures (mechanistically supported for R-type Ca²⁺ overload pathology).{' '}
        <strong>ABSOLUTE CI: TGB (NCSE) · VPA+POLG1 (Alpers). HIGH RISK: VGB long-term (VFD — ERG q3M mandatory).</strong>
      </div>
      ...
      <SectionCard title="Treatments (8)">
        {treatments.map((t, i) => (
          <div key={i} className="mb-3 border rounded p-2">
            ...
            <div className="small p-1 rounded" style={{ background: '#fce4ec' }}>
              <strong>CACNA1E-specific:</strong> {t.cacna1e_note}
            </div>
          </div>
        ))}
      </SectionCard>
...
function DefinitionsTab({ data }) {
  ...
  return (
    <div>
      <SectionCard title="Gene Summary — CACNA1E (1q25.3)">
...
export default function CACNA1EPage() {
  ...
  useEffect(() => { fetchData('/api/cacna1e/overview', setOverview, 'overview'); }, []);

  useEffect(() => {
    if (tab === 1 || tab === 2 || tab === 3) {
      if (!breakdown) fetchData('/api/cacna1e/breakdown', setBreakdown, 'breakdown');
    }
    if (tab === 4) {
      if (!definitions) fetchData('/api/cacna1e/definitions', setDefinitions, 'definitions');
    }
  }, [tab]);
      ...
      <div className="d-flex align-items-center mb-3 gap-3">
        <div>
          ...
          <div className="text-muted small">
            DEE69 / Cav2.3 R-type HVA Ca²⁺ Channel / GOF de novo / OMIM #618285 / 1q25.3 · n=40
          </div>
        </div>
        <div className="ms-auto">
          <span className="badge me-1" style={{ background: COLOR }}>CACNA1E GOF</span>
          <span className="badge me-1" style={{ background: DANGER }}>DEE69</span>
          <span className="badge me-1" style={{ background: '#1b5e20' }}>ACTH Level A</span>
