...
const TABS = ['Overview', 'Diagnostic Breakdown', 'TMEM216 Co-Dependency Pearls', 'Definitions'];

// JBTS16 colour scheme — TMEM138 / TZ membrane / TMEM216 co-dependency / No MKS tier
const ACCENT   = '#4a148c';   // deep purple — TZ membrane / TMEM138
const ACCENT2  = '#7b1fa2';   // medium purple — TMEM216 co-dependency
const ACCENT3  = '#1565c0';   // blue — neurological
const ACCENT4  = '#00695c';   // dark teal — renal
const ACCENT5  = '#e65100';   // orange — polydactyly
...
const ACCENT10 = '#6a1b9a';   // vivid purple — TMEM co-dependency
...
export default function JBTS16Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/jbts16/overview`).then(r => r.json()),
      fetch(`${API}/api/jbts16/breakdown`).then(r => r.json()),
      fetch(`${API}/api/jbts16/definitions`).then(r => r.json()),
    ])
  ...
  return (
    <div className="container-fluid py-3">
      ...
      <div className="mb-3 p-3 rounded" style={{ background: `linear-gradient(135deg, ${ACCENT} 0%, ${ACCENT10} 100%)`, color: '#fff' }}>
        <h4 className="mb-1 fw-bold">&#x1f9ec; TMEM138 — Joubert Syndrome Type 16 (JBTS16)</h4>
        <div className="small opacity-90">
          Transmembrane Protein 138 · 11q12.2 · 163 aa · 3-Pass TZ Membrane · TMEM216 Co-Dependency · No MKS Tier · AR · OMIM Gene *614965 · Disease #614465
        </div>
        <div className="small opacity-80 mt-1">
          Cohort: {N_COHORT} JBTS16 patients (seed {SEED}) · all biallelic genotypes → live birth (no MKS lethal tier) · adjacent to TMEM216/JBTS2 on 11q12.2
        </div>
      </div>

      {/* No MKS tier banner */}
      <Alert color={ACCENT}>
        <strong>&#x2714; No MKS Lethal Tier — TMEM138-Specific Rule:</strong>{' '}
        {overview?.no_mks_pearl?.slice(0, 220)}… TMEM138 is a TZ membrane co-stabiliser, not a core MKS-module scaffold.
        All biallelic genotypes → JBTS16 live birth. Standard 25% AR recurrence applies; no MKS tier calculation needed.
      </Alert>
      ...
      {tab === 0 && (
        <div>
          ...
          {/* TMEM138 Function Pearl */}
          <Alert color={ACCENT2}>
            <strong>&#x1f9ea; TMEM138 / TZ Membrane Gate Mechanism:</strong>{' '}
            {overview?.tmem138_function_pearl}
          </Alert>

          {/* TMEM216 Co-Dependency Pearl */}
          <Alert color={ACCENT10}>
            <strong>&#x1f9ec; TMEM216 Co-Dependency — Adjacent 11q12.2 Mutual Stabilisation:</strong>{' '}
            {overview?.tmem216_codependency_pearl}
          </Alert>
          ...
          <div className="card mb-3">
            ...
            <div className="card-body">
              <table className="table table-sm mb-0">
                <tbody>
                  <tr><th style={{ width: 180 }}>Gene</th><td>{overview?.gene} (TMEM138) — 11q12.2</td></tr>
                  <tr><th>OMIM (Gene)</th><td>*{overview?.omim_gene}</td></tr>
                  <tr><th>OMIM (Disease)</th><td>#{overview?.omim_disease} — JBTS16</td></tr>
          ...
          <div className="card mb-3">
            <div className="card-header fw-bold" style={{ background: ACCENT3 + '22' }}>Phenotype Frequency — JBTS16 Cohort (N={N_COHORT})</div>
            <div className="card-body">
              {[
      ...
      {tab === 1 && (
        <div>
          {/* Allele tiers */}
          <div className="card mb-3">
            <div className="card-header fw-bold" style={{ background: ACCENT + '22' }}>Allele Class → Clinical Tier — JBTS16</div>
            <div className="card-body p-0">
              <table className="table table-sm table-hover mb-0">
                <thead>
          ...
          {/* Key variants */}
          <div className="card mb-3">
            <div className="card-header fw-bold" style={{ background: ACCENT7 + '22' }}>Key Pathogenic Variants — TMEM138 / JBTS16</div>
            <div className="card-body p-0">
          ...
          <div className="card mb-3">
            <div className="card-header fw-bold" style={{ background: ACCENT6 + '22' }}>TMEM138 Domain → Phenotype Matrix</div>
            <div className="card-body p-0">
              <table className="table table-sm table-hover mb-0">
                <thead>
          ...
          {/* Pathway steps */}
          <div className="card mb-3">
            <div className="card-header fw-bold" style={{ background: ACCENT2 + '22' }}>TMEM138 → TZ Gate → Hedgehog Pathway — Step-by-Step</div>
            <div className="card-body">
              {breakdown?.pathway_steps?.map((step, i) => (
                <div key={i} className="d-flex mb-3">
                  ...
                  <div>
                    ...
                    <div className="small" style={{ color: ACCENT7 }}><strong>TMEM138 LOF:</strong> {step.effect_when_lost}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Management */}
          <div className="card mb-3">
            <div className="card-header fw-bold" style={{ background: ACCENT4 + '22' }}>Management Protocol — JBTS16 / TMEM138</div>
            <div className="card-body p-0">
              <table className="table table-sm table-hover mb-0">
                <thead>
      ...
      {/* ── TAB 2: TMEM216 CO-DEPENDENCY PEARLS ── */}
      {tab === 2 && (
        <div>
          <Alert color={ACCENT2}>
            <strong>&#x1f9ea; Mechanism Pearl — TMEM138 &amp; TMEM216 Mutual Stabilisation:</strong>{' '}
            {overview?.tmem138_function_pearl}
          </Alert>

          <Alert color={ACCENT10}>
            <strong>&#x1f9ec; TMEM216 Co-Dependency — Adjacent 11q12.2 Genes:</strong>{' '}
            {overview?.tmem216_codependency_pearl}
          </Alert>
          ...
          <div className="card mb-3">
            <div className="card-header fw-bold" style={{ background: ACCENT2 + '22' }}>
              JBTS16 vs Other No-MKS-Tier JBTS Types
            </div>
            <div className="card-body p-0">
              <table className="table table-sm table-hover mb-0">
                <thead>
      ...
      {tab === 3 && (
        <div>
            ...
            <div className="card-body">
              <table className="table table-sm mb-0">
                <tbody>
                  ...
                  <tr><th>OMIM JBTS16</th><td>#{definitions?.omim_jbts16}</td></tr>
          ...
          <div className="card mb-3">
            <div className="card-header fw-bold" style={{ background: ACCENT3 + '22' }}>Phenotype Frequencies — JBTS16</div>
            <div className="card-body p-0">
              <table className="table table-sm mb-0">
                <tbody>
          ...
          {/* Domain matrix */}
          <div className="card mb-3">
            <div className="card-header fw-bold" style={{ background: ACCENT6 + '22' }}>TMEM138 Domain Matrix (7 Domains)</div>
            <div className="card-body p-0">
              <div className="table-responsive">
                <table className="table table-sm table-hover mb-0">
                  <thead>
