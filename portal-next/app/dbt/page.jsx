...
const ACCENT  = '#1565c0';   // deep blue — DBT E2 transacylase / cubic core scaffold
...
const ACCENT4 = '#0277bd';   // steel blue — thiamine B1 / treatment (~5-10% response)
const ACCENT5 = '#6a1b9a';   // deep purple — alloisoleucine PATHOGNOMONIC / lipoamide mechanism
...
function OverviewTab() {
  const { data, err } = useFetch(`${API}/api/dbt/overview`);
  ...
  return (
    <div>
      <Alert
        text="🔑 PATHOGNOMONIC MARKER: Alloisoleucine >5 µmol/L in plasma — diagnostic ONLY in MSUD (BCKDHA/BCKDHB/DBT deficiency). DBT causes MSUD Type 2 — biochemically IDENTICAL to Type 1A (BCKDHA) and Type 1B (BCKDHB). ONLY gene panel (BCKDHA + BCKDHB + DBT + DLD) distinguishes MSUD subtypes."
        variant="info"
      />
      <Alert
        text="⚠️ LEUCINE NEUROTOXICITY — TREATMENT URGENCY: Plasma leucine >400 µmol/L → acute encephalopathy; >1000 µmol/L → cerebral oedema risk; >2000 µmol/L → life-threatening brainstem herniation. Classic neonatal DBT-MSUD presents day 3–7 with maple syrup odour + progressive encephalopathy. IV glucose (GIR ≥8 mg/kg/min) + insulin + BCAA-free formula + HD if leucine rising → IMMEDIATE ACTION REQUIRED."
        variant="danger"
      />
      <Alert
        text="✅ KEY NEGATIVES (confirm isolated BCKDH/E2 block): PDH activity NORMAL (DBT is E2 of BCKDH, not PDH; PDH uses DLAT as its E2) · αKGDH complex activity NORMAL (αKGDH uses DLST as its E2) · DLD free enzyme NORMAL (DLD protein intact in isolated DBT deficiency). If ANY of these reduced → suspect DLD/E3 deficiency (all four complexes use shared DLD/E3)."
        variant="success"
      />
      <Alert
        text={`💊 FIRST-LINE: (1) BCAA-restricted diet + BCAA-free synthetic formula (Level A — lifelong); (2) Thiamine B1 10–20 mg/kg/day trial ≥3 months ALL patients (Level A — ~5-10% thiamine-responsive in DBT: indirect mechanism via E1α stabilisation improving E1-E2 docking; intermediate between BCKDHA ~10-20% and BCKDHB ~5%). VPA ABSOLUTE CI (triple mechanism). Fasting EXTREME HAZARD. Liver transplant (Level B) corrects ~80% BCKDH activity.`}
        variant="warning"
      />
      ...
      <Section title="Gene Summary" color={ACCENT5}>
        <table className="table table-sm table-bordered small">
          <tbody>
            ...
            <tr><td className="fw-bold">Aliases</td><td>{data.aliases}</td></tr>
            <tr><td className="fw-bold">Locus</td><td>{data.locus} — Autosomal Recessive (chromosome 1p21.2)</td></tr>
            <tr><td className="fw-bold">Protein length</td><td>{data.aa_length} aa precursor (~400 aa mature after MTS cleavage); E2 CATALYTIC transacylase + 24-mer cubic core scaffold of BCKDH complex — lipoamide swinging arms on Lys82/Lys99</td></tr>
            ...
            <tr><td className="fw-bold" style={{color: ACCENT}}>Key distinguishing feature</td><td style={{color: ACCENT}}>{data.key_distinguishing_feature}</td></tr>
        ...
        <div className="table-responsive">
          <table className="table table-sm table-bordered small">
            <thead className="table-dark">
              <tr><th>Component</th><th>Gene(s)</th><th>Function in DBT deficiency context</th></tr>
            </thead>
            <tbody>
              {(data.bckdh_complex_components || []).map((comp, i) => (
                <tr key={i} style={comp.component.includes('THIS GENE') ? {background:'#e3f2fd'} : {}}>
                  <td className="fw-bold small">{comp.component}</td>
                  <td className="small fw-bold" style={{color: comp.component.includes('E1α') ? '#c62828' : comp.component.includes('E1β') ? '#6a1b9a' : comp.component.includes('E2') && comp.component.includes('THIS') ? ACCENT : comp.component.includes('E3') ? '#2e7d32' : '#546e7a'}}>
                    {comp.component.includes('E1α') ? 'BCKDHA' : comp.component.includes('E1β') ? 'BCKDHB' : comp.component.includes('E2') ? 'DBT' : comp.component.includes('E3') ? 'DLD' : 'BCKDK/PPM2C'}
                  </td>
      ...
      <Section title="BCAA Catabolism Pathway (DBT/E2 Block)" color={ACCENT}>
        {data.bcaa_catabolism_pathway && (
          <table className="table table-sm table-bordered small">
            <tbody>
              ...
              <tr style={{background:'#ffebee'}}><td className="fw-bold" style={{color:ACCENT2}}>Step 2 — IRREVERSIBLE — BLOCKED 🚫 (E2/DBT transacylation)</td><td style={{color:ACCENT2,fontWeight:'600'}}>{data.bcaa_catabolism_pathway.step_2_irreversible_blocked}</td></tr>
              <tr><td className="fw-bold small">Downstream Leu path (BLOCKED)</td><td className="small">{data.bcaa_catabolism_pathway.downstream_leu}</td></tr>
              <tr><td className="fw-bold small">Downstream Val path (BLOCKED)</td><td className="small">{data.bcaa_catabolism_pathway.downstream_val}</td></tr>
              <tr><td className="fw-bold small">Downstream Ile path (BLOCKED)</td><td className="small">{data.bcaa_catabolism_pathway.downstream_ile}</td></tr>
              <tr style={{background:'#f3e5f5'}}><td className="fw-bold" style={{color:ACCENT5}}>Alloisoleucine formation (PATHOGNOMONIC)</td><td style={{color:ACCENT5}}>{data.bcaa_catabolism_pathway.alloisoleucine_formation}</td></tr>
            </tbody>
          </table>
        )}
      </Section>

      <Section title="KPI Summary — 40-Patient DBT Cohort" color={ACCENT}>
...
function PatientsTab() {
  const { data, err } = useFetch(`${API}/api/dbt/breakdown`);
    ...
    <div>
      <Section title="Diagnostic Biomarkers" color={ACCENT5}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small">
            <thead className="table-dark">
              <tr><th>Biomarker</th><th>Normal range</th><th>DBT range</th><th>Clinical significance</th></tr>
            </thead>
            <tbody>
              {(data.biomarkers || []).map((b, i) => (
                <tr key={i} style={b.dbt_range && (b.dbt_range.includes('NORMAL') || b.dbt_range.includes('ABSENT')) ? {background:'#e8f5e9'} : b.dbt_range && b.dbt_range.includes('PATHOGNOMONIC') ? {background:'#f3e5f5'} : {}}>
                  <td className="fw-bold">{b.name}</td>
                  <td style={{color:'#388e3c'}}>{b.normal}</td>
                  <td style={{color: b.dbt_range && (b.dbt_range.includes('NORMAL') || b.dbt_range.includes('ABSENT')) ? '#388e3c' : b.dbt_range && b.dbt_range.includes('PATHOGNOMONIC') ? ACCENT5 : ACCENT2, fontWeight:'600'}}>{b.dbt_range}</td>
          ...
          <table className="table table-sm table-bordered small">
            ...
            <tbody>
              {(data.key_variants || []).map((v, i) => (
                <tr key={i}>
                  <td className="fw-bold" style={{color: ACCENT}}>{v.variant}</td>
                  <td>{v.effect}</td>
...
function SeizuresTab() {
  ...
  return (
    <div>
      <Alert
        text="⚡ Seizures in DBT deficiency are primarily METABOLIC — driven by acute leucine/KIC neurotoxicity, cerebral oedema, and myelinopathy (E2/DBT block identical to BCKDHA/B). Seizure control requires METABOLIC NORMALISATION first (leucine reduction via IV glucose + insulin + BCAA-free diet). AED monotherapy without BCAA control is ineffective. Burst suppression on neonatal EEG is common in classic neonatal DBT-MSUD crisis."
        variant="warning"
      />
      ...
      <Section title="Metabolic Crisis Triggers" color={ACCENT2}>
        <Alert text="ALL triggers cause BCAA surge via muscle protein catabolism → leucine crisis within hours. Metabolic emergency protocol must be pre-arranged for every DBT-MSUD patient (sick-day plan + emergency letter + nearest metabolic centre). IDENTICAL trigger profile to BCKDHA and BCKDHB." variant="danger" />
...
function TreatmentsTab() {
  ...
  return (
    <div>
      <Alert
        text="🔑 TREATMENT PRIORITY: (1) BCAA-restricted diet + BCAA-free synthetic formula — lifelong, non-negotiable (Level A). (2) Thiamine B1 high-dose trial ALL patients ≥3 months (Level A) — NOTE: DBT/E2 does NOT contain the TPP active site; thiamine works INDIRECTLY (~5-10% response); intermediate between BCKDHA ~10-20% and BCKDHB ~5%. (3) Acute crisis: IV glucose GIR 8-12 + insulin + BCAA-free AA infusion ± HD if leucine rising (Level A). (4) Carnitine supplementation (Level B). (5) Liver transplant — consider before age 10 (Level B)."
        variant="info"
      />
...
function DefinitionsTab() {
  const { data, err } = useFetch(`${API}/api/dbt/definitions`);
    ...
    <div>
      ...
      <Section title="Key Concepts" color={ACCENT5}>
        {(data.key_concepts || []).map((c, i) => (
          <div key={i} className="mb-3 p-2 rounded" style={{border:`1px solid ${ACCENT}`, background:'#e3f2fd'}}>
      ...
      <Section title="Differential Diagnosis" color={ACCENT7}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small">
            <thead className="table-dark">
              <tr><th style={{width:'30%'}}>Disease</th><th>How to distinguish from DBT deficiency</th></tr>
            </thead>
            <tbody>
    ...
    <div className="container-fluid py-3">
      <div className="mb-3">
        ...
        <div className="text-muted small">
          Maple Syrup Urine Disease Type 2 · Dihydrolipoamide Branched-Chain Transacylase Deficiency ·
          BCKDH complex E2 catalytic subunit (lipoamide-arm transacylase + 24-mer cubic core scaffold) ·
          Irreversible BCAA catabolism step 2b/2c BLOCKED (E2 transacylation) · Alloisoleucine PATHOGNOMONIC ·
          Leucine neurotoxicity (LAT1 / mTOR / myelinopathy) · Thiamine-responsive ~5-10% (indirect via E1α stabilisation) ·
          VPA ABSOLUTE CI (triple mechanism) ·
          Autosomal Recessive · 1p21.2 · OMIM *248610 / #248600 · ~1:185,000 globally (combined MSUD)
        </div>
      </div>
