...
const COLOR = '#4527a0';   // deep purple — dual-function GRIM-19 apoptosis + CI
const LIGHT = '#ede7f6';
  ...
  return (
    <>
      <div className="row g-3 mb-4">
        ...
        <KPI label="Also known as"  value="GRIM-19 / B16.6"   color={COLOR} />
      ...
      <SectionCard title="🔬 Biochemical Fingerprint">
        ...
        <Alert variant="success" text={`NDUFA13 is ISOLATED Complex I deficiency: CI ${data.cohort?.ci_activity_mean_pct ?? '~12'}% mean (range ${data.cohort?.ci_activity_range_pct ?? '5–20%'}), CII NORMAL, CIV NORMAL. Same biochemical fingerprint as other CI-Leigh nuclear subunit mutations; clinical distinguisher is the GRIM-19 dual-function (apoptosis regulator + STAT3 inhibitor) and N-module peripheral stabiliser BN-PAGE pattern (sub-assembly intermediates, not clean absent CI).`} />
      </SectionCard>
      ...
      <SectionCard title="🚫 KEY Differential Diagnosis Negatives">
        ...
        <Alert variant="warning" text="NDUFA13 N-module BN-PAGE: N-module sub-assembly intermediates (partial N-module visible — similar to NDUFA2/NDUFS5 pattern). CONTRAST: cleaner absent CI in NDUFS7/NDUFS8/NDUFB3/NDUFA11 (direct relay/membrane arm defects)." />
      </SectionCard>
    </>
  );
}
...
function PatientsTab({ data }) {
  ...
  return (
    <>
      <SectionCard title={`👥 Cohort Overview (n=${data.n}, seed-637)`}>
...
function TreatmentsTab({ overview }) {
  ...
  return (
    <>
      ...
      <SectionCard title="🔍 Key DDx (NDUFA13 is NEGATIVE for all below)">
        {ddx.map((d, i) => (
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function NDUFA13Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/ndufa13/overview`).then(r => r.json()),
      fetch(`${API}/api/ndufa13/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ndufa13/definitions`).then(r => r.json()),
    ])
    ...
    <div className="container-fluid py-4">
      <div className="mb-3">
        <h4 className="fw-bold" style={{ color: COLOR }}>
          🧬 NDUFA13 — Leigh Syndrome Isolated Complex I Deficiency
        </h4>
        <p className="text-muted small mb-0">
          GRIM-19 / B16.6 Subunit · N-Module Peripheral Stabiliser · Dual-Function Apoptosis Regulator ·
          19p13.11 · OMIM *609435 / #256000 · AR Biallelic · seed-637
        </p>
      </div>
