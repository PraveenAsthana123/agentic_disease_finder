...
const ACCENT  = '#b71c1c';   // dark-crimson-red — CLN6 / variant LINCL
const ACCENT2 = '#880e4f';   // deep-rose — ABSOLUTE CI / danger
const ACCENT3 = '#e65100';   // deep-orange — urgent alerts / warnings
const ACCENT4 = '#4a148c';   // deep-purple — safe treatments / monitoring
...
function OverviewTab({ ov }) {
  ...
  return (
    <div>
      <Alert
        variant="danger"
        text="⛔ ABSOLUTE CI: VGB (Vigabatrin) — CLN6 has 100% progressive retinal NCL; VGB retinopathy = CATASTROPHIC combined blindness. CBZ / Oxcarbazepine / Phenytoin / Fosphenytoin — ABSOLUTE CI: Na-channel blockers WORSEN myoclonus. PAEDIATRIC TRAP: child 1.5-8y with GTCS misidentified as idiopathic epilepsy → CBZ prescribed → ACUTE MYOCLONIC DETERIORATION. IV SE: LEV 60 mg/kg (NOT fosphenytoin). Tiagabine (TGB) — ABSOLUTE CI: NCSE risk in NCL/PME."
      />
      <Alert
        variant="warning"
        text="⚠ HIGH RISK: GBP/Pregabalin (worsens myoclonus — multi-specialty prescribing trap). LTG monotherapy (insufficient myoclonus cover — use only with VPA backbone). AED taper: NEVER — CLN6 is progressive fatal NCL. Photosensitivity in 65% — screen all at diagnosis. NCSE risk in advanced disease — urgent EEG for any acute cognitive change."
      />
      <Alert
        variant="info"
        text="🧠 CLN6 UNIQUE FEATURES: NO CLN6 ENZYME ASSAY → WES/gene panel required (no rapid DBS enzyme test). FP + CB on EM = CLN6 signature (DISTINCT from CLN5 rectilinear profiles). SEIZURES FIRST (not visual first like CLN3). DUAL INHERITANCE: AR variant LINCL (paediatric) + rare AD Kufs Type A (adult). Portuguese founder p.Arg24Gly; Turkish/Roma/South Asian p.Glu336Lys. South Hampshire sheep = best-characterised large animal NCL model (AAV-CLN6 gene therapy preclinical). VPA IS SAFE (ER dysfunction, NOT mitochondrial)."
      />
      <Alert
        variant="success"
        text="✅ DIAGNOSIS STEPS: (1) EM skin biopsy → FP + CB pattern (NCL confirmed, CLN6 signature — NOT RP which is CLN5, days). (2) Portuguese/Czech heritage → p.Arg24Gly PCR (2-3 days); Turkish/Roma/South Asian → p.Glu336Lys PCR. Others → WES/NCL gene panel (weeks). (3) Ophthalmology ERG + VEP. (4) POLG1 exclusion before VPA (children <8y). (5) BDSRA registry enrolment (gene therapy trial eligibility). (6) ACP from diagnosis. Sequence: EM concurrent with gene panel — no CLN6 enzyme assay shortcut."
      />
      ...
      <div className="row mb-4">
        ...
        <KPI label="Photosensitive" value={`${ov.portuguese_heritage_pct + ov.turkish_roma_heritage_pct}%`} color={ACCENT4} />
        <KPI label="On VPA" value={`${ov.on_vpa_pct}%`} color={ACCENT4} />
        <KPI label="AD Kufs A" value={`${ov.ad_kufs_type_a_pct}%`} color={ACCENT4} />
      </div>
      ...
      <SectionCard title="⚠ No CLN6 Enzyme Assay — WES / Gene Panel Required" borderColor={ACCENT3}>
        <p className="small mb-0">{ov.no_enzyme_assay}</p>
      </SectionCard>

      <SectionCard title="No Disease-Modifying Therapy (South Hampshire Sheep Gene Therapy Model)" borderColor={ACCENT2}>
...
function PatientsTab({ bk }) {
  ...
  return (
    <div>
      <SectionCard title="6-Class Etiology (40-patient CLN6 Cohort)">
      ...
      {bk.lifecycle_stages && (
        <SectionCard title="6 Lifecycle Stages (CLN6 Disease Trajectory)" borderColor={ACCENT4}>
        ...
        {(bk.treatments || []).map((tx, i) => (
          <div key={i} className="mb-4 p-3 border rounded">
            ...
            <div className="alert alert-primary py-1 mb-0" style={{ fontSize: 12 }}>
              <strong>CLN6 Note:</strong> {tx.cln6_note}
            </div>
          </div>
        ))}
      </SectionCard>
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function CLN6Page() {
  ...
  useEffect(() => {
    fetch(`${API}/api/cln6/overview`).then(r => r.json()).then(setOv).catch(e => setErr(String(e)));
  }, []);

  useEffect(() => {
    if (tab === 1 || tab === 2 || tab === 3) {
      if (!bk) fetch(`${API}/api/cln6/breakdown`).then(r => r.json()).then(setBk).catch(e => setErr(String(e)));
    }
    if (tab === 4) {
      if (!df) fetch(`${API}/api/cln6/definitions`).then(r => r.json()).then(setDf).catch(e => setErr(String(e)));
    }
  }, [tab, bk, df]);
      ...
      <div className="d-flex align-items-center mb-3 gap-3">
        ...
        <div>
          <h4 className="mb-0 fw-bold" style={{ color: ACCENT }}>
            CLN6 Epilepsy — Variant Late-Infantile NCL (vLINCL)
          </h4>
          <div className="small text-muted">
            CLN6 (15q23) · ER Transmembrane Protein · FP + CB on EM · Seizures First ·
            Portuguese p.Arg24Gly Founder · Turkish/Roma p.Glu336Lys · No CLN6 Enzyme Assay ·
            Dual AR/AD Inheritance · South Hampshire Sheep Gene Therapy Model · VGB ABSOLUTE CI · VPA Safe · 40-pt Cohort
          </div>
        </div>
      </div>
