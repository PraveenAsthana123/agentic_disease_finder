...
const COLOR  = '#1b5e20';   // deep green — CIII early assembly / milder childhood onset
const LIGHT  = '#e8f5e9';
const COLOR2 = '#2e7d32';
const COLOR3 = '#e65100';   // amber-orange — severity / alert highlight
  ...
  return (
    <div>
      ...
      <div className="row mb-4">
        ...
        <KPI label="Early childhood onset" value={`${s.early_childhood_onset_pct}%`} />
        ...
        <KPI label="Deceased" value={`${s.deceased_pct}%`} color={COLOR4} />
      </div>
          ...
          <SectionCard title="Clinical Features (% of cohort)">
            {feats.map((f, i) => (
              <Bar key={i} label={f.feature} value={f.pct}
                color={f.pct === 0 ? '#9e9e9e' : (f.pct < 35 ? COLOR3 : COLOR)} />
            ))}
          </SectionCard>
        </div>
        <div className="col-md-6">
          <SectionCard title="Top Variants (allele frequency)">
            {(data.top_variant_counts || []).map((v, i) => (
              <div key={i} className="d-flex justify-content-between small py-1 border-bottom">
                <code>{v.variant}</code><span className="badge" style={{ background: COLOR }}>{v.count} alleles</span>
              </div>
            ))}
          </SectionCard>
          <SectionCard title="🔑 Milder than UQCC2 — Key Distinguishing Facts" borderColor={COLOR2}>
            <ul className="small mb-0 ps-3">
              <li><strong>Onset:</strong> Early childhood (6 months–2 years); NOT neonatal</li>
              <li><strong>Residual CIII:</strong> 10–30% (vs UQCC2's 5–18%) → milder</li>
              <li><strong>BN-PAGE:</strong> CIII reduced; some sub-complexes persist</li>
              <li><strong>Survival:</strong> Better — many survive to adulthood</li>
              <li><strong>Lactic acidosis:</strong> Moderate (3.5–10 mM) vs severe neonatal in UQCC2</li>
              <li><strong>Chromosome:</strong> 11q12.3 (vs UQCC2 6p21.2) — WES mandatory</li>
            </ul>
          </SectionCard>
        </div>
      </div>
            ...
            <tbody>
              {(data.patients || []).map((p, i) => (
                <tr key={i}>
                  ...
                  <td><span style={{ color: p.ciii_activity_pct < 15 ? COLOR3 : COLOR }}>{p.ciii_activity_pct}%</span></td>
                  <td>{p.lactic_acid_mmolL}</td>
                  <td><span className="badge" style={{ background: p.outcome.includes('Deceased') ? '#c62828' : COLOR, fontSize: '0.7em' }}>{p.outcome.substring(0,25)}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function VariantsTab({ data }) {
  ...
  return (
    <div>
      <SectionCard title="Pathogenic Variants in UQCC3">
        <div className="table-responsive">
            ...
            <tbody>
              {(data.all_variants || []).map((v, i) => (
                <tr key={i}>
                  ...
                  <td>{v.domain}</td>
                  <td>{v.type}</td>
                  <td><span className="badge" style={{ background: v.severity.includes('Severe') ? COLOR3 : COLOR }}>{v.severity}</span></td>
      ...
      <div className="row">
        <div className="col-md-6">
          <SectionCard title="Biochemistry Distribution">
            ...
            <Bar label="CIII activity <15%" value={bio.ciii_below_15_pct || 0} color={COLOR3} />
            <Bar label="CIII activity 15-25%" value={bio.ciii_15to25_pct || 0} />
            <Bar label="CIII activity >25%" value={bio.ciii_above_25_pct || 0} color={COLOR2} />
            <hr />
            <Bar label="Lactate >8 mM" value={bio.lactic_above_8_pct || 0} color={COLOR3} />
            <Bar label="Lactate 4-8 mM" value={bio.lactic_4_to_8_pct || 0} color={COLOR} />
          </SectionCard>
        </div>
        <div className="col-md-6">
        ...
        <div className="col-md-6">
          <SectionCard title="Outcome Distribution">
            {(data.outcome_distribution || []).map((o, i) => (
              <div key={i} className="d-flex justify-content-between small py-1 border-bottom">
                <span>{o.outcome}</span>
                <span className="badge" style={{ background: o.outcome.includes('Deceased') ? '#c62828' : COLOR }}>{o.count}</span>
              </div>
            ))}
          </SectionCard>
        </div>
  ...
  return (
    <div>
      <SectionCard title="Differential Diagnosis" borderColor={COLOR4}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered mb-0">
            <thead><tr><th>Condition</th><th>Distinguishing from UQCC3</th></tr></thead>
            <tbody>
              {ddx.map((d, i) => (
                <tr key={i}>
      ...
      <SectionCard title="CIII Assembly Cascade — UQCC3 Position" borderColor={COLOR2}>
        <div className="row text-center small">
          {[
            { step: '1', label: 'MT-CYB synthesis', detail: 'Mitochondrial translation', color: '#616161' },
            { step: '2', label: 'UQCC1-UQCC2 bind', detail: 'Earliest nuclear assembly (UQCC2 6p21.2)', color: '#1a237e' },
            { step: '3', label: 'UQCC3 associates', detail: 'Cooperates at MT-CYB scaffold (UQCC3 11q12.3)', color: COLOR },
            { step: '4', label: 'Early CIII core forms', detail: 'UQCRC1, UQCRC2 join', color: COLOR2 },
            { step: '5', label: 'TTC19 stabilises', detail: 'Later intermediate (17p12)', color: '#4a148c' },
            { step: '6', label: 'BCS1L inserts RISP', detail: 'Rate-limiting; mature CIII (2q35)', color: '#b71c1c' },
          ].map((s, i) => (
          ...
          <SectionCard title="Protein Details">
            <table className="table table-sm mb-0">
              <tbody>
                ...
                <tr><td><strong>TM helices</strong></td><td>{p.tm_helices} TM helix (IMM-anchored)</td></tr>
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function UQCC3Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/uqcc3/overview`).then(r => r.json()),
      fetch(`${API}/api/uqcc3/breakdown`).then(r => r.json()),
      fetch(`${API}/api/uqcc3/definitions`).then(r => r.json()),
    ...
    <div className="container-fluid py-4">
      <div className="mb-3" style={{ borderBottom: `3px solid ${COLOR}` }}>
        <h4 className="fw-bold" style={{ color: COLOR }}>
          🧬 UQCC3 — Complex III Deficiency, Nuclear Type 8 (CIII-D8)
        </h4>
        <p className="text-muted mb-2 small">
          OMIM Gene *616097 · Disease #615838 · AR biallelic · 11q12.3 · 40-patient cohort (seed 719)
        </p>
