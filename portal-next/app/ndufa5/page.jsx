...
const COLOR = '#1a237e';   // deep indigo — AR / Q-module matrix arm / beta-sandwich / no TM helix
const LIGHT = '#e8eaf6';
    ...
    <div>
      <SectionCard title="Gene & Disease Identity">
        <div className="row g-2 small">
          {[
            ['Gene', data.gene],
            ['Protein', 'NADH:Ubiquinone Oxidoreductase Subunit A5 (B13)'],
            ['Module', data.module],
...
function PatientsTab({ data }) {
  ...
  return (
    <div>
      ...
      <SectionCard title="Patient Cohort (40 patients, seed 657)">
...
// ── Root component ────────────────────────────────────────────────────────────
export default function NDUFA5Page() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/ndufa5/overview`).then(r => r.json()),
      fetch(`${API}/api/ndufa5/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ndufa5/definitions`).then(r => r.json()),
    ])
  ...
  return (
    <div className="container-fluid py-3">
      <div className="mb-3 p-3 rounded" style={{ background: COLOR, color: '#fff' }}>
        <h4 className="mb-1 fw-bold">🧬 NDUFA5 — Leigh Syndrome / Isolated Complex I Deficiency</h4>
        <div className="small opacity-75">
          B13 · Q-module N/Q Interface · Matrix Arm Beta-Sandwich · No TM Helix · AR · 7q32.1 ·
          OMIM Gene *603835 · Disease #256000
        </div>
      </div>

      {loading && <div className="alert alert-info">Loading NDUFA5 dashboard…</div>}
