...
// Congenital Myopathy Atlas color palette — neuromuscular / structural
const COLOR  = '#1b5e20';  // deep green — muscle/structural
const LIGHT  = '#e8f5e9';  // light green tint
const COLOR2 = '#b71c1c';  // red — fatal/MH/lethal
const COLOR3 = '#1565c0';  // blue — targeted/trial therapy
const COLOR4 = '#e65100';  // orange — warning / MH risk
const COLOR5 = '#4a148c';  // dark purple — X-linked / severe neonatal
const COLOR6 = '#37474f';  // blue-grey — structural
const COLOR7 = '#880e4f';  // dark magenta — critical error / drug avoid

const GENE_COLORS = {
  NEB:     '#2e7d32',  // green — nemaline rods / sarcomeric
  RYR1:    '#b71c1c',  // red — MH risk ABSOLUTELY CI volatile agents
  ACTA1:   '#1565c0',  // blue — nemaline myopathy 3 / de novo
  TPM2:    '#6a1b9a',  // purple — trismus / distal arthrogryposis
  TPM3:    '#00695c',  // teal — type 1 fibre / mild NEM1
  MTM1:    '#4a148c',  // dark purple — XLMTM / neonatal ventilator
  DNM2:    '#e65100',  // orange — centronuclear / necklace fibres
  SELENON: '#37474f',  // blue-grey — rigid spine / respiratory
};
...
function Loading() {
  return (
    <div className="text-center py-5">
      ...
      <div className="mt-2 text-muted small">Loading Congenital Myopathy Atlas…</div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      {(ov.drug_alerts || []).map((a, i) => (
        <AlertBox key={i}
          type={
            a.includes('ABSOLUTELY') || a.includes('MANDATORY') || a.includes('CONTRAINDICATED') ||
            a.includes('EMERGENCY') || a.includes('LETHAL') || a.includes('AVOID')
              ? 'danger'
            ...
            <div className="card-body py-2 px-3">
              {Object.entries(cf).map(([k, v], i) => (
                <BarRow key={i} label={k} pct={v} color={
                  k.includes('Malignant') ? COLOR2
                  : k.includes('Ventilator') ? COLOR5
                  : k.includes('Error') ? COLOR7
                  : k.includes('Targeted') ? COLOR3
                  : k.includes('Hepatopathy') ? COLOR4
                  : COLOR
                } />
              ))}
            </div>
          ...
          {genes.map(g => {
            ...
            return (
              <tr key={g}>
                ...
                <td><span className={d.ventilator_dependent_pct > 20 ? 'text-danger fw-semibold' : 'text-muted'}>{d.ventilator_dependent_pct}%</span></td>
                <td><span className={d.mh_risk_pct > 0 ? 'text-danger fw-semibold' : 'text-muted'}>{d.mh_risk_pct}%</span></td>
                <td><span className={d.drug_error_pct > 0 ? 'text-danger' : 'text-muted'}>{d.drug_error_pct}%</span></td>
                <td style={{ maxWidth: 160 }} className="text-truncate">{d.first_line_drug}</td>
              </tr>
            );
          })}
        </tbody>
          ...
          <div className="card-body py-3 px-3">
            ...
            <div className="row g-2 mb-3">
              {[
                ...
                { label: 'Vent-Dep', value: `${d.ventilator_dependent_pct}%` },
                { label: 'Drug-Err', value: `${d.drug_error_pct}%` },
              ].map((m, i) => (
            ...
            {/* MH risk badge */}
            {d.mh_risk_pct > 0 && (
              <AlertBox type="danger" title="Malignant Hyperthermia Risk">
                MH risk in {d.mh_risk_pct}% of this gene cohort.
                VOLATILE ANAESTHETICS + SUCCINYLCHOLINE ABSOLUTELY CONTRAINDICATED. Dantrolene emergency antidote.
              </AlertBox>
            )}
            ...
            <div className="mb-3">
              ...
              {[
                { label: 'Disease Progression', pct: d.progression_pct, color: COLOR2 },
                { label: 'Ventilator Dependent', pct: d.ventilator_dependent_pct, color: COLOR5 },
                { label: 'MH Risk', pct: d.mh_risk_pct, color: COLOR2 },
                { label: 'Hepatopathy', pct: d.hepatopathy_pct, color: COLOR4 },
                { label: 'Cognitive Impairment', pct: d.cognitive_impairment_pct, color: COLOR6 },
                { label: 'Drug-Prescribing Error', pct: d.drug_error_pct, color: COLOR7 },
                { label: 'On Targeted Therapy', pct: d.on_targeted_therapy_pct, color: COLOR3 },
...
// ── Main Page ──────────────────────────────────────────────────────────
export default function CongenitalMyopathyAtlasPage() {
  ...
  useEffect(() => {
    const base = `${API}/api/congenital-myopathy-atlas`;
    Promise.all([
  ...
  return (
    <div className="container-fluid py-3">
      ...
      <div className="d-flex align-items-center gap-3 mb-3">
        <div>
          <h4 className="fw-bold mb-0" style={{ color: COLOR }}>💪 Congenital Myopathy Atlas</h4>
          <p className="text-muted small mb-0">
            Complete 8-Gene Congenital Myopathy Atlas — NEB · RYR1 · ACTA1 · TPM2 · TPM3 · MTM1 · DNM2 · SELENON
          </p>
          <p className="text-muted small mb-0">
            320 patients (8×40, seeds 1086–1093) · NEM2 · CCD·MHS1 · NEM3 · NEM4·DA1 · NEM1 · XLMTM · CNM1 · RSMD1
          </p>
        </div>
      </div>
