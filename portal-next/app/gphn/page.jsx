...
const COLOR = '#5d4037';    // deep brown — distinct from GLRA1 teal, GLRB dark-teal, SLC6A5 green
const LIGHT = '#efebe9';
  ...
  return (
    <div>
      <div className="alert alert-warning py-2 small mb-3" style={{ borderLeft: `4px solid ${COLOR}` }}>
        <strong>GPHN (14q23.3) — Gephyrin · OMIM 603930 · 4th of 5-gene Hyperekplexia Panel:</strong>{' '}
        Gephyrin forms the <strong>hexagonal inhibitory postsynaptic density (iPSD) scaffold</strong> that anchors
        BOTH GlyR (glycine receptors) AND GABA<sub>A</sub>R simultaneously.{' '}
        GPHN LOF → dual inhibitory failure → hyperekplexia + epilepsy + intellectual disability.{' '}
        <strong>RAREST of 5-gene panel (~1-2%)</strong>; the ONLY gene where both GlyR and GABA<sub>A</sub>R
        anchoring fail together.{' '}
        <span className="text-danger fw-bold">
          EEG MANDATORY before ASM (unlike pure GLRA1/GLRB/SLC6A5, GPHN-DEE has abnormal EEG).
          5-gene panel + CNV 14q23.3 simultaneously. POLG before VPA. Vigevano Manoeuvre training mandatory.
        </span>
      </div>
      ...
      <div className="row g-2 mb-4">
        ...
        <KPI label="MLPA 14q23" value={`${kpis.mlpa_14q23_pct}%`} color="#0d6efd" />
      ...
      <div className="card shadow-sm">
        ...
        <div className="table-responsive">
          <table className="table table-sm table-hover small mb-0">
            <thead>
              <tr>
                <th>ID</th><th>Sex</th><th>Age</th><th>Onset (mo)</th><th>Category</th>
                <th>Apnoea</th><th>Rigid</th><th>Epileptic</th><th>ID</th>
                <th>CLZ</th><th>ASM</th><th>Trained</th><th>Nose+</th><th>EEG Abn</th>
              </tr>
            </thead>
            <tbody>
              {patients.map((p, i) => (
                <tr key={i}>
                  ...
                  <td>{p.sex}</td>
                  <td>{p.age}y</td>
                  <td>{p.onset_age_mos}mo</td>
                  <td><span className="badge" style={{ background: COLOR, fontSize: '0.65rem' }}>
                    {p.category.replace('GPHN-', '').replace(/-/g, ' ').substring(0, 22)}
                  </span></td>
...
export default function GPHNPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/gphn/overview`).then(r => r.json()),
      fetch(`${API}/api/gphn/breakdown`).then(r => r.json()),
      fetch(`${API}/api/gphn/definitions`).then(r => r.json()),
    ]).then(([ov, bk, df]) => {
      setOverview(ov);
      setBreakdown(bk);
      setDefinitions(df);
      ...
      <div className="d-flex align-items-center mb-3 gap-3">
        <div>
          <h4 className="mb-0 fw-bold" style={{ color: COLOR }}>
            🧬 GPHN Hyperekplexia / DEE (Gephyrin iPSD Scaffold)
          </h4>
          <div className="text-muted small">
            Gephyrin / 14q23.3 · OMIM Gene 603930 · 736aa · G-domain (1-181) + E-domain (319-736) ·
            AR biallelic LOF + AD de novo + 14q23.3 CNV · Rarest of 5-gene panel (~1-2%) ·
            Dual GlyR + GABA<sub>A</sub>R anchoring failure · 40-patient cohort seed-501
          </div>
        </div>
      </div>
