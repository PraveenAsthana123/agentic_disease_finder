...
const COLOR  = '#006064';   // deep teal — tRNA-His / CI+CIV Combined / Leigh-like
const LIGHT  = '#e0f7fa';
const COLOR2 = '#00838f';   // teal — CPEO / ophthalmoplegia
const COLOR3 = '#b71c1c';   // dark red — CI (absolute CIs)
const COLOR4 = '#1565c0';   // deep blue — CI+CIV biochemical fingerprint
const COLOR5 = '#2e7d32';   // green — treatment / positive actions
    ...
    <div>
      ...
      <div className="p-3 mb-4 rounded" style={{ background: LIGHT, borderLeft: `5px solid ${COLOR}` }}>
        <div className="d-flex flex-wrap align-items-start gap-3">
          <div>
            <h4 className="fw-bold mb-1" style={{ color: COLOR }}>MT-TH — tRNA-His</h4>
            <div className="text-muted small">
              <span className="badge me-1" style={{ background: COLOR }}>OMIM *590080</span>
              <span className="badge me-1" style={{ background: COLOR2 }}>Combined CI+CIV Deficiency</span>
              <span className="badge me-1 bg-dark">Leigh-like Syndrome</span>
              <span className="badge me-1" style={{ background: COLOR3 }}>CPEO / Cardiomyopathy</span>
            </div>
            <p className="mt-2 mb-0 small">
              MT-TH encodes mitochondrial tRNA-His (GUG anticodon, 69 nt) — H-strand rCRS 12138–12206 — between MT-ND4 and MT-TS2.
              Mutations cause <strong>combined CI + CIV deficiency</strong> (mt-translation fingerprint: CII NORMAL).
              Key features: Leigh-like MRI, CPEO, cardiomyopathy, SNHL, lactic acidosis.
              <strong className="text-danger"> NO stroke-like episodes</strong> (vs MT-TL1) ·
              <strong className="text-danger"> NO MSL</strong> (vs MT-TK).
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="row g-2 mb-4">
        ...
        <KPI label="CII (NORMAL)" value={`${s.avg_cii_activity_pct_normal}%`} color={COLOR5} />
        <KPI label="Leigh-like MRI" value={`${s.pct_leigh_mri}%`} color={COLOR4} />
      ...
      <div className="row g-3 mb-4">
        <div className="col-md-6">
          <SectionCard title="🔬 OXPHOS Biochemical Fingerprint (mt-Translation)" borderColor={COLOR4}>
            <div className="p-2 rounded mb-2" style={{ background: '#e3f2fd' }}>
            ...
            <Bar label="CII Activity (% normal — NORMAL)" value={Math.min(s.avg_cii_activity_pct_normal, 100)} color={COLOR5} />
            <div className="alert alert-info mt-2 py-2 small">
              BN-PAGE: CI + CIV bands absent/reduced · CII band present · Muscle histology: RRF possible · COX-negative fibres at high heteroplasmy
            </div>
          </SectionCard>
        </div>
        <div className="col-md-6">
          <SectionCard title="📊 Heteroplasmy Threshold Map (blood; muscle ~10-20% higher)" borderColor={COLOR2}>
            {hmap.map((row, i) => (
      ...
      <SectionCard title="📈 Cohort Phenotype Distribution (n=40, seed-791)" borderColor={COLOR}>
        <div className="row">
          {pheno_dist.map((ph, i) => (
...
function VariantsTab({ data }) {
  ...
  return (
    <div>
          ...
          <table className="table table-sm table-hover">
            <thead style={{ background: LIGHT }}>
              <tr>
                ...
                <th>CI%</th><th>CIV%</th><th>Leigh%</th><th>CPEO%</th><th>Cardio%</th><th>SNHL%</th>
              </tr>
            </thead>
            <tbody>
              {variants.map((v, i) => (
      ...
      {/* Per-patient table */}
      <SectionCard title="🗂️ Per-Patient Data (40 patients, seed-791)" borderColor={COLOR}>
          ...
          <table className="table table-sm table-striped">
            <thead style={{ background: LIGHT, position: 'sticky', top: 0 }}>
              <tr>
                ...
                <th>Leigh</th><th>CPEO</th><th>Cardio</th><th>KSS</th><th>SNHL</th>
              </tr>
            </thead>
            <tbody>
              {per_patient.map((p, i) => (
                <tr key={i}>
                  ...
                  <td style={{ color: COLOR5 }}>{p.cii_pct}%</td>
  ...
  return (
    <div>
      ...
      <SectionCard title="💊 Evidence-Based Management" borderColor={COLOR5}>
        {tx.map((t, i) => (
          <div key={i} className="mb-2 p-2 rounded" style={{ background: i % 2 === 0 ? '#e8f5e9' : '#fff' }}>
            <div className="d-flex align-items-center gap-2">
  ...
  return (
    <div>
      ...
      <SectionCard title="💊 Pharmacology" borderColor={COLOR4}>
        ...
        {pharm.cardiac_monitoring && (
          <div className="mb-2 small"><strong style={{ color: COLOR2 }}>Cardiac Monitoring:</strong> {pharm.cardiac_monitoring}</div>
        )}
      </SectionCard>
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function MtthPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/mtth/overview`).then(r => r.json()),
      fetch(`${API}/api/mtth/breakdown`).then(r => r.json()),
      fetch(`${API}/api/mtth/definitions`).then(r => r.json()),
    ])
  ...
  return (
    <div className="container-fluid py-3">
      <div className="d-flex align-items-center gap-2 mb-3">
        ...
        <div>
          <h3 className="mb-0 fw-bold" style={{ color: COLOR }}>MT-TH — tRNA-His</h3>
          <div className="text-muted small">
            Combined CI+CIV Deficiency · Leigh-like Syndrome · CPEO · Cardiomyopathy ·
            m.12147G&gt;A · H-strand rCRS 12138–12206 · Maternal Inheritance
          </div>
        </div>
      </div>
