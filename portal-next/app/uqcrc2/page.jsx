...
const COLOR  = '#1565c0';   // deep blue — UQCRC2 / core scaffold protein 2
const LIGHT  = '#e3f2fd';
const COLOR2 = '#0277bd';
    ...
    <div>
      ...
      <div className="row">
        ...
        <div className="col-md-6">
          ...
          <SectionCard title="🔑 UQCRC2 — Critical DDx vs CYC1 / LYRM7 / BCS1L" borderColor={COLOR4}>
            <ul className="small mb-0 ps-3">
              <li><strong>vs CYC1:</strong> BN-PAGE/immunoblot <span style={{ color: COLOR3 }}>IDENTICAL</span> — WES mandatory; <span style={{ color: COLOR }}>No cataracts in UQCRC2</span> (cataracts 35% in CYC1) — KEY bedside clue</li>
              <li><strong>UQCRC1 (Core1):</strong> <span style={{ color: COLOR3 }}>ABSENT</span> in UQCRC2 deficiency · <span style={{ color: COLOR }}>PRESERVED</span> in LYRM7/BCS1L — definitive distinguisher</li>
              <li><strong>BN-PAGE CIII:</strong> Absent (UQCRC2) · Reduced (LYRM7) · Precomplex (BCS1L)</li>
              <li><strong>Cardiomyopathy:</strong> ~25% UQCRC2 · ~18% CYC1 — higher in UQCRC2</li>
              <li><strong>Mechanism:</strong> UQCRC2 = core scaffold (holocomplex loss) vs LYRM7/BCS1L = chaperone (core preserved)</li>
            </ul>
          </SectionCard>
        </div>
      </div>
...
function VariantsTab({ data }) {
  ...
  return (
    <div>
      <SectionCard title="Pathogenic Variants in UQCRC2">
        <div className="table-responsive">
  ...
  return (
    <div>
      <SectionCard title="Differential Diagnosis" borderColor={COLOR4}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered mb-0">
            <thead><tr><th>Condition</th><th>Distinguishing from UQCRC2</th></tr></thead>
            <tbody>
              {ddx.map((d, i) => (
                <tr key={i}>
      ...
      <SectionCard title="CIII Core Scaffold — UQCRC2 Position" borderColor={COLOR2}>
        <div className="row text-center small">
          {[
            ...
            { step: '3', label: 'UQCRC1 + UQCRC2', detail: 'Core scaffold heterodimer (16p12.2 / 3p21.3)', color: COLOR },
        ...
        <div className="alert mt-3 mb-0 small" style={{ background: LIGHT, borderLeft: `4px solid ${COLOR3}` }}>
          <strong>UQCRC2 block effect:</strong> UQCRC2 absent → UQCRC1 (Core1) secondarily absent →
          ENTIRE CIII holocomplex destabilised → ALL subunits (UQCRC1, RISP, CYC1, MT-CYB) absent on immunoblot →
          BN-PAGE: CIII2 and supercomplexes completely absent.
          Indistinguishable from CYC1 by biochemistry — WES mandatory for gene-level diagnosis.
          No cataracts in UQCRC2 (cataracts present in CYC1 ~35%) — KEY bedside DDx clue.
        </div>
      </SectionCard>
    </div>
  );
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function UQCRC2Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/uqcrc2/overview`).then(r => r.json()),
      fetch(`${API}/api/uqcrc2/breakdown`).then(r => r.json()),
      fetch(`${API}/api/uqcrc2/definitions`).then(r => r.json()),
    ...
    <div className="container-fluid py-4">
      <div className="mb-3" style={{ borderBottom: `3px solid ${COLOR}` }}>
        <h4 className="fw-bold" style={{ color: COLOR }}>
          🧬 UQCRC2 — Complex III Deficiency, Nuclear Type 4 (CIII-D4)
        </h4>
        <p className="text-muted mb-2 small">
          OMIM Gene *191329 · Disease #615159 · AR biallelic · 16p12.2 · 40-patient cohort (seed 727)
        </p>
