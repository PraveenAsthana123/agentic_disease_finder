...
const ACCENT  = '#1565c0';   // deep blue — MPS IIIC / acetyl-CoA acetyltransferase
const ACCENT2 = '#c62828';   // dark red — HIGH RISK / contraindications
const ACCENT3 = '#e65100';   // deep orange — CAUTION / relative CI
const ACCENT4 = '#6a1b9a';   // deep purple — gene therapy / investigational
const ACCENT5 = '#4a148c';   // indigo — melatonin / sleep-seizure nexus
const ACCENT6 = '#2e7d32';   // green — SAFE alternatives / LEV

const ETIOLOGY_COLORS = {
  'Compound Heterozygous — Null + Missense': '#1565c0',
  'French-Canadian Founder — p.Arg344Cys (Homozygous / Compound Het)': '#0277bd',
  'Dutch Founder — p.Trp649stop (Homozygous / Compound Het)': '#c62828',
  'Italian Founder — p.Pro524Leu (Compound Het or Hom)': '#e65100',
  'Attenuated — Biallelic Missense (Residual Activity)': '#6a1b9a',
};
  ...
  return (
    <>
      <div className="alert mb-4 text-white fw-bold" style={{ background: ACCENT }}>
        🧬 HGSNAT / MPS IIIC — Sanfilippo Syndrome C (Heparan Alpha-Glucosaminide N-Acetyltransferase Deficiency)
      ...
      <div className="alert border-2 mb-4" style={{ borderColor: ACCENT2, background: '#ffebee' }}>
        ...
        <div className="mt-1 small">
          MPS IIIC has NO approved ERT — HGSNAT requires cytosolic acetyl-CoA import into lysosomes,
          a reaction ERT cannot replicate. HSCT is NOT recommended in established disease (same as
          MPS IIIA/IIIB). Refer eligible patients to <strong>AAV9-HGSNAT intrathecal gene therapy
          Phase I trial</strong>. MPS IIIC progresses SLOWER than IIIA/IIIB — survival into
          3rd–4th decade is possible.
        </div>
      </div>

      {/* MPS IIIC vs IIIA/IIIB distinguishing box */}
      {data.distinguishing_from_mps_iiia_iiib && (
        <div className="alert border-2 mb-4" style={{ borderColor: ACCENT, background: '#e3f2fd' }}>
          <strong style={{ color: ACCENT }}>🔬 DISTINGUISHING MPS IIIC FROM IIIA/IIIB:</strong>
          <div className="mt-1 small">{data.distinguishing_from_mps_iiia_iiib}</div>
        </div>
      )}
      ...
      <div className="alert border-2 mb-4" style={{ borderColor: ACCENT5, background: '#f3e5f5' }}>
        ...
        <div className="mt-1 small">
          Sleep disorder (85–90%) is the cardinal earliest symptom in MPS IIIC — hypothalamic SCN
          HS accumulation disrupts circadian rhythm. Melatonin 5–10 mg nocte is strongly indicated
          from behavioral phase onset. Sleep-triggered seizure clusters are the primary seizure
          precipitant. Initiate melatonin BEFORE AEDs in new behavioral phase presentations.
        </div>
      </div>
      ...
      <div className="row g-2 mb-4">
        ...
        <KPI label="Sleep disorder" value="85–90%" color={ACCENT5} />
        <KPI label="Gene therapy" value="Phase I" color={ACCENT4} />
          ...
          <div className="row">
            {data.lifecycle.map((l, i) => (
              <div key={i} className="col-md-6 mb-3">
                <div className="card h-100 border-0" style={{ background: '#e3f2fd' }}>
                  <div className="card-body small">
  ...
  return (
    <>
      ...
      <SectionCard title="Genotype Distribution — 40 Patients (5 Etiologies)" color={ACCENT}>
        {etiologies.map((e, i) => (
          <div key={i} className="mb-3 p-2 rounded" style={{ background: '#e3f2fd' }}>
...
function SeizuresTab({ data }) {
  ...
  return (
    <>
      ...
      {monitoring.length > 0 && (
        <SectionCard title="Monitoring Protocol" color={ACCENT5}>
          <ul className="mb-0 small">
...
function TreatmentsTab({ data }) {
  ...
  return (
    <>
      <div className="alert border-2 mb-4" style={{ borderColor: ACCENT2, background: '#ffebee' }}>
        <strong style={{ color: ACCENT2 }}>NO APPROVED DISEASE-MODIFYING THERAPY FOR MPS IIIC (2026):</strong>
        <div className="mt-1 small">
          ERT not approved (HGSNAT requires cytosolic acetyl-CoA — ERT cannot replicate this reaction).
          HSCT NOT recommended in established disease. Gene therapy (AAV9-HGSNAT intrathecal, Phase I)
          is the current best investigational option. Refer all eligible patients to trial.
          AED management is symptomatic only.
        </div>
      </div>
  ...
  return (
    <>
      ...
      {definitions.length > 0 && (
        <SectionCard title="HGSNAT / MPS IIIC Glossary (10 Terms)" color={ACCENT}>
          {definitions.map((d, i) => (
            <div key={i} className="mb-3 p-2 rounded" style={{ background: '#e3f2fd' }}>
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function HgsnatPage() {
  ...
  useEffect(() => {
    fetch(`${API}/api/hgsnat/overview`).then(r => r.json()).then(setOverview).catch(e => setErr(String(e)));
  }, []);

  useEffect(() => {
    if (tab >= 1 && tab <= 3) {
      fetch(`${API}/api/hgsnat/breakdown`).then(r => r.json()).then(setBreakdown).catch(() => {});
    }
    if (tab === 4) {
      fetch(`${API}/api/hgsnat/definitions`).then(r => r.json()).then(setDefinitions).catch(() => {});
    }
  }, [tab]);
  ...
  return (
    <div className="container-fluid py-4" style={{ maxWidth: 1200 }}>
      ...
      <p className="text-muted small mb-3">
        OMIM 252930 / 610453 · 8p11.21 · AR biallelic LOF · No approved ERT (2026) ·
        AAV9-HGSNAT intrathecal gene therapy Phase I · HSCT NOT recommended · 40-patient cohort ·
        Founders: p.Arg344Cys (French-Canadian) · p.Trp649stop (Dutch) · p.Pro524Leu (Italian)
      </p>
