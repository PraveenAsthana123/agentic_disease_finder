...
const TABS = ['Overview', 'Variant & Cohort', 'Treatment & Misdiagnosis', 'Definitions'];

// MODY3 colour scheme — teal/green (sulfo success; young onset; reversible misdiagnosis)
const ACCENT  = '#00695c';   // deep teal — HNF1A; most-common MODY
const ACCENT2 = '#1b5e20';   // deep green — sulfo SUCCESS; 85-90% excellent response
const ACCENT3 = '#b71c1c';   // deep red — misdiagnosis (30% T1D, 25% T2D); urgent
const ACCENT4 = '#e65100';   // deep orange — renal glycosuria; diagnostic clue
const ACCENT5 = '#4a148c';   // purple — haploinsufficiency; genetics
const ACCENT6 = '#0d47a1';   // deep blue — HbA1c; clinical metrics
const ACCENT7 = '#37474f';   // dark slate — epidemiology
const ACCENT8 = '#6a1b9a';   // dark purple — P291fsinsC founder variant
...
export default function MODY3Page() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/mody3/overview`).then(r => r.json()),
      fetch(`${API}/api/mody3/breakdown`).then(r => r.json()),
      fetch(`${API}/api/mody3/definitions`).then(r => r.json()),
    ]).then(([ov, br, df]) => {
    ...
    <div className="container-fluid py-3">
      ...
      <div className="mb-3 p-3 rounded" style={{ background: `linear-gradient(135deg, ${ACCENT}22, ${ACCENT2}11)`, border: `1px solid ${ACCENT}44` }}>
        <div className="d-flex align-items-center gap-2 flex-wrap">
          ...
          <div>
            <h4 className="mb-0 fw-bold" style={{ color: ACCENT }}>MODY3 — HNF1A-MODY</h4>
            <div className="text-muted small">Maturity-Onset Diabetes of the Young Type 3 · Chr 12q24.31 · OMIM #600496 · Most common MODY (~35%) · Autosomal Dominant · Sulfo-sensitive</div>
          </div>
          <div className="ms-auto d-flex gap-1 flex-wrap">
            <Badge text="HNF1A *142410" color={ACCENT} />
      ...
      <div className="row mb-3">
        ...
        <KPI label="Excellent Response" value={`${kpis.pct_sulfo_excellent_response?.toFixed(0)}%`} color={ACCENT2} />
        ...
        <KPI label="Renal Glycosuria" value={`${kpis.pct_renal_glycosuria?.toFixed(0)}%`} color={ACCENT4} />
        <KPI label="Misdiagnosed" value={`${kpis.pct_misdiagnosed?.toFixed(0)}%`} color={ACCENT3} />
        <KPI label="Mean C-pep (nmol/L)" value={kpis.mean_c_peptide_nmol_L?.toFixed(2)} color={ACCENT} />
        <KPI label="Mean Duration (yr)" value={kpis.mean_duration_years?.toFixed(1)} color={ACCENT7} />
      </div>
        ...
        <div className="row g-3">
          <div className="col-lg-6">
            <Section title="🧬 Disease Overview" color={ACCENT}>
              <Alert color={ACCENT2}>
                <strong>SULFO-FIRST:</strong> Sulfonylurea is 100–1000× more potent in MODY3 than T2D. Start low (0.5 mg glibenclamide) — 85–90% achieve excellent control.
              </Alert>
              <Alert color={ACCENT3}>
                <strong>MISDIAGNOSIS:</strong> ~55% of MODY3 patients are initially misdiagnosed as T1D or T2D. Antibody-negative + young onset + family history → test for MODY immediately.
              </Alert>
              <Alert color={ACCENT4}>
                <strong>RENAL GLYCOSURIA:</strong> Glucose in urine at NORMAL blood glucose — HNF1A controls SGLT2 (renal glucose transporter). This pathognomonic clue often precedes hyperglycaemia.
              </Alert>
              <table className="table table-sm table-bordered">
                <tbody>
                  <tr><td className="fw-bold">Gene</td><td>HNF1A (Hepatocyte Nuclear Factor 1α) · Chr 12q24.31 · *142410</td></tr>
                  <tr><td className="fw-bold">Disease OMIM</td><td>#600496 (MODY3 / NIDDM2)</td></tr>
                  <tr><td className="fw-bold">Inheritance</td><td>Autosomal Dominant · 50% recurrence per child</td></tr>
                  <tr><td className="fw-bold">Mechanism</td><td>HNF1A LOF → ↓GLUT2/GCK/SGLT2 → impaired GSIS → progressive beta-cell failure</td></tr>
                  <tr><td className="fw-bold">Prevalence</td><td>~1:10,000–1:20,000 (underdiagnosed; often misclassified as T1D/T2D)</td></tr>
                  <tr><td className="fw-bold">MODY fraction</td><td>~35% of all MODY — most common subtype</td></tr>
                  <tr><td className="fw-bold">Onset age</td><td>Typically late teens – early 30s (mean ~24–25 yr)</td></tr>
      ...
      {tab === 1 && (
        <div className="row g-3">
          ...
          <div className="col-lg-7">
            <Section title="👩‍⚕️ Patient Cohort (40 patients, seed 303)" color={ACCENT}>
                ...
                <table className="table table-sm table-bordered table-hover" style={{ fontSize: '0.73em' }}>
                  <thead className="table-dark">
                    <tr>
                      ...
                      <th>C-pep</th><th>Variant</th><th>Treatment</th><th>Response</th><th>Glycosuria</th>
                    </tr>
                  </thead>
                  <tbody>
                    {patients.map(p => (
        ...
        <div className="row g-3">
          <div className="col-lg-6">
            ...
            <Section title="✅ Sulfo Response" color={ACCENT}>
              ...
              <Alert color={ACCENT2}>
                <strong>Key:</strong> Start sulfonylure 10–100× lower than T2D dose (0.5 mg glibenclamide). Titrate slowly. Risk of severe hypoglycaemia if standard T2D doses used.
              </Alert>
            </Section>
            <Section title="🔁 Prior Misdiagnosis" color={ACCENT3}>
              ...
              <Alert color={ACCENT3}>
                <strong>Action:</strong> Confirmed MODY3 → STOP unnecessary insulin / metformin → switch to low-dose sulfo. Re-test first-degree relatives.
              </Alert>
            </Section>
          </div>
          <div className="col-lg-6">
            ...
            <Section title="🧬 Key Differentiators vs T1D / T2D / GCK-MODY" color={ACCENT5}>
              <table className="table table-sm table-bordered" style={{ fontSize: '0.8em' }}>
                <thead><tr><th>Feature</th><th>MODY3</th><th>T1D</th><th>T2D</th><th>GCK-MODY</th></tr></thead>
                <tbody>
                  <tr><td>Onset age</td><td>Teens–30s</td><td>Any</td><td>Usually &gt;40</td><td>Teens–30s</td></tr>
                  <tr><td>Antibodies</td><td style={{ color: ACCENT2 }}>NEGATIVE</td><td style={{ color: ACCENT3 }}>POSITIVE</td><td style={{ color: ACCENT2 }}>NEGATIVE</td><td style={{ color: ACCENT2 }}>NEGATIVE</td></tr>
                  <tr><td>C-peptide</td><td>Preserved</td><td>Low/absent</td><td>Variable</td><td>Normal</td></tr>
                  <tr><td>Renal glycosuria</td><td style={{ color: ACCENT4 }}>YES (~50%)</td><td>No</td><td>No</td><td>No</td></tr>
                  <tr><td>Sulfo sensitivity</td><td style={{ color: ACCENT2 }}>Extraordinary</td><td>Not used</td><td>Standard</td><td>Not needed</td></tr>
                  <tr><td>Progression</td><td>Progressive</td><td>Acute</td><td>Gradual</td><td>Stable</td></tr>
                  <tr><td>Family hx</td><td>~90%</td><td>Variable</td><td>Variable</td><td>~90%</td></tr>
                  <tr><td>HbA1c stability</td><td>Rises without Rx</td><td>Rapid if untreated</td><td>Slow rise</td><td>Stable ~38–48</td></tr>
                </tbody>
              </table>
            </Section>
          </div>
      ...
      {tab === 3 && (
        <div className="row g-3">
          <div className="col-12">
            <Section title="📖 Glossary — MODY3 / HNF1A-MODY" color={ACCENT}>
