...
const COLOR = '#004d40';   // dark teal — N6a/N6b dual Fe-S TYKY NDUFS8 theme
const LIGHT = '#e0f2f1';
...
function OverviewTab({ data }) {
  ...
  return (
    <div>
      ...
      <SectionCard title="NDUFS8 = TYKY Subunit — Dual N6a + N6b [4Fe-4S] Fe-S Clusters (Q-Module Approach)" borderColor="#00695c">
        <Alert variant="info" text="NDUFS8 (TYKY subunit) is unique among Complex I subunits: it carries TWO [4Fe-4S] Fe-S clusters (N6a and N6b) via two CXXC tetracysteine motifs. N6a and N6b occupy the 4th and 5th positions in the CI electron relay chain, positioned in the Q-module approach — between the N4 relay (NDUFS7, Q/N-module junction) and the peripheral N5 relay (NDUFS1). Loss of NDUFS8 eliminates BOTH N6a AND N6b simultaneously, creating a dual electron transfer block. CI activity: 5–20%, CII/CIII/CIV NORMAL." />
        <Alert variant="success" text="NDUFS8-CI-Leigh does NOT cause peripheral neuropathy. This is a critical distinguishing feature from NDUFS1 (IP1 subunit, ~50% axonal/demyelinating neuropathy). Absence of neuropathy + isolated CI + no olfactory lesions + no leukodystrophy: genetic panel required to distinguish NDUFS2/NDUFS3/NDUFS7/NDUFS8 (all within Q-module approach or its junction)." />
        <Alert variant="warning" text="BN-PAGE in NDUFS8: absent/severely reduced CI with a relatively clean pattern — fewer assembly intermediates than NDUFS3 (Q-module scaffold failure). This reflects NDUFS8's role as a primary Fe-S electron relay subunit (direct cluster loss) rather than a structural scaffold." />
      </SectionCard>

      <SectionCard title={`KPIs — 40-patient NDUFS8 cohort (seed-619)`}>
        <div className="row g-2">
          {data.kpis?.map(k => <KPI key={k.label} label={k.label} value={k.value} color={k.color} />)}
        </div>
      </SectionCard>

      <SectionCard title="Feature Frequencies — NDUFS8/CI-Leigh 40-patient cohort">
        {Object.entries(data.feature_frequencies || {}).map(([feat, pct]) => (
          <Bar key={feat} label={feat} value={pct} color={featureColor(feat)} />
        ))}
      </SectionCard>

      <SectionCard title="Fe-S Relay Chain — NDUFS8 N6a/N6b Position in Complex I" borderColor="#1b5e20">
          ...
          <table className="table table-sm table-bordered small">
            ...
            <tbody>
              {[
                ...
                ['NDUFV2', 'N1b [2Fe-2S]',      'N-module',                '2nd relay',                 'No major clinical DDx marker'],
                ['NDUFS7', 'N4 [4Fe-4S]',       'Q/N-module junction',     '3rd relay',                 'No neuropathy / no olfactory / no WM'],
                ['NDUFS8', 'N6a + N6b [4Fe-4S]','Q-module approach (TYKY)','4th + 5th relay — THIS',    'Dual Fe-S block; no neuropathy; no WM'],
                ...
                <tr key={r[0]} style={{ background: r[0] === 'NDUFS8' ? LIGHT : undefined, fontWeight: r[0] === 'NDUFS8' ? 'bold' : undefined }}>
      ...
      <SectionCard title="Absolute Contraindications — NDUFS8/CI-Leigh" borderColor="#b71c1c">
        <Alert variant="danger" text="ABSOLUTE CI: Valproate (VPA) — triple mechanism: CoA sequestration + POLG inhibition + hepatotoxicity. In NDUFS8-CI-Leigh, CI is already at 5–20%; VPA collapses residual OXPHOS. Use LEV." />
        <Alert variant="danger" text="ABSOLUTE CI: Metformin — direct Complex I inhibitor at ND1/quinone-binding site. NDUFS8/N6a-N6b block already prevents electrons reaching N2; metformin eliminates residual CI. Catastrophic." />
        <Alert variant="danger" text="ABSOLUTE CI: Linezolid + Chloramphenicol — mt-ribosome 23S rRNA block → all 7 mtDNA-encoded ND subunits absent → CI P-module collapses → near-zero CI in already-deficient NDUFS8 patient." />
        <Alert variant="danger" text="CONTRAINDICATED: Ketogenic Diet — forces NADH overload via beta-oxidation; N6a/N6b-blocked CI cannot re-oxidise NADH → worsened lactic acidosis. Beneficial in GLUT1-DS and PDHD, not CI-Leigh." />
        <Alert variant="warning" text="AVOID: Propofol (PRIS risk — CIV inhibition creates 2nd ETC block downstream of already-blocked CI N6a/N6b). Use sevoflurane or dexmedetomidine." />
...
function PatientsTab({ data }) {
  ...
  return (
    <div>
      <SectionCard title="Complex Activity Summary — NDUFS8 Cohort">
      ...
      <SectionCard title="40-Patient Cohort (seed-619)">
        <div className="table-responsive">
...
function TreatmentsTab({ data }) {
  ...
  return (
    <div>
      <SectionCard title="NDUFS8-CI-Leigh vs CI-Leigh Series — Critical DDx" borderColor="#1b5e20">
        <Alert variant="info" text="All CI-Leigh diseases share identical biochemistry: CI 5–20%, CII/CIII/CIV NORMAL. The only differentiators are clinical + MRI features. NDUFS8 has NO peripheral neuropathy (DDx NDUFS1 50%), NO olfactory bulb lesions (DDx NDUFS4 52–65%), NO leukodystrophy (DDx NDUFV1 40–50%). NDUFS8 is clinically indistinguishable from NDUFS7 without genetics." />
        <div className="table-responsive">
          <table className="table table-sm table-bordered small">
            <thead className="table-dark">
            ...
            <tbody>
              {[
                ...
                ['NDUFS1',  'N-module IP1 (75 kDa) N1b-N5',     'Peripheral neuropathy ~50%',                    'EMG / NCS'],
                ['NDUFS7',  'Q/N-junction N4',                  'Single N4 block; BN-PAGE clean',               'WES / genetic panel'],
                ['NDUFS8',  'Q-module TYKY N6a+N6b — THIS',     'Dual N6a+N6b block; no neuropathy, no WM, no olfactory', 'WES / genetic panel'],
                ['NDUFS2',  'Q-module N2 terminal (49 kDa)',    'Terminal N2 loss; HCM ~8%',                     'WES / genetic panel'],
                ['NDUFS3',  'Q-module scaffold (30 kDa QP-C)',  'BN-PAGE sub-assembly intermediates',           'BN-PAGE + genetics'],
              ].map(r => (
                <tr key={r[0]} style={{ background: r[0] === 'NDUFS8' ? LIGHT : undefined, fontWeight: r[0] === 'NDUFS8' ? 'bold' : undefined }}>
                  {r.map((c, i) => <td key={i}>{c}</td>)}
                </tr>
      ...
      <SectionCard title="Supportive Treatment Ladder — NDUFS8/CI-Leigh" borderColor="#2e7d32">
        {[
          ['IV Dextrose (GIR 6-8 mg/kg/min)',          'Crisis — FIRST LINE — never fast in CI-Leigh',                            'success'],
          ['Riboflavin B2 (100–400 mg/day)',            'CI-specific (FMN precursor — upstream of NDUFS8 N6a/N6b block)',          'info'],
          ['Succinate (2–8 g/day oral / IV if avail)', 'CII bypass — electrons enter ubiquinol BYPASSING NDUFS8 N6a/N6b block',   'info'],
      ...
      <SectionCard title="Contraindications Summary — NDUFS8/CI-Leigh" borderColor="#b71c1c">
        <div className="table-responsive">
          <table className="table table-sm table-bordered small">
            <thead className="table-dark">
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function NDUFS8Page() {
  ...
  useEffect(() => {
    fetch(`${API}/api/ndufs8/overview`).then(r => r.json()).then(setOverview).catch(e => setErr(String(e)));
  }, []);

  useEffect(() => {
    if (tab === 1 && !breakdown)
      fetch(`${API}/api/ndufs8/breakdown`).then(r => r.json()).then(setBreakdown).catch(e => setErr(String(e)));
    if ((tab === 2) && !breakdown)
      fetch(`${API}/api/ndufs8/breakdown`).then(r => r.json()).then(setBreakdown).catch(e => setErr(String(e)));
    if (tab === 3 && !defs)
      fetch(`${API}/api/ndufs8/definitions`).then(r => r.json()).then(setDefs).catch(e => setErr(String(e)));
  }, [tab]);
    ...
    <div className="container-fluid py-3">
      <div className="mb-3">
        ...
        <p className="text-muted small mb-0">
          NDUFS8 · TYKY Subunit · Dual N6a + N6b [4Fe-4S] Fe-S Clusters · Q-Module Approach ·
          11q13.2 · OMIM *602141 / #256000 · AR Biallelic · 40-patient cohort seed-619
        </p>
      </div>
