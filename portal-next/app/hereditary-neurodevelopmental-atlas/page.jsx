...
const SLUG = 'hereditary-neurodevelopmental-atlas';
const TABS = ['Overview', 'Gene Table', 'Clinical Atlas', 'Definitions'];

const GENE_COLORS = {
  'SHANK3':  '#1565c0',  // deep blue      — Phelan-McDermid; absent speech PATHOGNOMONIC; autism 94%; lymphedema 50%
  'SYNGAP1': '#6a1b9a',  // deep purple    — MRD5; epilepsy 50%; photoparoxysmal response 70%; drop attacks
  'ADNP':    '#2e7d32',  // dark green     — Helsmoortel-Van der Aa; autism 90%; p.Tyr719Ter hotspot 25%
  'ANKRD11': '#e65100',  // deep orange    — KBG; macrodontia PATHOGNOMONIC; short stature; delayed bone age
  'KAT6A':   '#b71c1c',  // deep red       — KAT6A syndrome; absent speech majority; cardiac CHD 30%
  'WAC':     '#00695c',  // dark teal      — DeSanto-Shinawi; behavioural dysregulation PATHOGNOMONIC
  'MED13L':  '#4e342e',  // dark brown     — MED13L syndrome; non-verbal 50%; wide mouth; gait ataxia
  'FOXP1':   '#f9a825',  // amber          — FOXP1/MRDFOXP1; verbal dyspraxia + ID + ASD; DDx FOXP2
};
...
export default function HereditaryNeurodevelopmentalAtlasPage() {
    ...
    <div style={{ fontFamily: 'system-ui, sans-serif', maxWidth: 1100, margin: '0 auto', padding: '20px 16px' }}>
      <div style={{ marginBottom: 18 }}>
        ...
        <p style={{ color: '#555', fontSize: 13, margin: 0 }}>
          Complete 8-Gene Neurodevelopmental Genetics Reference · SHANK3-SYNGAP1-ADNP-ANKRD11-KAT6A-WAC-MED13L-FOXP1 · 320 Patients · Seeds 3038-3045
        </p>
      </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr style={{ background: '#1565c0', color: '#fff' }}>
                    {['Gene','Locus','n','Severe%','Mod%','Mild%','Mean IQ','NonVerb%','Epilepsy%','Cardiac%','Autism%','Lymph%','Behav%','Regress%','Age Dx(mo)'].map(h => (
                ...
                <tbody>
                  {breakdown.genes?.map((g, i) => (
                    <tr key={g.gene}
                      ...
                      <td style={{ padding: '7px 10px', color: g.mean_iq < 50 ? '#c62828' : g.mean_iq < 70 ? '#e65100' : '#333', fontWeight: g.mean_iq < 50 ? 700 : 400 }}>{g.mean_iq}</td>
                      <td style={{ padding: '7px 10px', color: g.non_verbal_pct > 50 ? '#c62828' : '#aaa', fontWeight: g.non_verbal_pct > 50 ? 700 : 400 }}>{g.non_verbal_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.epilepsy_pct > 30 ? '#6a1b9a' : '#aaa', fontWeight: g.epilepsy_pct > 30 ? 700 : 400 }}>{g.epilepsy_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.cardiac_pct > 0 ? '#1565c0' : '#aaa' }}>{g.cardiac_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.autism_pct > 50 ? '#e65100' : '#666' }}>{g.autism_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.lymphedema_pct > 0 ? '#2e7d32' : '#aaa', fontWeight: g.lymphedema_pct > 0 ? 700 : 400 }}>{g.lymphedema_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.behavioural_pct > 0 ? '#00695c' : '#aaa', fontWeight: g.behavioural_pct > 0 ? 700 : 400 }}>{g.behavioural_pct}%</td>
                      <td style={{ padding: '7px 10px', color: g.regression_pct > 0 ? '#c62828' : '#aaa' }}>{g.regression_pct}%</td>
            ...
            <div key={g.gene} style={{ ...cardStyle, borderLeft: `4px solid ${GENE_COLORS[g.gene] || '#ccc'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {g.non_verbal_pct > 50 && <Badge text={`NonVerbal ${g.non_verbal_pct}%`} color="#c62828" />}
                  {g.epilepsy_pct > 30 && <Badge text={`Epilepsy ${g.epilepsy_pct}%`} color="#6a1b9a" />}
                  {g.cardiac_pct > 20 && <Badge text={`Cardiac ${g.cardiac_pct}%`} color="#1565c0" />}
                  {g.autism_pct > 50 && <Badge text={`Autism ${g.autism_pct}%`} color="#e65100" />}
                  {g.lymphedema_pct > 0 && <Badge text={`Lymphedema ${g.lymphedema_pct}%`} color="#2e7d32" />}
                  {g.behavioural_pct > 0 && <Badge text={`BehavDysreg ${g.behavioural_pct}%`} color="#00695c" />}
                  {g.mean_iq < 50 && <Badge text={`IQ ${g.mean_iq}`} color="#c62828" />}
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 6, marginTop: 8 }}>
                {[
                  ...
                  { label: 'Mean IQ', value: g.mean_iq, color: g.mean_iq < 50 ? '#c62828' : '#555' },
                  { label: 'NonVerb%', value: g.non_verbal_pct + '%', color: g.non_verbal_pct > 50 ? '#c62828' : '#555' },
      ...
      {tab === 'Definitions' && definitions && !loading && (
        <div>
          <div style={{ color: '#888', fontSize: 12, marginBottom: 12 }}>
            {definitions.count} clinical definition entries — NDD classification, absent-speech differential (SHANK3/ADNP/KAT6A/MED13L), SYNGAP1 epilepsy/photoparoxysmal response, ANKRD11/KBG macrodontia dental protocol, FOXP1 vs FOXP2 verbal dyspraxia
          </div>
