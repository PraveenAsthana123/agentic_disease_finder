...
const ACCENT  = '#1a237e';   // deep navy — GNAO1 / G-protein
const ACCENT2 = '#b71c1c';   // dark red — CI / danger
const ACCENT3 = '#1b5e20';   // dark green — KD / success
const ACCENT4 = '#e65100';   // deep orange — dyskinesia / GOF alert
const ACCENT5 = '#4a148c';   // purple — movement disorder / quinidine
...
function OverviewTab({ ov }) {
  ...
  return (
    <div>
      ...
      <SectionCard title="🧬 GNAO1 Gene & Syndrome" borderColor={ACCENT}>
      ...
      <div className="row g-3 mb-4">
        ...
        <KPI label="Movement D/O" value={`${ov.movement_disorder_pct}%`} color={ACCENT4} />
        <KPI label="On KD" value={`${ov.on_kd_pct}%`} color={ACCENT3} />
        <KPI label="G-Tube" value={`${ov.gtube_pct}%`} color={ACCENT5} />
        <KPI label="CBZ for MD" value={`${ov.cbz_for_md_pct}%`} color={ACCENT5} />
...
function PatientsTab({ bk }) {
  ...
  return (
    <div>
      <Alert variant="info" text={`41-patient GNAO1 cohort · GOF ${s.gof_pct}% · Ohtahara ${s.ohtahara_pct}% · West syndrome ${s.west_syndrome_pct}% · DRE ${s.drug_resistant_pct}% · KD ${s.kd_pct}% · G-Tube ${s.gtube_pct}% · CBZ-for-MD ${s.cbz_md_pct}% · POLG-without-screen: ${s.vpa_without_polg} patients ⚠`} />
          ...
          <table className="table table-sm table-striped" style={{ fontSize: 12 }}>
            <thead>
              <tr>
                ...
                <th>Ohtahara</th><th>West</th><th>DRE</th><th>MD</th><th>KD</th><th>CBZ-MD</th><th>POLG</th>
              </tr>
            </thead>
            <tbody>
              {(bk.patients_sample || []).map(p => (
                <tr key={p.id}>
                  ...
                  <td className={p.movement_disorder ? 'text-warning fw-bold' : ''}>{p.movement_disorder ? 'Y' : 'N'}</td>
                  <td className={p.on_kd ? 'text-success' : ''}>{p.on_kd ? 'Y' : 'N'}</td>
                  <td className={p.on_cbz_md ? 'text-primary' : ''}>{p.on_cbz_md ? 'Y' : 'N'}</td>
  ...
  return (
    <div>
      <div className="alert alert-warning fw-bold py-2 mb-3" style={{ fontSize: 13 }}>
        🎥 <strong>VIDEO-EEG CRITICAL:</strong> Hyperkinetic dyskinesias (GOF) are NON-ICTAL — EEG normal during event. Tonic spasms/focal seizures have EEG correlate. Video-EEG mandatory to distinguish — misdiagnosis → inappropriate AED escalation.
      </div>
      ...
      <SectionCard title="⚡ Seizure Type Prevalence" borderColor={ACCENT}>
        {(bk.seizure_types || []).map((s, i) => (
          <PctBar key={i} label={s.type} pct={s.prevalence_pct}
            color={s.type.includes('Dyskineti') ? ACCENT4 : ACCENT} />
        ))}
      </SectionCard>
      ...
      <SectionCard title="⚡ Seizure Type Detail" borderColor={ACCENT}>
        {(bk.seizure_detail || []).map((s, i) => (
          <div key={i} className="mb-3 pb-2 border-bottom">
            <div className="fw-bold small" style={{ color: s.type.includes('Dyskineti') ? ACCENT4 : ACCENT }}>
...
function TreatmentsTab({ bk }) {
  ...
  return (
    <div>
      <div className="alert alert-danger fw-bold py-2 mb-3" style={{ fontSize: 13 }}>
        🚫 <strong>ABSOLUTE CI:</strong> Tiagabine (NCSE risk) | Gpi-DBS in GOF (worsens seizures).<br/>
        💊 <strong>CBZ NOTE:</strong> CBZ in GNAO1 = LOW dose for MOVEMENT DISORDER only (NOT epilepsy dose). Does NOT control seizures.<br/>
        🔬 <strong>QUINIDINE:</strong> Investigational for GOF movement disorder (GIRK blockade) — QTc monitoring mandatory. Specialist centre only.
      </div>
          ...
          <div key={i} className="mb-4 pb-3 border-bottom">
            ...
            {t.gnao1_specific && (
              <div className="small mt-1 p-1 rounded" style={{ backgroundColor: '#e8eaf6', color: ACCENT }}>
                ⚡ <strong>GNAO1-Specific:</strong> {t.gnao1_specific}
              </div>
            )}
          </div>
        ))}
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function GNAO1Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/gnao1/overview`).then(r => r.json()),
      fetch(`${API}/api/gnao1/breakdown`).then(r => r.json()),
      fetch(`${API}/api/gnao1/definitions`).then(r => r.json()),
    ]).then(([ov, bk, df]) => {
      setOverview(ov);
      setBreakdown(bk);
      setDefinitions(df);
    ...
    <div className="container-fluid py-3" style={{ maxWidth: 1200 }}>
      <div className="mb-3">
        ...
        <div className="text-muted small">
          DEE17 · Ohtahara Syndrome · G-protein Alpha-O1 (Gαo) · GOF-LOF Dual Mechanism · 16q13
        </div>
        <div className="text-muted small mt-1">
          <strong>GOF (56%):</strong> Constitutive Gαo → Gβγ → VGCC inhibition → compensatory hyperexcitability + hyperkinetic dyskinesias (non-ictal!) &nbsp;|&nbsp;
          <strong>LOF (44%):</strong> Haploinsufficiency → impaired GABA-B/GIRK → epilepsy predominant &nbsp;|&nbsp;
          <strong style={{ color: ACCENT2 }}>CBZ LOW DOSE = movement disorder (GOF) NOT seizures</strong>
        </div>
      </div>
