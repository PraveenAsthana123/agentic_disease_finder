...
const COLOR  = '#1b5e20';   // deep green — low-malignancy PGL3
const LIGHT  = '#e8f5e9';
const COLOR2 = '#2e7d32';   // secondary green
  ...
  return (
    <div>
      ...
      <div className="alert mb-4" style={{ background: LIGHT, borderLeft: `5px solid ${COLOR}` }}>
        ...
        <p className="mb-1 small">
          <strong>Disease:</strong> Paraganglioma 3 (PGL3, OMIM #{data.omim_disease}) + Carney-Stratakis Syndrome — AD, NOT imprinted &nbsp;|&nbsp;
          <strong>Penetrance:</strong> {data.penetrance} &nbsp;|&nbsp;
          <strong>Malignancy:</strong> <span className="fw-bold" style={{ color: COLOR }}>{data.malignancy}</span>
        </p>
        <p className="mb-0 small fw-semibold" style={{ color: COLOR2 }}>
          ✅ SDHC: LOW malignancy ~1-3% (vs SDHB 20-50%). NOT imprinted — biparental (unlike SDHD PGL1 maternal imprinting).
          HNPGL predominant. Carney-Stratakis GIST ~10%. IHC: SDHB null ONLY.
        </p>
      </div>
      ...
      <div className="row mb-3">
        ...
        <KPI label="Malignant" value={`${s.malignant_pct}%`} color={COLOR} />
        <KPI label="GIST (CSS)" value={`${s.gist_pct}%`} color={COLOR2} />
            ...
            <tbody>
              {(data.patients || []).map(p => (
                <tr key={p.id}>
                  ...
                  <td>{p.bilateral ? '✓' : ''}</td>
                  <td>{p.secretory ? '✓' : ''}</td>
                  <td>{p.malignant ? <span className="text-success fw-bold">✓</span> : ''}</td>
...
function VariantsTab({ data }) {
  ...
  return (
    <div>
      {/* Structural features */}
      <SectionCard title="SDHC Structural Features">
      ...
      {/* Variants table */}
      <SectionCard title="Pathogenic / Likely-Pathogenic Variants in SDHC">
        <div className="table-responsive">
          <table className="table table-sm table-striped mb-0">
            <thead className="table-dark">
              <tr>
            ...
            <tbody>
              {(data.variants || []).map((v, i) => (
                <tr key={i}>
                  ...
                  <td><span className={`badge ${v.severity === 'Severe (null)' ? 'bg-danger' : v.severity === 'Severe' ? 'bg-warning text-dark' : v.severity === 'Intermediate' ? 'bg-info text-dark' : 'bg-secondary'}`}>{v.severity}</span></td>
...
function DDxTab({ data }) {
  ...
  const css = data.carney_stratakis || {};
  const ddx = data.key_ddx || [];

  return (
    <div>
      {/* CRITICAL imprinting alert */}
      <div className="alert alert-success mb-4" style={{ borderLeft: `5px solid ${COLOR}` }}>
        <strong>✅ SDHC NOT IMPRINTED — CRITICAL DDx from SDHD and SDHAF2</strong>
        <ul className="mb-0 mt-2 small">
          <li><strong>SDHC (PGL3, 1q23.3):</strong> NOT imprinted — both maternal and paternal SDHC mutations cause PGL3</li>
          <li><strong>SDHD (PGL1, 11q23.1):</strong> MATERNALLY IMPRINTED — only paternal SDHD mutations cause disease</li>
          <li><strong>SDHAF2 (PGL2, 11q13.1):</strong> MATERNALLY IMPRINTED — only paternal SDHAF2 mutations cause disease</li>
          <li>A female SDHC carrier: all children at 50% risk. A female SDHD carrier: children NOT at risk.</li>
        </ul>
      </div>
          ...
          <table className="table table-sm table-striped mb-0">
            ...
            <tbody>
              <tr style={{ background: LIGHT }}>
                <td><strong>SDHC (THIS)</strong></td>
                <td>1q23.3</td>
                <td>NOT imprinted; HNPGL; LOW malignancy ~1-3%; CSS GIST ~10%</td>
                <td className="text-success fw-bold">~1-3%</td>
                <td>~50-60%</td>
              </tr>
              {ddx.map((d, i) => (
                <tr key={i}>
      ...
      {/* Carney-Stratakis */}
      <SectionCard title="Carney-Stratakis Syndrome (CSS) — SDHC + GIST" borderColor={COLOR2}>
      ...
      {/* Pharmacology alerts */}
      <SectionCard title="Pharmacology Alerts" borderColor="#c62828">
        <ul className="mb-0 small">
          <li className="mb-1"><strong className="text-danger">Alpha-blockade BEFORE beta-blockade:</strong> Phenoxybenzamine must precede any beta-blocker — beta-first causes hypertensive crisis (unopposed alpha).</li>
          <li className="mb-1"><strong>Sunitinib:</strong> Anti-VEGFR/PDGFR — best systemic evidence for metastatic SDH-deficient PGL (including rare malignant SDHC).</li>
          <li className="mb-1"><strong>177Lu-DOTATATE PRRT:</strong> SSTR2-positive metastatic PGL — effective for progressive SDHC PGL (DOTATATE+ ~72%).</li>
          <li className="mb-1"><strong>Belzutifan (HIF-2α):</strong> Emerging for unresectable/metastatic SDH-deficient PGL — same pseudo-hypoxia axis.</li>
          <li className="mb-1"><strong>CSS GIST — imatinib reduced response:</strong> SDH-deficient GIST (SDHC-mutant) less responsive to imatinib than KIT-mutant GIST; sunitinib preferred for progressive disease.</li>
          <li className="mb-1"><strong>No contraindicated drugs specific to SDHC:</strong> No mitochondrial respiratory chain involvement in PGL3 phenotype (unlike SDHA Leigh/CII deficiency).</li>
        </ul>
      </SectionCard>
    </div>
  );
  ...
  return (
    <div>
      <SectionCard title="Gene Summary">
        ...
        <p className="small mb-1"><strong>OMIM Gene:</strong> *{data.omim_gene} &nbsp;|&nbsp; <strong>Disease:</strong> PGL3 #{data.omim_disease} / CSS #{data.omim_css || '606764'}</p>
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function SDHCPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/sdhc/overview`).then(r => r.json()),
      fetch(`${API}/api/sdhc/breakdown`).then(r => r.json()),
      fetch(`${API}/api/sdhc/definitions`).then(r => r.json()),
    ])
