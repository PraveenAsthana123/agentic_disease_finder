...
const COLOR = '#4a148c';   // deep purple — mitochondrial / metabolic severity
const LIGHT = '#f3e5f5';
  ...
  return (
    <div>
      <div className="alert py-2 small mb-3" style={{ borderLeft: `4px solid ${COLOR}`, background: LIGHT }}>
        <strong>SLC25A22 (11p15.5) — Mitochondrial Glutamate Carrier 1 (GC1) · OMIM 609302/609304 · DEE3/EIEE3:</strong>{' '}
        GC1 imports cytoplasmic glutamate into the mitochondrial matrix for TCA cycle and malate–aspartate shuttle.{' '}
        LOF → <strong>cytoplasmic glutamate accumulation (plasma &gt;200 µmol/L)</strong> + mitochondrial bioenergetic failure.{' '}
        Biallelic null → <strong>Ohtahara syndrome (burst-suppression, neonatal)</strong>;{' '}
        hypomorphic → <strong>West syndrome (infantile spasms, hypsarrhythmia)</strong>.{' '}
        <strong>Pathognomonic: elevated plasma glutamate + bilateral symmetric BG/thalamic MRI changes.</strong>{' '}
        <span className="fw-bold" style={{ color: COLOR }}>
          ABSOLUTE CI: PHT/CBZ/OXC worsen burst-suppression. VPA heightened mitochondrial toxicity (POLG mandatory).
          PYRIDOXINE 100 mg IV MANDATORY trial before diagnosis locked (B6-responsive seizures = treatable mimic).
        </span>
      </div>
      ...
      <div className="row g-2 mb-4">
        ...
        <KPI label="Migratory Focal" value={`${kpis.migratory_focal_pct}%`} color="#6f42c1" />
        <KPI label="Elevated Plasma Glut" value={`${kpis.elevated_plasma_glut_pct}%`} color={COLOR} />
        <KPI label="Mean Plasma Glut" value={`${kpis.mean_plasma_glut_umol_L} µmol/L`} color={COLOR} />
        <KPI label="BG MRI Changes" value={`${kpis.bg_mri_changes_pct}%`} color="#795548" />
        <KPI label="Thalamic Changes" value={`${kpis.thalamic_changes_pct}%`} color="#795548" />
        <KPI label="Profound ID" value={`${kpis.profound_id_pct}%`} color={COLOR} />
        <KPI label="Any ID" value={`${kpis.any_id_pct}%`} color="#6f42c1" />
        <KPI label="B6 Trial Done" value={`${kpis.b6_trial_pct}%`} color="#0d6efd" />
...
function BreakdownTab({ data }) {
  ...
  return (
    <div>
      <div className="alert py-2 small mb-3" style={{ borderLeft: `4px solid ${COLOR}`, background: LIGHT }}>
        <strong>40-patient cohort (seed 507) across 4 etiologic categories.</strong>{' '}
        Mean plasma glutamate: <strong>{summary.mean_plasma_glut_umol_L} µmol/L</strong> (normal &lt;100).{' '}
        Elevated plasma glutamate rate: <strong>{summary.elevated_plasma_glut_pct}%</strong>.{' '}
        BG MRI changes: <strong>{summary.bg_mri_changes_pct}%</strong>. B6 trial done: <strong>{summary.b6_trial_pct}%</strong>.
      </div>
        ...
        <table className="table table-sm table-striped table-bordered small">
          <thead className="table-dark">
            <tr>
              ...
              <th>Burst-Sup%</th><th>Hyps%</th><th>Elev Glut%</th><th>Mean Glut</th>
              <th>BG MRI%</th><th>Profound ID%</th><th>ACTH+VGB%</th><th>KD%</th><th>Yr1 Mort%</th>
            </tr>
          </thead>
          <tbody>
            {cats.map((c, i) => (
              <tr key={i}>
                ...
                <td>{c.n}</td>
                <td>{c.ohtahara_pct}%</td>
                <td>{c.west_pct}%</td>
                <td>{c.burst_suppression_pct}%</td>
                <td>{c.hypsarrhythmia_pct}%</td>
                <td>{c.elevated_plasma_glut_pct}%</td>
                <td>{c.mean_plasma_glut} µmol/L</td>
                <td>{c.bg_mri_pct}%</td>
                <td>{c.profound_id_pct}%</td>
                <td>{c.acth_vgb_pct}%</td>
                <td>{c.kd_pct}%</td>
                <td>{c.yr1_mortality_pct}%</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div>
      <div className="card shadow-sm mb-4">
        <div className="card-header fw-bold small" style={{ background: '#fff3e0' }}>
          5 Key Facts — SLC25A22 DEE3/EIEE3
        </div>
        <div className="card-body">
...
export default function SLC25A22Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/slc25a22/overview`).then(r => r.json()),
      fetch(`${API}/api/slc25a22/breakdown`).then(r => r.json()),
      fetch(`${API}/api/slc25a22/definitions`).then(r => r.json()),
    ]).then(([ov, bk, df]) => {
      setOverview(ov);
      setBreakdown(bk);
      setDefinitions(df);
    ...
    <div className="container-fluid py-3">
      <div className="mb-3">
        ...
        <div className="text-muted small">
          Mitochondrial Glutamate Carrier 1 (GC1) · 323 aa · 6 TM helices · 3 MCF repeat modules ·
          Chr 11p15.5 · OMIM 609302/609304 · AR biallelic · 40-patient cohort (seed 507)
        </div>
        <div className="text-muted small mt-1">
          <strong>Pathognomonic:</strong> Elevated plasma glutamate &gt;200 µmol/L (fasting AA) +
          bilateral symmetric BG/thalamic MRI signal changes · Ohtahara (null) → West (hypomorphic) ·
          Pyridoxine 100 mg IV mandatory · PHT/CBZ/OXC ABSOLUTE CI · POLG mandatory before VPA
        </div>
      </div>
