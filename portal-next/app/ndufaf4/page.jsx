...
const COLOR = '#311b92';   // deep indigo — NDUFAF4 / early ND1-module / obligate partner of NDUFAF3
const LIGHT = '#ede7f6';
...
function OverviewTab({ data }) {
  ...
  const mod = data.ndufaf4_ndufaf3_module_summary || {};
  ...
  return (
    <>
      ...
      <div className="alert mb-4" style={{ background: '#ffebee', borderLeft: '4px solid #c62828' }}>
        <strong>🔴 NO Riboflavin Response — Critical DDx vs ACAD9</strong> — NDUFAF4 has NO FAD-binding domain.
        High-dose riboflavin does NOT rescue NDUFAF4 deficiency. If MCIA-class CI deficiency shows riboflavin
        response: ACAD9 is the diagnosis. No response → consider NDUFAF4 / NDUFAF3 / other assembly factors.
      </div>

      <div className="alert mb-4" style={{ background: '#fff8e1', borderLeft: '4px solid #f57f17' }}>
        <strong>🟡 PROTEIN BLOT UNRELIABLE — NDUFAF3 IS SECONDARILY REDUCED IN NDUFAF4 PATIENTS</strong> —
        NDUFAF4 loss causes secondary destabilization of its obligate partner NDUFAF3. Western blot showing
        reduced NDUFAF3 does NOT rule out primary NDUFAF4 deficiency. <strong>WES is mandatory</strong> to
        distinguish NDUFAF4 (6q16.3) from NDUFAF3 (2q33.1).
      </div>

      <div className="alert mb-4" style={{ background: LIGHT, borderLeft: `4px solid ${COLOR}` }}>
        <strong>🟣 NDUFAF4 — Earliest ND1-Module CI Assembly Factor / NDUFAF3 Obligate Heterodimer Partner</strong> —
        NDUFAF4 (C6orf66, 6q16.3) forms an obligate heterodimer with NDUFAF3 (C3orf60, 2q33.1). This
      ...
      <SectionCard title="🔄 Key Pathway Note — ND1-Module / NDUFAF4-NDUFAF3 Heterodimer / No Riboflavin">
        <p className="small mb-0">{data.key_pathway_note}</p>
      </SectionCard>

      {mod.heterodimer && (
        <SectionCard title="⚙️ NDUFAF4-NDUFAF3 Module Summary">
          ...
          <div className="mt-2">
            ...
            <div className="small text-muted mb-1">Effect of NDUFAF4 loss</div>
            <p className="small mb-0">{mod.ndufaf4_loss_effect}</p>
          </div>
        </SectionCard>
      )}
      ...
      <SectionCard title="📊 Feature Frequencies (40-patient cohort, seed-685)">
...
function ND1ModuleTab({ data }) {
  ...
  const stepColors = ['#311b92', '#4527a0', '#b71c1c', '#c62828'];

  return (
    <>
      <div className="alert mb-4" style={{ background: LIGHT, borderLeft: `4px solid ${COLOR}` }}>
        <strong>NDUFAF4-NDUFAF3 Obligate Heterodimer:</strong> NDUFAF4 (6q16.3) + NDUFAF3 (2q33.1)<br />
        <span className="small text-muted">
          ...
          handles ND2/ND5. Loss of NDUFAF4 destabilizes NDUFAF3, stalling the ND1-module assembly.
        </span>
      </div>
              ...
              <div className="flex-grow-1">
                ...
                <div className="mb-1">
                  <span className="badge" style={{
                    background: step.status_in_ndufaf4_deficiency.startsWith('INTACT') ? '#2e7d32' :
                                step.status_in_ndufaf4_deficiency.startsWith('DISRUPTED') ? '#c62828' :
                                step.status_in_ndufaf4_deficiency.startsWith('STALLED') ? '#e65100' : '#b71c1c',
                    fontSize: '0.7rem'
                  }}>
                    {step.status_in_ndufaf4_deficiency}
                  </span>
                </div>
      ...
      <SectionCard title="🔗 CI Assembly Intermediate Classes — BN-PAGE" borderColor="#1565c0">
        <p className="small text-muted mb-3">
          ...
          NDUFAF4 belongs exclusively to Class 3 (ND1-module) — same class as NDUFAF3.
        </p>
        <div className="table-responsive">
          <table className="table table-sm small">
                ...
                <tr key={i} style={{ background: c.class.includes('Class 3') ? LIGHT : undefined }}>
                  <td className="fw-bold" style={{ color: c.class.includes('Class 3') ? COLOR : undefined }}>
                    ...
                    {c.class.includes('Class 3') && (
                      <span className="badge ms-1" style={{ background: COLOR, fontSize: '0.6rem' }}>NDUFAF4</span>
                    )}
                  </td>
      ...
      <SectionCard title="🏗️ NDUFAF4 vs NDUFAF3 — Obligate Heterodimer Partners" borderColor={COLOR}>
        <div className="alert mb-3" style={{ background: '#fff8e1', borderLeft: '4px solid #f57f17' }}>
          <strong>Protein blot cannot distinguish NDUFAF4 from NDUFAF3 deficiency.</strong> NDUFAF4 loss
          causes secondary reduction of NDUFAF3 protein — mimicking primary NDUFAF3 deficiency.
          Only WES with chromosomal locus (6q16.3 vs 2q33.1) assigns the diagnosis definitively.
        </div>
        <div className="table-responsive">
          <table className="table table-sm small">
            <thead><tr><th>Feature</th><th>NDUFAF4</th><th>NDUFAF3</th></tr></thead>
            <tbody>
              {[
                { feature: 'Chromosome',              n4: '6q16.3',         n3: '2q33.1' },
                { feature: 'OMIM gene',               n4: '*611776',        n3: '*612911' },
                { feature: 'Protein size',            n4: '130 aa / 14.5 kDa', n3: '139 aa / 16.1 kDa' },
                { feature: 'CI deficiency',           n4: '5-20%',          n3: '5-20%' },
                { feature: 'Phenotype',               n4: 'Leigh / CI',     n3: 'Leigh / CI' },
                { feature: 'Riboflavin response',     n4: 'None (0%)',      n3: 'None (0%)' },
                { feature: 'Partner protein effect',  n4: 'NDUFAF3 secondarily reduced', n3: 'NDUFAF4 secondarily reduced' },
                { feature: 'BN-PAGE class',           n4: 'Class 3 (ND1)', n3: 'Class 3 (ND1)' },
                { feature: 'Distinguishing DDx',      n4: 'WES: 6q16.3',   n3: 'WES: 2q33.1' },
              ].map(r => (
                <tr key={r.feature}>
                  <td className="text-muted">{r.feature}</td>
                  <td className="fw-bold" style={{ color: COLOR }}>{r.n4}</td>
                  <td>{r.n3}</td>
                </tr>
              ))}
            </tbody>
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function NDUFAF4Page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/ndufaf4/overview`).then(r => r.json()),
      fetch(`${API}/api/ndufaf4/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ndufaf4/definitions`).then(r => r.json()),
  ...
  return (
    <div className="container-fluid py-3">
      <div className="d-flex align-items-center mb-3 flex-wrap gap-2">
        <h4 className="fw-bold mb-0" style={{ color: COLOR }}>🧬 NDUFAF4 — Complex I Deficiency (MC1DN20)</h4>
        ...
        <span className="badge ms-1" style={{ background: '#1565c0' }}>NDUFAF3 Obligate Heterodimer</span>
        <span className="badge ms-1" style={{ background: '#e65100' }}>Earliest CI Assembly Complex</span>
        <span className="badge ms-1 bg-secondary">6q16.3</span>
        <span className="badge ms-1 bg-secondary">OMIM *611776</span>
