...
const ACCENT  = '#1b5e20';   // deep forest green — LMBRD1 / lysosomal membrane
...
const ACCENT5 = '#e65100';   // deep orange — stomatitis / pathognomonic cblF sign
const ACCENT6 = '#1a237e';   // deep indigo — KEY NEGATIVES / differential
const ACCENT7 = '#37474f';   // slate — variant data / gene card
const ACCENT8 = '#4a148c';   // deep purple — lysosomal pathway / upstream block
...
export default function LMBRD1Page() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API}/api/lmbrd1/overview`).then(r => r.json()),
      fetch(`${API}/api/lmbrd1/breakdown`).then(r => r.json()),
      fetch(`${API}/api/lmbrd1/definitions`).then(r => r.json()),
    ])
  ...
  if (loading) return <div className="p-4 text-center text-muted">Loading LMBRD1 (cblF) dashboard…</div>;
  ...
  return (
    <div className="container-fluid py-3 px-3" style={{ maxWidth: 1100 }}>
      ...
      <div className="mb-3 p-3 rounded" style={{ background: `linear-gradient(135deg, ${ACCENT} 0%, ${ACCENT8} 100%)`, color: '#fff' }}>
        <h4 className="mb-1 fw-bold">&#x1f9ec; LMBRD1 (cblF) Epilepsy Dashboard</h4>
        <div style={{ fontSize: 13, opacity: 0.9 }}>
          Methylmalonic Aciduria &amp; Homocystinuria — cblF type · MOST UPSTREAM Lysosomal Cobalamin Transporter ·
          6q13 · AR · OMIM {ov.omim_gene} / {ov.omim_disease}
        </div>
      ...
      <Alert variant="warning" text="⚠️ VPA (Valproate) — HIGH RISK: carnitine depletion + metabolic stress → MMA surge. Use LEV first-line for ALL seizure types in LMBRD1/cblF." />
      <Alert variant="warning" text="⚠️ Betaine MANDATORY alongside OHCbl — cobalamin-independent BHMT remethylation. Neither alone is sufficient." />
      <Alert variant="info" text="ℹ️ LMBRD1 (cblF) MOST UPSTREAM: cobalamin TRAPPED in lysosomes → MMACHC receives no substrate → Combined MMA + HHcy (identical pattern to cblC but upstream). Gene panel LMBRD1 + ABCD4 mandatory." />
        ...
        <div>
          <Section title="Key Performance Indicators" color={ACCENT}>
            <div className="row g-2">
              ...
              <KPI label="Stomatitis (%)" value={`${ov.kpis.stomatitis_pct}%`} color={ACCENT5} />
              <KPI label="Vacuolated Lymph (%)" value={`${ov.kpis.vacuolated_lymphocyte_pct}%`} color={ACCENT8} />
              ...
              <KPI label="Avg C3 (µmol/L)" value={ov.kpis.avg_c3_umol_l} color={ACCENT5} />
            </div>
          </Section>
            ...
            {(ov.phenotype_distribution || []).map((ph, i) => (
              <div key={i} className="mb-3 p-3 border rounded" style={{ borderLeft: `4px solid ${ACCENT8}` }}>
                ...
                <div className="row mt-2" style={{ fontSize: 12 }}>
                  ...
                  <div className="col-md-2"><strong>Stomatitis:</strong> {ph.stomatitis}</div>
                  <div className="col-md-3"><strong>Vacuolated Lymph:</strong> {ph.vacuolated_lymphocytes}</div>
                  <div className="col-md-4"><strong>MMA:</strong> {ph.mma}</div>
                </div>
          ...
          <Section title="Function &amp; Mechanism" color={ACCENT}>
            <div className="p-3 rounded mb-3" style={{ background: '#f0f7f0', fontSize: 13 }}>
            ...
            <div className="p-3 rounded" style={{ background: '#e8eaf6', fontSize: 13 }}>
              <strong>Key Negatives:</strong> {ov.key_negative}
            </div>
          </Section>

          <Section title="LMBRD1 Cobalamin Pathway (Step-by-Step)" color={ACCENT3}>
            <div className="table-responsive">
              <table className="table table-sm table-bordered" style={{ fontSize: 12 }}>
                <thead className="table-dark">
                  <tr><th>Step</th><th>Reaction</th><th>Enzyme / Gene</th><th>Consequence of cblF LOF</th></tr>
                </thead>
                <tbody>
                  {(ov.lmbrd1_pathway || []).map((s, i) => (
            ...
            {(ov.high_risk_situations || []).map((s, i) => (
              <div key={i} className="mb-2 p-2 border rounded" style={{ borderLeft: `3px solid ${ACCENT3}` }}>
                <div className="d-flex justify-content-between">
                  ...
                  <span className="badge" style={{ background: s.risk.includes('ABSOLUTE') ? '#b71c1c' : s.risk.includes('HIGH') ? '#e65100' : '#1565c0', color: '#fff', fontSize: 11 }}>{s.risk}</span>
                </div>
      ...
      {tab === 'Patients & Biomarkers' && (
        <div>
          <Section title="Biomarker Reference — cblF (LMBRD1)" color={ACCENT2}>
            {(bd.biomarkers || []).map((b, i) => (
              <div key={i} className="mb-3 p-3 border rounded" style={{ borderLeft: `3px solid ${ACCENT2}` }}>
                ...
                <div className="row mt-1" style={{ fontSize: 12 }}>
                  ...
                  <div className="col-md-4" style={{ color: ACCENT3 }}><strong>cblF Range:</strong> {b.lmbrd1_range}</div>
          ...
          <Section title="Key Variants (LMBRD1)" color={ACCENT7}>
            <div className="table-responsive">
              <table className="table table-sm table-bordered" style={{ fontSize: 12 }}>
                <thead className="table-dark">
                ...
                <tbody>
                  {(bd.key_variants || []).map((v, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ fontSize: 11 }}>{v.domain}</td>
              ...
              <table className="table table-sm table-bordered" style={{ fontSize: 11 }}>
                <thead className="table-dark">
                  <tr>
                    ...
                    <th>NH3↑</th><th>Stom.</th><th>Vacuol.</th><th>NBS</th><th>OHCbl Resp.</th><th>AED</th>
                  </tr>
                </thead>
                <tbody>
                  {(bd.patient_sample || []).map((p, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ color: ACCENT5 }}>{p.c3_umol_l}</td>
                      <td>{p.ammonia_umol_l}</td>
                      <td style={{ color: p.stomatitis ? ACCENT5 : '#999' }}>{p.stomatitis ? '✓' : '—'}</td>
                      <td style={{ color: p.vacuolated_lymphocytes ? ACCENT8 : '#999' }}>{p.vacuolated_lymphocytes ? '✓' : '—'}</td>
      ...
      {tab === 'Seizures & Triggers' && (
        <div>
          <Section title="Seizure Types — LMBRD1 (cblF)" color={ACCENT3}>
          ...
          <Section title="Metabolic Triggers" color={ACCENT5}>
            {(bd.metabolic_triggers || []).map((t, i) => (
              <div key={i} className="mb-3 p-2 border rounded" style={{ borderLeft: `3px solid ${ACCENT5}` }}>
                <div className="d-flex justify-content-between mb-1">
                  <span className="fw-bold" style={{ color: ACCENT5, fontSize: 13 }}>{t.trigger}</span>
          ...
          <Section title="High-Risk Drugs — cblF" color={ACCENT3}>
      ...
      {tab === 'Treatments' && (
        <div>
          ...
          <Section title="Treatments — cblF (LMBRD1)" color={ACCENT4}>
      ...
      {tab === 'Definitions' && (
        <div>
          <Section title="Gene Card — LMBRD1 (cblF)" color={ACCENT7}>
            <div className="table-responsive">
              <table className="table table-sm table-bordered" style={{ fontSize: 12 }}>
                <tbody>
