...
function PctBar({ label, pct, color = '#0891b2' }) {
  return (
    <div className="mb-2">
...
function SectionCard({ title, children, borderColor = '#0891b2' }) {
  return (
    <div className="card mb-4 shadow-sm" style={{ borderLeft: `4px solid ${borderColor}` }}>
      <div className="card-header fw-bold" style={{ backgroundColor: '#f0f9ff', color: borderColor }}>
        {title}
      </div>
...
export default function BECTSDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/bects/overview`).then(r => r.json()),
      fetch(`${API}/api/bects/breakdown`).then(r => r.json()),
      fetch(`${API}/api/bects/definitions`).then(r => r.json()),
    ]).then(([ov, br, df]) => {
      setOverview(ov);
      setBreakdown(br);
      setDefinitions(df);
  ...
  if (loading) return <div className="container py-5 text-center text-muted">Loading SeLECTS/BECTS Dashboard…</div>;
  ...
  return (
    <div className="container-fluid py-3" style={{ maxWidth: 1200 }}>
      <div className="mb-3">
        <h2 className="fw-bold mb-0" style={{ color: '#0891b2' }}>🧒 Self-Limited Epilepsy with Centrotemporal Spikes (SeLECTS / BECTS)</h2>
        <div className="text-muted small">
          Most common benign childhood epilepsy · Rolandic Epilepsy · 95% remission by puberty ·{' '}
      ...
      {tab === 0 && (
        <>
          ...
          <div className="row g-2 mb-4">
            <KPI label="Total Patients" value={overview.total_patients} color="#0891b2" />
            <KPI label="Male" value={`${overview.male_n} (${overview.male_pct}%)`} color="#2563eb" />
            <KPI label="Oro-facial Seizures" value={`${overview.oro_facial_seizure_n} (${overview.oro_facial_seizure_pct}%)`} color="#0891b2" />
            <KPI label="Nocturnal GTCS" value={`${overview.nocturnal_gtcs_n} (${overview.nocturnal_gtcs_pct}%)`} color="#f59e0b" />
            <KPI label="CSWS Evolution" value={`${overview.csws_evolution_n} (${overview.csws_evolution_pct}%)`} color="#dc2626" />
            <KPI label="Treatment-Free" value={`${overview.treatment_free_n} (${overview.treatment_free_pct}%)`} color="#16a34a" />
          </div>

          {/* Alerts */}
          <SectionCard title="⚠️ Clinical Safety Alerts" borderColor="#f59e0b">
            {(overview.clinical_alerts || []).map((a, i) => (
              <Alert key={i} text={a} variant={a.startsWith('⛔') ? 'danger' : a.startsWith('✅') ? 'success' : 'warning'} />
            ))}
          </SectionCard>

          {/* Prognosis + Etiology side by side */}
          <div className="row mb-4">
            <div className="col-md-6">
              <SectionCard title="📊 Etiology Distribution" borderColor="#0891b2">
                ...
                <table className="table table-sm table-bordered mb-0">
                  <tbody>
                    {Object.entries(prognosis).map(([k, v]) => (
                      <tr key={k}>
                        <td className="small fw-bold text-muted" style={{ whiteSpace: 'nowrap' }}>{k.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</td>
      ...
      {tab === 1 && (
        <>
              ...
              <div className="accordion-item" key={i}>
                <h2 className="accordion-header">
                  <button className="accordion-button collapsed fw-bold" type="button"
                    ...
                    🧬 {et.etiology} — {et.pct}%
                    <span className="badge ms-2" style={{ backgroundColor: '#0891b2', fontSize: 11 }}>{et.category}</span>
                  </button>
                </h2>
          ...
          {/* Patient Table */}
          <SectionCard title={`👥 Patient Register (${filtered.length}/${patients.length})`} borderColor="#0891b2">
            <div className="mb-2">
              ...
              <table className="table table-sm table-hover table-bordered">
                <thead className="table-light sticky-top">
                  <tr>
                    ...
                    <Th k="csws_evolution" label="CSWS" />
                    <Th k="grin2a_positive" label="GRIN2A" />
                    <Th k="learning_difficulty" label="Learning Δ" />
                  </tr>
                </thead>
                <tbody>
                  {sorted.map(p => (
                    <tr key={p.patient_id}>
                      ...
                      <td className="text-center">{p.csws_evolution ? '⚠️' : '—'}</td>
                      <td className="text-center">{p.grin2a_positive ? '🧬' : '—'}</td>
                      <td className="text-center">{p.learning_difficulty ? '📚' : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {tab === 2 && (
        <>
          <SectionCard title="⚡ Seizure Types in SeLECTS" borderColor="#0891b2">
            {(breakdown.seizure_types || []).map((st, i) => (
              <div key={i} className="mb-4 border-bottom pb-3">
                <div className="d-flex align-items-center mb-2">
                  ...
                  <span className="badge" style={{ backgroundColor: '#0891b2' }}>{st.freq_pct}% of SeLECTS</span>
      ...
      {tab === 3 && (
        <>
          <div className="alert alert-info mb-3">
            <strong>ℹ️ Key Principle — Self-Limited Syndrome:</strong>
            <ul className="mb-0 mt-1">
              <li><strong>Watch-and-wait</strong> is appropriate for infrequent nocturnal-only seizures (25% of patients)</li>
              <li><strong>Vigabatrin (VGB)</strong> is <strong className="text-danger">CONTRAINDICATED</strong> — worsens focal seizures; can provoke CSWS</li>
              <li><strong>AED discontinuation</strong> is mandatory after ≥2 years seizure-free (self-limited condition)</li>
              <li><strong>OXC/CBZ:</strong> Screen HLA-B*1502 before prescribing in Asian-descent patients (SJS risk)</li>
            </ul>
          </div>
                ...
                <h2 className="accordion-header">
                  <button className="accordion-button collapsed fw-bold" type="button"
                    ...
                    {tx.contraindication_note && (
                      <span className="badge bg-warning text-dark ms-2" style={{ fontSize: 10, maxWidth: 200, whiteSpace: 'normal', textAlign: 'left' }}>
          ...
          <SectionCard title="📅 SeLECTS Lifecycle Management" borderColor="#2563eb">
      ...
      {tab === 4 && (
        <>
          ...
          <SectionCard title="⚠️ Contraindications / Cautions" borderColor="#f59e0b">
            <div className="table-responsive">
              <table className="table table-sm table-bordered">
                <thead className="table-light">
                ...
                <tbody>
                  {(definitions?.absolute_contraindications || []).map((ci, i) => (
                    <tr key={i}>
                      <td><strong className="text-warning">{ci.drug}</strong></td>
          ...
          {/* Concepts */}
          <SectionCard title="📖 Glossary — SeLECTS / BECTS Concepts" borderColor="#0891b2">
