...
const COLOR  = '#00695c';   // deep teal — tRNA-Trp / H-strand / SEVENTH tRNA
const LIGHT  = '#e0f2f1';
const COLOR2 = '#00838f';   // medium cyan — CPEO / myopathy
const COLOR3 = '#b71c1c';   // dark red — absolute CIs / UGA-recoding alert
const COLOR4 = '#1b5e20';   // dark green — biochemical fingerprint / OXPHOS
const COLOR5 = '#4a148c';   // deep indigo — WARS2 DDx / nuclear gene
...
function OverviewTab({ data }) {
  ...
  const uga_alert = data.uga_recoding_alert || {};
  ...
  return (
    <>
      ...
      {/* UGA-recoding alert — prominent */}
      <div className="alert border-0 mb-4" style={{ backgroundColor: '#fce4ec', borderLeft: `6px solid ${COLOR3}` }}>
        <strong style={{ color: COLOR3 }}>🔴 UGA CODON RECODING — MT-TW EXCLUSIVE — MITOCHONDRIAL GENETIC CODE:</strong>
        <br />
        In the standard code UGA = STOP. In the mitochondrial code, <strong>UGA = Tryptophan</strong>.
        MT-TW (UCA anticodon) is the <strong>SOLE tRNA decoding UGA as Trp</strong> in human mitochondria,
        reading both UGG and UGA codons. MT-CO1 (COX I) has the highest density of UGA-Trp residues —
        anticodon variants (m.5543TC, m.5545GA) cause <strong>preferential CIV &gt; CI deficiency</strong>
        (an asymmetry unusual among mt-tRNA diseases).
      </div>

      {/* H-strand / L-strand cluster note */}
      <div className="alert border-0 mb-4" style={{ backgroundColor: '#e8f5e9', borderLeft: `6px solid ${COLOR4}` }}>
        <strong style={{ color: COLOR4 }}>✅ H-STRAND ENCODED — but 4 consecutive L-strand tRNAs follow immediately 3′:</strong>
        <br />
        MT-TW (rCRS 5512–5579) is H-strand encoded — no L-strand NGS pitfall for MT-TW itself.
        However, <strong>MT-TA, MT-TN, MT-TC, MT-TY</strong> (rCRS 5587–5891) are ALL L-strand encoded.
        Laboratories MUST switch to L-strand QC after rCRS 5579.
      </div>

      {/* KPIs */}
      <SectionCard title="40-Patient Cohort — Key Statistics (seed-813)" borderColor={COLOR2}>
        ...
        <div className="row g-2 mt-1">
          ...
          <KPI label="UGA Disrupted" value={`${s.pct_uga_recoding_disrupted}%`} color={COLOR3} />
          <KPI label="CIV > CI Asym" value={`${s.pct_uga_asymmetry_civ_worse}%`} color={COLOR3} />
      ...
      {/* Gene facts */}
      <SectionCard title="Gene Facts — MT-TW" borderColor={COLOR}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small mb-0">
            <tbody>
...
function VariantsTab({ data }) {
  ...
  return (
    <>
      <SectionCard title="Variant Breakdown — 40-Patient Cohort (seed-813)" borderColor={COLOR}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small mb-0">
            <thead className="table-light">
              <tr>
                ...
                <th>CPEO%</th><th>Myopathy%</th><th>Cardio%</th><th>UGA disrupted%</th>
              </tr>
            </thead>
            <tbody>
              {vb.map((v, i) => (
                <tr key={i}>
                  ...
                  <td>{v.pct_uga_recoding_disrupted}%</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <>
      <SectionCard title="Differential Diagnosis Table" borderColor={COLOR5}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small mb-0">
            ...
            <tbody>
              {ddx.map((d, i) => (
                <tr key={i} className={d.entity.startsWith('MT-TW') ? 'table-info' : ''}>
      ...
      <div className="card mb-4 shadow-sm" style={{ borderTop: `3px solid ${COLOR}` }}>
        <div className="card-body">
          <h6 className="fw-bold mb-3" style={{ color: COLOR }}>UGA Codon Recoding — Clinical Decision Tree</h6>
          <div className="p-3 rounded" style={{ backgroundColor: LIGHT, fontSize: '0.85rem' }}>
            <p className="mb-2"><strong>CPEO + CI+CIV deficiency (CII NORMAL)?</strong></p>
            <ul>
              <li>→ <strong>CIV more deficient than CI by &gt;8%?</strong> (UGA-asymmetry): Consider MT-TW anticodon variants (m.5543TC, m.5545GA) — request mtDNA panel H-strand 5512–5579</li>
              <li>→ <strong>CIV ≈ CI deficiency?</strong>: Consider MT-TM (initiation block at high hetero), MT-TI, MT-TQ, MT-TV, MT-TF</li>
              <li>→ <strong>Stroke-like episodes?</strong>: MT-TL1 (MELAS) — NOT MT-TW</li>
              <li>→ <strong>Myoclonic epilepsy?</strong>: MT-TK (MERRF) — NOT MT-TW</li>
              <li>→ <strong>Isolated HCM?</strong>: MT-TI m.4300AG — NOT MT-TW</li>
              <li>→ <strong>Neonatal/infantile onset + cardiomyopathy?</strong>: WARS2 (AR nuclear) — WES, not mtDNA panel</li>
      ...
      <div className="card mb-4 shadow-sm" style={{ borderTop: `3px solid ${COLOR3}` }}>
        <div className="card-body">
          <h6 className="fw-bold mb-3" style={{ color: COLOR3 }}>Perioperative Protocol (ALL MT-TW patients)</h6>
          <div className="row g-3 small">
            {[
              ...
              { label: 'ICU for >50% heteroplasmy', desc: 'High-heteroplasmy MT-TW: ICU perioperative; anaesthetic + metabolic co-management' },
            ].map((item, i) => (
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function MTTWPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/mttw/overview`).then(r => r.json()),
      fetch(`${API}/api/mttw/breakdown`).then(r => r.json()),
      fetch(`${API}/api/mttw/definitions`).then(r => r.json()),
    ]).then(([ov, bk, df]) => {
      setOverview(ov);
      setBreakdown(bk);
      setDefs(df);
      ...
      <div className="d-flex align-items-center gap-2 mb-3">
        ...
        <div>
          <h5 className="mb-0 fw-bold" style={{ color: COLOR }}>
            MT-TW — tRNA-Trp — UGA Codon Recoding — SEVENTH tRNA — H-strand rCRS 5512–5579
          </h5>
          <small className="text-muted">
            Combined CI+CIV Deficiency · CPEO · Myopathy · UGA recoding exclusive · WARS2 DDx · OMIM *590095 · seed-813
          </small>
        </div>
      </div>
