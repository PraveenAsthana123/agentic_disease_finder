...
// Movement Disorders Atlas color palette — neurology / neurodegenerative
const COLOR  = '#1a237e';  // deep indigo — neurology
const LIGHT  = '#e8eaf6';  // light indigo tint
const COLOR2 = '#b71c1c';  // red — fatal/progressive
const COLOR3 = '#1565c0';  // blue — targeted therapy
const COLOR4 = '#e65100';  // orange — warning / avoid
const COLOR5 = '#4a148c';  // dark purple — X-linked / severe
const COLOR6 = '#37474f';  // blue-grey — structural
const COLOR7 = '#880e4f';  // dark magenta — critical error

const GENE_COLORS = {
  HTT:    '#b71c1c',  // red — Huntington chorea / fatal progressive
  ATP7B:  '#2e7d32',  // green — Wilson treatable / copper
  TOR1A:  '#1565c0',  // blue — DYT1 dystonia / GPi-DBS
  PANK2:  '#4a148c',  // dark purple — PKAN iron accumulation
  VPS13A: '#e65100',  // orange — Chorea-Acanthocytosis self-mutilation
  GCH1:   '#00695c',  // teal — DRD dopa-responsive curative
  PRRT2:  '#6a1b9a',  // purple — PKD paroxysmal kinesigenic
  ATP1A3: '#c62828',  // dark red — AHC alternating hemiplegia
};
...
function Loading() {
  return (
    <div className="text-center py-5">
      ...
      <div className="mt-2 text-muted small">Loading Movement Disorders Atlas…</div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      {(ov.drug_alerts || []).map((a, i) => (
        <AlertBox key={i}
          type={
            a.includes('ABSOLUTELY') || a.includes('MANDATORY') || a.includes('AVOID') ||
            a.includes('EMERGENCY') || a.includes('CONTRAINDICATED') || a.includes('WORSENS')
              ? 'danger'
          ...
          {genes.map(g => {
            ...
            return (
              <tr key={g}>
                ...
                <td><span className="text-warning fw-semibold">{d.progression_pct}%</span></td>
                <td><span className="text-secondary">{d.cognitive_impairment_pct}%</span></td>
          ...
          <div className="card-body py-3 px-3">
            ...
            <div className="row g-2 mb-3">
              {[
                ...
                { label: 'Progression', value: `${d.progression_pct}%` },
...
// ── Main Page ──────────────────────────────────────────────────────────
export default function MovementDisordersAtlasPage() {
  ...
  useEffect(() => {
    const base = `${API}/api/movement-disorders-atlas`;
    Promise.all([
  ...
  return (
    <div className="container-fluid py-3">
      ...
      <div className="d-flex align-items-center gap-3 mb-3">
        <div>
          <h4 className="fw-bold mb-0" style={{ color: COLOR }}>🧠 Movement Disorders Atlas</h4>
          <p className="text-muted small mb-0">
            Complete 8-Gene Hereditary Movement Disorders Atlas — HTT · ATP7B · TOR1A · PANK2 · VPS13A · GCH1 · PRRT2 · ATP1A3
          </p>
          <p className="text-muted small mb-0">
            320 patients (8×40, seeds 1078–1085) · Huntington · Wilson · DYT1 · PKAN · ChAc · DRD · PKD · AHC
          </p>
        </div>
      </div>
