...
const GENE_COLORS = {
  HFE:     '#1565c0',  // deep blue — HH1, most common, C282Y 1:200 Northern Europeans, phlebotomy curative
  HJV:     '#b71c1c',  // dark red — Juvenile HH2A, most severe, cardiomyopathy+hypogonadism age 15–30
  HAMP:    '#880e4f',  // dark pink — Juvenile HH2B, hepcidin itself absent, rarest JH
  TFR2:    '#4a148c',  // deep purple — HH3, diferric-transferrin sensor, worldwide no founder
  SLC40A1: '#e65100',  // deep orange — HH4 AD ONLY, LOF 4A macrophage TS-low, GOF 4B hepatocyte TS-high
  CP:      '#1b5e20',  // dark green — Aceruloplasminemia, retina+neuro+diabetes triad, TS-LOW paradox
  FTL:     '#37474f',  // dark slate — HHCS, 5'UTR IRE GOF, massive ferritin NORMAL iron, cataracts ONLY
  BMP6:    '#4e342e',  // dark brown — BMP6-HH, rarest <50 cases, SMAD-hepcidin driver, WES required
};

const GENE_DISEASE = {
  HFE:     'HH type 1 (HH1) AR — Homeostatic Iron Regulator — C282Y/C282Y 1:200 Northern Europeans — TS >45% Fasting Screen — Phlebotomy Curative Pre-Cirrhosis — Ferritin <50 µg/L Target — Raw-Shellfish CI (Vibrio)',
  HJV:     'Juvenile HH type 2A (JH2A) AR — Hemojuvelin BMP6-Coreceptor Absent — Hepcidin Undetectable — Cardiomyopathy+Hypogonadism Age 15–30 MOST SEVERE — Cardiac MRI T2* Mandatory — Urgent Phlebotomy+Deferoxamine',
  HAMP:    'Juvenile HH type 2B (JH2B) AR — Hepcidin (25-aa Peptide) Itself Absent — BMP-SMAD Intact — Urinary Hepcidin Undetectable Diagnostic — Identical Phenotype JH2A — Rarest JH (<30 Families Worldwide)',
  TFR2:    'HH type 3 (HH3) AR — TFR2 Hepatic Diferric-Transferrin Sensor — Hepcidin Inadequate for Iron Load — Worldwide No Founder — HFE-Negative HH2nd-Step — Phenotype Similar HH1 — Phlebotomy Standard',
  SLC40A1: 'HH type 4 (HH4) AD ONLY-AD-HH — Ferroportin Sole-Iron-Exporter — LOF Type-4A TS-NORMAL/LOW Macrophage-Loading Cautious-Phlebotomy — GOF Type-4B TS-HIGH Hepcidin-Resistant Hepatocyte-Loading Standard-Phlebotomy',
  CP:      'Aceruloplasminemia AR — Ceruloplasmin Ferroxidase Absent — Fe2+ Cannot Load Transferrin — Iron Trapped in Cells — TS LOW Despite Overload — Triad Retinal-Degeneration+Neurological+Diabetes PATHOGNOMONIC — Deferoxamine Chelation',
  FTL:     'HHCS AD — Ferritin-L-Chain 5\'UTR-IRE-GOF — Massive Ferritin 1000–12000 µg/L + NORMAL Iron Stores + Nuclear-Cataracts-Childhood — Phlebotomy CONTRAINDICATED (Iron-Deficiency No-Ferritin-Drop) — Cataract Surgery Only',
  BMP6:    'BMP6-HH AR Rarest (<50 Cases Worldwide) — BMP6 Main-Hepatic-Hepcidin-Driver via SMAD1/5/8 — HFE/HJV/HAMP/TFR2/SLC40A1-Negative → WES Required — Phenotype Similar HH1 — Phlebotomy Curative',
};
...
function GeneTableTab({ data }) {
  ...
  return (
    <div>
      <h3 style={{ color: '#e2e8f0', marginBottom: 12 }}>Gene Reference Table — All 8 HH Genes</h3>
          ...
          <div>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                <thead>
                  <tr style={{ background: '#0f172a' }}>
                    {['ID','Age Dx','Delay(mo)','Ferritin(µg/L)','TS(%)','S-Iron(µmol/L)',
                      'Phlebotomy','Cirrhosis','Diabetes','Hypogonadism','Cataract'].map(h => (
                ...
                <tbody>
                  {gene.sample_patients.map((p, i) => (
                    <tr key={p.patient_id} style={{ background: i % 2 === 0 ? '#1e293b' : '#162032' }}>
                      ...
                      <td style={{ padding: '6px 8px', color: '#e2e8f0' }}>{p.ferritin_ugL}</td>
                      <td style={{ padding: '6px 8px', color: '#e2e8f0' }}>{p.transferrin_saturation_pct}</td>
                      <td style={{ padding: '6px 8px', color: '#e2e8f0' }}>{p.serum_iron_umolL}</td>
                      <td style={{ padding: '6px 8px', color: p.on_phlebotomy ? '#10b981' : '#ef4444' }}>
                        {p.on_phlebotomy ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: p.cirrhosis_present ? '#ef4444' : '#64748b' }}>
                        {p.cirrhosis_present ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: p.diabetes_present ? '#f59e0b' : '#64748b' }}>
                        {p.diabetes_present ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: p.hypogonadism_present ? '#8b5cf6' : '#64748b' }}>
                        {p.hypogonadism_present ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: p.cataract_present ? '#0ea5e9' : '#64748b' }}>
                        {p.cataract_present ? 'Yes' : 'No'}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
export default function HemoPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/hereditary-hemochromatosis-atlas/overview`).then(r => r.json()),
      fetch(`${API}/api/hereditary-hemochromatosis-atlas/breakdown`).then(r => r.json()),
      fetch(`${API}/api/hereditary-hemochromatosis-atlas/definitions`).then(r => r.json()),
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ color: '#f1f5f9', fontSize: 22, fontWeight: 700, marginBottom: 4 }}>
            🧬 Hereditary Hemochromatosis Atlas
          </h1>
          <p style={{ color: '#64748b', fontSize: 13 }}>
            Complete 8-Gene Iron-Overload Atlas — HFE / HJV / HAMP / TFR2 / SLC40A1 / CP / FTL / BMP6 — 320 Patients (8×40, Seeds 1678–1685)
          </p>
        </div>
