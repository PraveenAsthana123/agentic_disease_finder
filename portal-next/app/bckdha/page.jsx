...
const ACCENT  = '#bf360c';   // deep burnt orange — MSUD / leucine toxicity / myelinopathy
...
const ACCENT4 = '#0277bd';   // steel blue — thiamine B1 / treatment / TPP cofactor
const ACCENT5 = '#4a148c';   // deep purple — BCKDH complex / mechanism / alloisoleucine
const ACCENT6 = '#1b5e20';   // dark green — KEY NEGATIVES (PDH normal / αKGDH normal / DLD normal)
const ACCENT7 = '#37474f';   // dark slate — Mennonite founder / variant data
...
function OverviewTab() {
  const { data, err } = useFetch(`${API}/api/bckdha/overview`);
  ...
  return (
    <div>
      <Alert
        text="🔑 PATHOGNOMONIC MARKER: Alloisoleucine >5 µmol/L in plasma — diagnostic ONLY in MSUD (BCKDHA/BCKDHB/DBT deficiency). Alloisoleucine is absent from all non-MSUD conditions. A single plasma amino acids test showing alloisoleucine confirms the diagnosis. BCAA elevation (Leu>>Ile>Val) + alloisoleucine + maple syrup odour = MSUD until proven otherwise."
        variant="info"
      />
      <Alert
        text="⚠️ LEUCINE NEUROTOXICITY — TREATMENT URGENCY: Plasma leucine >400 µmol/L → acute encephalopathy; >1000 µmol/L → cerebral oedema risk; >2000 µmol/L → life-threatening brainstem herniation. Classic neonatal MSUD presents day 3–7 with maple syrup odour + progressive encephalopathy. IV glucose (GIR ≥8 mg/kg/min) + insulin + BCAA-free formula + HD if leucine rising → IMMEDIATE ACTION REQUIRED."
        variant="danger"
      />
      <Alert
        text="✅ KEY NEGATIVES (confirm isolated BCKDH block): PDH activity NORMAL · αKGDH complex activity NORMAL · GCS intact · DLD free enzyme NORMAL. If any of these are reduced, consider DLD deficiency (all four complexes blocked) — requires gene panel: BCKDHA + BCKDHB + DBT + DLD."
        variant="success"
      />
      <Alert
        text={`💊 FIRST-LINE: (1) BCAA-restricted diet + BCAA-free synthetic formula (Level A — lifelong); (2) Thiamine B1 10–20 mg/kg/day trial ≥3 months for ALL patients (Level A — ~15% are thiamine-responsive). VPA ABSOLUTE CI (triple mechanism). Fasting EXTREME HAZARD. Liver transplant (Level B) markedly improves leucine tolerance.`}
        variant="warning"
      />
      ...
      <Section title="Gene Summary" color={ACCENT5}>
        <table className="table table-sm table-bordered small">
          <tbody>
            ...
            <tr><td className="fw-bold">Locus</td><td>{data.locus} — Autosomal Recessive (chromosome 19q13.2)</td></tr>
            <tr><td className="fw-bold">Protein length</td><td>{data.aa_length} aa precursor (~418 aa mature after MTS cleavage); E1α catalytic subunit of BCKDH E1 decarboxylase (α₂β₂ heterotetramer)</td></tr>
        ...
        <div className="table-responsive">
          <table className="table table-sm table-bordered small">
            <thead className="table-dark">
              <tr><th>Component</th><th>Gene(s)</th><th>Function in BCKDHA deficiency context</th></tr>
            </thead>
            <tbody>
              {(data.bckdh_complex_components || []).map((comp, i) => (
                <tr key={i} style={comp.component.includes('THIS GENE') ? {background:'#fff3e0'} : {}}>
        ...
        {data.bcaa_catabolism_pathway && (
          <table className="table table-sm table-bordered small">
            <tbody>
              ...
              <tr style={{background:'#fff3e0'}}><td className="fw-bold" style={{color:ACCENT5}}>Alloisoleucine formation</td><td style={{color:ACCENT5}}>{data.bcaa_catabolism_pathway.alloisoleucine_formation}</td></tr>
            </tbody>
          </table>
        )}
      </Section>

      <Section title="KPI Summary — 40-Patient BCKDHA Cohort" color={ACCENT}>
...
function PatientsTab() {
  const { data, err } = useFetch(`${API}/api/bckdha/breakdown`);
    ...
    <div>
      <Section title="Diagnostic Biomarkers" color={ACCENT5}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small">
            <thead className="table-dark">
              <tr><th>Biomarker</th><th>Normal range</th><th>BCKDHA range</th><th>Clinical significance</th></tr>
            </thead>
            <tbody>
              {(data.biomarkers || []).map((b, i) => (
                <tr key={i} style={b.bckdha_range && (b.bckdha_range.includes('NORMAL') || b.bckdha_range.includes('ABSENT')) ? {background:'#e8f5e9'} : b.bckdha_range && b.bckdha_range.includes('PATHOGNOMONIC') ? {background:'#fff3e0'} : {}}>
                  <td className="fw-bold">{b.name}</td>
                  <td style={{color:'#388e3c'}}>{b.normal}</td>
                  <td style={{color: b.bckdha_range && (b.bckdha_range.includes('NORMAL') || b.bckdha_range.includes('ABSENT')) ? '#388e3c' : b.bckdha_range && b.bckdha_range.includes('PATHOGNOMONIC') ? ACCENT5 : ACCENT2, fontWeight:'600'}}>{b.bckdha_range}</td>
...
function SeizuresTab() {
  ...
  return (
    <div>
      <Alert
        text="⚡ Seizures in BCKDHA deficiency are primarily METABOLIC — driven by acute leucine/KIC neurotoxicity, cerebral oedema, and energy failure (myelinopathy). Seizure control requires METABOLIC NORMALISATION first (leucine reduction). AED monotherapy without BCAA control is ineffective. Burst suppression on neonatal EEG is common in classic MSUD acute crisis."
        variant="warning"
      />
      ...
      <Section title="Metabolic Crisis Triggers" color={ACCENT2}>
        <Alert text="ALL triggers cause BCAA surge via muscle protein catabolism → leucine crisis within hours. Metabolic emergency protocol must be pre-arranged for every MSUD patient (sick-day plan + emergency letter + nearest metabolic centre)." variant="danger" />
...
function DefinitionsTab() {
  const { data, err } = useFetch(`${API}/api/bckdha/definitions`);
    ...
    <div>
        ...
        <table className="table table-sm table-bordered small">
          <tbody>
            {Object.entries(data.diagnostic_thresholds || {}).map(([k, v], i) => (
              <tr key={i} style={k.includes('ci') || k.includes('absolute') ? {background:'#ffebee'} : k.includes('pathognomonic') ? {background:'#fff3e0'} : {}}>
      ...
      <Section title="Differential Diagnosis" color={ACCENT7}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small">
            <thead className="table-dark">
              <tr><th style={{width:'30%'}}>Disease</th><th>How to distinguish from BCKDHA deficiency</th></tr>
            </thead>
            <tbody>
...
export default function BCKDHAPage() {
    ...
    <div className="container-fluid py-3">
      <div className="mb-3">
        ...
        <div className="text-muted small">
          Maple Syrup Urine Disease Type 1A · Branched-Chain Keto Acid Dehydrogenase E1α Subunit Deficiency ·
          BCKDH complex E1 catalytic subunit (α₂β₂ heterotetramer with BCKDHB E1β) ·
          Irreversible BCAA catabolism step 2 BLOCKED · Alloisoleucine PATHOGNOMONIC ·
          Leucine neurotoxicity (LAT1 / mTOR / myelinopathy) · Mennonite founder p.Tyr393Asn ·
          TPP cofactor (thiamine-responsive 10–20%) · VPA ABSOLUTE CI (triple mechanism) ·
          Autosomal Recessive · 19q13.2 · OMIM *608348 / #248600 · ~1:185,000 globally
        </div>
      </div>
