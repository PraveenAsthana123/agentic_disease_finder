...
const GENE_COLORS = {
  SDHB:  '#b71c1c',  // dark red — most aggressive, 40% malignant, DOTATATE mandatory
  SDHD:  '#1565c0',  // deep blue — paternal imprinting, head/neck PGL, bilateral CBT
  VHL:   '#1b5e20',  // dark green — bilateral adrenal pheo + RCC + hemangioblastoma
  RET:   '#4a148c',  // deep purple — MEN2A/MEN2B, MTC + pheo, prophylactic thyroidectomy
  NF1:   '#e65100',  // deep orange — café-au-lait, epinephrine-secreting adrenal only
  MAX:   '#880e4f',  // dark pink — paternal imprinting, bilateral adrenal, young onset
  SDHA:  '#37474f',  // dark slate — pituitary unique, SDHA IHC specific, rarest SDH pheo
  SDHC:  '#4e342e',  // dark brown — head/neck PGL, gastric GIST unique, no imprinting
};

const GENE_DISEASE = {
  SDHB:  'SDH4 AD — Iron-Sulfur SDH Complex II Subunit — Most Aggressive ~40% Malignant PGL — Extraadrenal/Multifocal — DOTATATE-PET Mandatory — MIBG Inferior — Sunitinib FIRSTMAPPP — Surveillance Age 5',
  SDHD:  'PGL1 AD Paternal-Imprinting — Membrane Anchor D — Head/Neck PGL Parasympathetic Non-Secreting — Bilateral Carotid Body PATHOGNOMONIC — Maternal Transmission = Carrier Only — Low Malignancy ~5%',
  VHL:   'VHL-Syndrome AD — E3-Ubiquitin-Ligase HIF2α Substrate — Type2 = Bilateral Adrenal Pheo + ClearCell-RCC + Cerebellar Hemangioblastoma — Belzutifan FDA2021 — 3cm Active Surveillance — Annual Ophthalmology',
  RET:   'MEN2A/MEN2B AD-GOF — RTK GDNF Receptor — C634R: MTC+Pheo+PHPT — M918T: Marfanoid+MucoNeuromas+MTC+Pheo — Pheo-BEFORE-Thyroid Surgery MANDATORY — Prophylactic Thyroidectomy Age-Variant-Dependent',
  NF1:   'NF1 AD — RAS-GAP Neurofibromin — Café-au-Lait + Lisch Nodules + Neurofibromas — Pheo 0.1–5.7% Adrenal-Only EPINEPHRINE-Secreting — Hypertensive Crisis — Selumetinib FDA2020 Plexiform',
  MAX:   'MAX AD Paternal-Imprinting — MYC-Associated Factor X bHLH-LZ — Bilateral Adrenal Pheo ONLY — No Head/Neck PGL — Youngest Onset ~28y — Catecholamine Secreting — SDHB IHC Retained — Malignancy ~20%',
  SDHA:  'SDH5 AR-biallelic-Leigh/AD-haploinsufficiency — FAD Flavoprotein Catalytic Subunit — Pituitary Adenoma UNIQUE — SDHA IHC Loss SPECIFIC (not sensitive) — Rarest SDH Pheo — 1% Population Carrier',
  SDHC:  'PGL3 AD No-Imprinting — Integral Membrane Anchor C — Head/Neck PGL Like-SDHD — Gastric GIST UNIQUE SDH Gene — Imatinib Ineffective — Carney-Stratakis Syndrome — SDHB IHC Loss Retained-SDHA',
};
...
function GeneTableTab({ data }) {
  ...
  return (
    <div>
      <h3 style={{ color: '#e2e8f0', marginBottom: 12 }}>Gene Reference Table — All 8 Pheo/PGL Genes</h3>
          ...
          <div>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                <thead>
                  <tr style={{ background: '#0f172a' }}>
                    {['ID','Age Dx','Delay(mo)','NMN+','MN+','Location','Malignant','Bilateral',
                      'RCC','Hypert Crisis','α-Block'].map(h => (
                ...
                <tbody>
                  {gene.sample_patients.map((p, i) => (
                    <tr key={p.patient_id} style={{ background: i % 2 === 0 ? '#1e293b' : '#162032' }}>
                      ...
                      <td style={{ padding: '6px 8px', color: p.plasma_normetanephrine_elevated ? '#10b981' : '#64748b' }}>
                        {p.plasma_normetanephrine_elevated ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: p.plasma_metanephrine_elevated ? '#10b981' : '#64748b' }}>
                        {p.plasma_metanephrine_elevated ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: '#94a3b8', fontSize: 10 }}>{p.tumour_location}</td>
                      <td style={{ padding: '6px 8px', color: p.malignant_pgl ? '#ef4444' : '#64748b' }}>
                        {p.malignant_pgl ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: p.bilateral_tumour ? '#f59e0b' : '#64748b' }}>
                        {p.bilateral_tumour ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: p.rcc_present ? '#8b5cf6' : '#64748b' }}>
                        {p.rcc_present ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: p.hypertensive_crisis ? '#ef4444' : '#64748b' }}>
                        {p.hypertensive_crisis ? 'Yes' : 'No'}
                      </td>
                      <td style={{ padding: '6px 8px', color: p.on_alpha_blockade ? '#10b981' : '#64748b' }}>
                        {p.on_alpha_blockade ? 'Yes' : 'No'}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
export default function PheoPGLPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/hereditary-pheo-pgl-atlas/overview`).then(r => r.json()),
      fetch(`${API}/api/hereditary-pheo-pgl-atlas/breakdown`).then(r => r.json()),
      fetch(`${API}/api/hereditary-pheo-pgl-atlas/definitions`).then(r => r.json()),
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ color: '#64748b', fontSize: 13 }}>
            Complete 8-Gene Hereditary Pheo/PGL Atlas — SDHB / SDHD / VHL / RET / NF1 / MAX / SDHA / SDHC — 320 Patients (8×40, Seeds 1686–1693)
          </p>
        </div>
