...
// BBS5 colour scheme — deep indigo-emerald-amber-crimson (PH domain; lipid sensor; ciliary membrane anchor)
const ACCENT  = '#1a237e';   // deep indigo — BBS5 PH domain; phosphoinositide lipid binding
const ACCENT2 = '#004d40';   // dark emerald — ciliary membrane anchoring; phosphoinositide
...
export default function BBS5Page() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(`${API}/api/bbs5/overview`).then(r => r.json()),
      fetch(`${API}/api/bbs5/breakdown`).then(r => r.json()),
      fetch(`${API}/api/bbs5/definitions`).then(r => r.json()),
    ])
      .then(([ov, bd, df]) => { setOverview(ov); setBreakdown(bd); setDefinitions(df); setLoading(false); })
      .catch(e => { setError(e.message); setLoading(false); });
  }, []);

  if (loading) return <div className="container py-5 text-center text-muted">Loading BBS5 / PH-Domain Lipid Sensor dashboard…</div>;
    ...
    <div className="container-fluid py-3">
      ...
      <div className="mb-3 p-3 rounded" style={{ background: ACCENT + '12', borderLeft: `5px solid ${ACCENT}` }}>
        <div className="d-flex flex-wrap align-items-center gap-2 mb-1">
          <h4 className="mb-0 fw-bold" style={{ color: ACCENT }}>BBS5 — Bardet-Biedl Syndrome Type 5</h4>
          <Badge text="BBS5 / *603650" color={ACCENT} />
          <Badge text="Chr 2q31.1" color={ACCENT6} />
          <Badge text="AR Biallelic LOF" color={ACCENT4} />
          <Badge text="~2–3% of BBS" color={ACCENT2} />
          <Badge text="BBSome PH-Domain Lipid Sensor" color={ACCENT2} />
          <Badge text="40-patient cohort · seed-341" color={ACCENT6} />
        </div>
        <p className="mb-0 small text-muted">
          BBS5 is the <strong>sole intrinsic lipid sensor</strong> of the BBSome octameric complex — its PH (Pleckstrin Homology)
          domain anchors the assembled BBSome to <strong>phosphoinositide lipids (PtdIns(3)P / PtdIns(4,5)P2)</strong> at the
          ciliary transition zone membrane inner leaflet.
          BBS5 LOF → BBSome assembles normally at PCM1 satellites (BBS4 intact, anti-BBS4 IF <em>normal</em>) →
          ARL6/BBS3-GTP delivers BBSome to ciliary membrane zone → BBSome <strong>cannot anchor to PtdIns lipids</strong> →
          detaches before IFT-B engagement → GPCR ciliary trafficking fails → multi-system BBS.
          Failure is one step <strong>later than BBS4</strong> (satellite tethering) and intrinsic vs <strong>BBS3</strong> (external GTPase).
        </p>
      </div>
      ...
      {tab === 0 && (
        <div>
          <Alert color={ACCENT}>
            <strong>Unique mechanism:</strong> BBS5 is the <strong>only BBSome subunit with a PH domain</strong> — the
            intrinsic phosphoinositide lipid anchor of the complex.
            Anti-BBS4 IF in BBS5-null patient fibroblasts shows <em>normal</em> PCM1-colocalised BBS4 puncta,
            distinguishing BBS5 LOF from BBS4 LOF (where BBS4 IF is absent) — the key cellular discriminator.
          </Alert>
          ...
          {/* Mechanism */}
          <Section title="Molecular Mechanism — BBS5 PH Domain & Ciliary Membrane Phosphoinositide Anchoring" color={ACCENT}>
          ...
          {/* Key distinction */}
          <Section title="Key Distinction — BBS5 vs BBS1 / BBS2 / BBS3 / BBS4" color={ACCENT2}>
          ...
          <Section title="BBS Pathway Functional Comparison — BBS5 Position" color={ACCENT}>
      ...
      {tab === 1 && breakdown && (
        <div>
          <Alert color={ACCENT2}>
            <strong>BBS5 systemic burden:</strong> Rod-cone dystrophy (100%) → polydactyly (60%) → obesity (75%) →
            hypogonadism (58%) → anosmia (55%) → cognitive impairment (42%) → renal anomaly (38%) → CHD (5%).
            Slightly lower polydactyly + renal penetrance vs BBS4 (65%/45%) — consistent with BBS5 peripheral BBSome
            position and membrane anchoring (vs satellite-tethering) failure mechanism.
          </Alert>
      ...
      {tab === 2 && definitions && (
        <div>
          <Alert color={ACCENT}>
            <strong>Diagnostic key:</strong> Full BBS gene panel (20–24 genes) mandatory — BBS5 clinically indistinguishable
            from BBS1–BBS4 without genetics. BBS5 VUS: classify by PH domain position (β5–β6 loop = lipid binding;
            BBS4-contact surface = module integration; C-terminal = BBS9 platform contact).
            Confirm LOF with anti-BBS4 IF in patient fibroblasts — <em>normal</em> PCM1 puncta excludes BBS4 LOF and
            supports BBS5 (or downstream) defect.
          </Alert>

          {/* Key variants table */}
          <Section title="Key BBS5 Pathogenic Variants" color={ACCENT}>
            <div className="table-responsive">
              <table className="table table-sm table-bordered small">
                <thead style={{ background: ACCENT + '18' }}>
          ...
          {/* Diagnostic workup */}
          <Section title="Diagnostic Workup — BBS5" color={ACCENT2}>
            ...
            <div className="table-responsive">
              <table className="table table-sm table-bordered small">
                <thead style={{ background: ACCENT3 + '18' }}>
                  <tr><th>Disease</th><th>Key Differentiator from BBS5</th></tr>
                </thead>
                <tbody>
      ...
      {tab === 3 && definitions && (
        <div>
          {/* Gene card */}
          <Section title="Gene Card — BBS5 (*603650)" color={ACCENT}>
            <div className="table-responsive">
              <table className="table table-sm table-bordered small">
                <tbody>
          ...
          {/* Disease card */}
          <Section title="Disease Card — BBS5 (#209900)" color={ACCENT4}>
            <div className="table-responsive">
              <table className="table table-sm table-bordered small">
                <tbody>
          ...
          {/* Mechanism glossary */}
          <Section title="Mechanism Glossary — BBS5 PH Domain, Phosphoinositides & BBSome Membrane Anchoring" color={ACCENT2}>
      ...
      <div className="mt-4 pt-3 border-top small text-muted">
        ...
        {' · '}
        <strong>BBS5 (BBS5)</strong>
        {' · '}
        <span>OMIM Gene: <a href="https://www.omim.org/entry/603650" target="_blank" rel="noreferrer" className="text-decoration-none">*603650</a></span>
        {' · '}
