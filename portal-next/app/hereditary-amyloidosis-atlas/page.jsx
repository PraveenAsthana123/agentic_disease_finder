...
const GENE_COLORS = {
  TTR:   '#1b5e20',  // deep green — transthyretin, most treatable, patisiran/tafamidis
  APOA1: '#0d47a1',  // deep blue — apoA-I, renal/hepatic, low HDL
  APOA2: '#4a148c',  // deep purple — apoA-II, stop-gain, late-onset renal
  FGA:   '#e65100',  // deep orange — fibrinogen Aα, liver transplant curative
  LYZ:   '#880e4f',  // deep pink — lysozyme, hepatic rupture risk
  CST3:  '#b71c1c',  // deep red — cystatin C, young cerebral haemorrhage
  GSN:   '#37474f',  // dark slate — gelsolin, corneal lattice + facial palsy
  B2M:   '#004d40',  // dark teal — beta-2 microglobulin, D76N hereditary form
};

const GENE_DISEASE = {
  TTR:   'ATTRv AD — Val30Met/Val122Ile — FAP/FAC — Patisiran/Vutrisiran/Tafamidis Approved — 99mTc-DPD Scan',
  APOA1: 'Hereditary APOA1 Amyloidosis AD — Low HDL PATHOGNOMONIC — Renal/Hepatic/Neuropathic — Liver Transplant',
  APOA2: 'Hereditary APOA2 Amyloidosis AD Stop-Gain — C-Terminal Extension — Renal Dominant — Late-Onset',
  FGA:   'Hereditary Fibrinogen Aα Amyloidosis AD Ostertag-Type — Renal Dominant — No Neuropathy — OLT CURATIVE',
  LYZ:   'Hereditary Lysozyme Amyloidosis AD — Hepatic Rupture Risk — Renal/GI — I56T/W64R — No DMT',
  CST3:  'Hereditary Cystatin C Amyloid Angiopathy Iceland AD — L68Q — Cerebral CAA — Young Stroke 20s–30s',
  GSN:   'Hereditary Gelsolin Amyloidosis Finnish-Type AD — G654A — Corneal Lattice II PATHOGNOMONIC + Facial Palsy',
  B2M:   'Hereditary B2M Amyloidosis AD D76N — Systemic — Distinct from Dialysis-Related — Mass Spectrometry Mandatory',
};
...
function OverviewTab({ data }) {
  ...
  return (
    <div>
      <h2 style={{ color: '#e2e8f0', marginBottom: 4 }}>Hereditary Amyloidosis Atlas</h2>
      <p style={{ color: '#94a3b8', marginBottom: 20, fontSize: 13 }}>
        Complete 8-Gene Hereditary Systemic Amyloidosis Reference — {s.total_patients} patients (8×40, seeds 1622–1629) ·
        Mean diagnosis age {s.mean_dx_age_years}y · Mean delay {s.mean_dx_delay_months} months
      </p>

      {/* KPI row 1 — TTR (most treatable) */}
      <h3 style={{ color: '#94a3b8', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
        TTR — ATTRv Hereditary Transthyretin Amyloidosis (Most Treatable)
      </h3>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
        <KPI label="Carpal Tunnel — First Sign (5–10y Before Neuropathy)" value={`${s.ttr_carpal_tunnel_first_pct}%`} color="#1b5e20" />
        <KPI label="Cardiac Involvement" value={`${s.ttr_cardiac_involvement_pct}%`} color="#f59e0b" />
        <KPI label="Misdiagnosed as CMT/CIDP" value={`${s.ttr_misdiagnosed_pct}%`} color="#dc2626" />
      </div>

      {/* KPI row 2 — CST3 (most dangerous young-onset) */}
      <h3 style={{ color: '#94a3b8', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
        CST3 — Hereditary Cystatin C Amyloid Angiopathy (Young Cerebral Haemorrhage)
      </h3>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
        <KPI label="Cerebral Haemorrhage as First Presentation" value={`${s.cst3_young_haemorrhage_pct}%`} color="#b71c1c" />
        <KPI label="Mortality at First Bleed" value={`${s.cst3_mortality_first_bleed_pct}%`} color="#ef4444" />
      </div>

      {/* KPI row 3 — GSN (diagnostic triad) */}
      <h3 style={{ color: '#94a3b8', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
        GSN — Hereditary Gelsolin Amyloidosis (Finnish Type / Meretoja Disease)
      </h3>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
        <KPI label="Corneal Lattice Dystrophy Type II (PATHOGNOMONIC)" value={`${s.gsn_corneal_lattice_pct}%`} color="#37474f" />
        <KPI label="Facial Nerve Palsy" value={`${s.gsn_facial_palsy_pct}%`} color="#607d8b" />
      </div>

      {/* KPI row 4 — FGA (curative) */}
      <h3 style={{ color: '#94a3b8', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
        FGA — Fibrinogen Aα Amyloidosis (Liver Transplant Curative) · LYZ — Lysozyme (Hepatic Rupture)
      </h3>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
        <KPI label="FGA Renal Function Stable Post-OLT" value={`${s.fga_curative_lt_pct}%`} color="#e65100" />
        <KPI label="LYZ Hepatic Rupture Risk" value={`${s.lyz_hepatic_rupture_risk_pct}%`} color="#880e4f" />
      </div>

      {/* Gene cards */}
      <h3 style={{ color: '#94a3b8', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
        8 Genes — Hereditary Amyloidosis Reference
      </h3>
...
// ── MAIN PAGE ─────────────────────────────────────────────────────────────────
export default function HereditaryAmyloidosisAtlasPage() {
  ...
  useEffect(() => {
    const base = `${API}/api/hereditary-amyloidosis-atlas`;
    Promise.all([
    ...
    <div style={PAGE}>
      <div style={HEADER}>
        <h1 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: '#e2e8f0' }}>
          🧬 Hereditary Amyloidosis Atlas
        </h1>
        <p style={{ margin: '4px 0 0', color: '#94a3b8', fontSize: 12 }}>
          Complete 8-Gene Hereditary Systemic Amyloidosis Reference — TTR · APOA1 · APOA2 · FGA · LYZ · CST3 · GSN · B2M — 320 patients (seeds 1622–1629)
        </p>
      </div>
      <div style={CONTENT}>
