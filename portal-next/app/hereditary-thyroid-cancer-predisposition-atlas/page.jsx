...
const GENE_INFO = {
  RET:     { full: "REarranged during Transfection (RET kinase)",         locus: "10q11.21", size: "1114 aa / 124 kDa",  inh: "AD GOF" },
  PTEN:    { full: "Phosphatase and tensin homologue",                   locus: "10q23.31", size: "403 aa / 47 kDa",    inh: "AD LOF" },
  APC:     { full: "Adenomatous polyposis coli",                         locus: "5q22.2",   size: "2843 aa / 310 kDa",  inh: "AD LOF" },
  DICER1:  { full: "DICER1 RNase III endoribonuclease",                  locus: "14q32.13", size: "1922 aa / 219 kDa",  inh: "AD LOF" },
...
export default function HereditaryThyroidCancerAtlasPage() {
  ...
  useEffect(() => {
    const base = "/api/hereditary-thyroid-cancer-predisposition-atlas";
    Promise.all([
  ...
  if (loading) return <div className="p-6 text-white">Loading Hereditary Thyroid Cancer Predisposition Atlas…</div>;
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 24, borderBottom: "2px solid #e74c3c", paddingBottom: 16 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#e74c3c", margin: 0 }}>
            🦋 Hereditary Thyroid Cancer Predisposition Atlas
          </h1>
          <div style={{ fontSize: 12, color: "#95a5a6", marginTop: 4 }}>
            Complete 8-Gene Reference · RET-PTEN-APC-DICER1-MEN1-PRKAR1A-TP53-NF1 ·{" "}
            320-Patient Aggregate (8×40, seeds 3326–3333)
          </div>
        </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              <thead>
                <tr style={{ background: "#1a1d2e", color: "#7f8c8d" }}>
                  {["Gene", "Full Name", "Locus", "Size", "Inheritance", "Syndrome", "TC Risk", "Key Rule"].map(h => (
                ...
                {breakdown?.genes?.map((g, i) => {
                  ...
                  return (
                    <tr key={g} style={{ background: i % 2 === 0 ? "#0f1117" : "#1a1d2e" }}>
                      ...
                      <td style={{ padding: "8px 10px", color: "#2ecc71", fontSize: 10 }}>{bd?.tc_risk?.substring(0, 55)}…</td>
        ...
        <div style={{ marginTop: 32, paddingTop: 16, borderTop: "1px solid #2c3e50", fontSize: 10, color: "#4a5568", textAlign: "center" }}>
          Hereditary Thyroid Cancer Predisposition Atlas · 8-Gene Reference (RET-PTEN-APC-DICER1-MEN1-PRKAR1A-TP53-NF1) ·
          320 patients (8×40, seeds 3326–3333) · Agenticfinder Expert Dashboards
        </div>
      </div>
    </div>
  );
