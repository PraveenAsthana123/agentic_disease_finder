...
const ACCENT  = '#00695c';   // deep teal — heparan sulfate / CNS-dominant LSD
...
const ETIOLOGY_COLORS = {
  'Compound Heterozygous — Null + Missense': '#00695c',
  ...
  return (
    <>
      <div className="alert mb-4 text-white fw-bold" style={{ background: ACCENT }}>
        🧬 NAGLU / MPS IIIB — Sanfilippo Syndrome B (N-Acetyl-Alpha-Glucosaminidase Deficiency)
      ...
      <div className="alert border-2 mb-4" style={{ borderColor: ACCENT2, background: '#ffebee' }}>
        ...
        <div className="mt-1 small">
          ...
          MPS IIIB has NO approved ERT. Enrol eligible patients in <strong>AAV9-NAGLU intrathecal gene therapy
          trial NCT03315232</strong> (Lysogene / Nationwide Children's Hospital). HSCT is NOT recommended
          in established MPS IIIB (unlike MPS I/II — neurological decline continues post-HSCT in MPS III).
        </div>
      </div>
      ...
      <div className="alert border-2 mb-4" style={{ borderColor: ACCENT5, background: '#f3e5f5' }}>
        ...
        <div className="mt-1 small">
          Sleep disorder (90%+) is the cardinal trigger for seizure clusters in MPS IIIB — hypothalamic
          SCN HS accumulation disrupts circadian rhythm → sleep fragmentation → lowered seizure threshold.
          Melatonin 2–10 mg at bedtime is strongly indicated in ALL MPS IIIB patients with sleep disorder
...
function TreatmentsTab({ data }) {
  ...
  return (
    <>
      <div className="alert border-2 mb-4" style={{ borderColor: ACCENT2, background: '#ffebee' }}>
        <strong style={{ color: ACCENT2 }}>NO APPROVED DISEASE-MODIFYING THERAPY FOR MPS IIIB (2026):</strong>
        <div className="mt-1 small">
          ERT not approved (BBB limits CNS penetration). HSCT NOT recommended in established disease.
          Gene therapy (AAV9-NAGLU intrathecal, NCT03315232) is the current best investigational option.
  ...
  return (
    <>
      ...
      {definitions.length > 0 && (
        <SectionCard title="NAGLU / MPS IIIB Glossary (12 Terms)" color={ACCENT}>
          {definitions.map((d, i) => (
            <div key={i} className="mb-3 p-2 rounded" style={{ background: '#e0f2f1' }}>
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function NagluPage() {
  ...
  useEffect(() => {
    fetch(`${API}/api/naglu/overview`).then(r => r.json()).then(setOverview).catch(e => setErr(String(e)));
  }, []);

  useEffect(() => {
    if (tab === 1 || tab === 2 || tab === 3) {
      fetch(`${API}/api/naglu/breakdown`).then(r => r.json()).then(setBreakdown).catch(() => {});
    }
    if (tab === 4) {
      fetch(`${API}/api/naglu/definitions`).then(r => r.json()).then(setDefinitions).catch(() => {});
    }
  }, [tab]);
  ...
  return (
    <div className="container-fluid py-4" style={{ maxWidth: 1200 }}>
      ...
      <p className="text-muted small mb-3">
        OMIM 252920 / 609701 · 17q21.2 · AR biallelic LOF · No approved ERT (2026) ·
        AAV9-NAGLU gene therapy Phase I/II (NCT03315232) · HSCT NOT recommended · 40-patient cohort
      </p>
