...
const GENE_COLORS = {
  COL1A1:  '#b71c1c',   // deep red — OI Type I
  COL1A2:  '#880e4f',   // deep pink — OI Type III/IV
  FGFR3:   '#4a148c',   // deep purple — Achondroplasia
  EXT1:    '#1a237e',   // deep navy — HME1
  EXT2:    '#0d47a1',   // deep blue — HME2
  SLC26A2: '#006064',   // deep teal — Diastrophic Dysplasia
  RMRP:    '#1b5e20',   // deep green — CHH
  COMP:    '#e65100',   // deep orange — PSACH/MED
};
...
function Loading() {
  return (
    <div className="text-center py-5">
      ...
      <p className="mt-3 text-muted">Loading skeletal dysplasia atlas…</p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="rounded-3 p-4 mb-4 text-white" style={{ background: 'linear-gradient(135deg,#b71c1c,#4a148c)' }}>
        <h3 className="mb-1">{ov.atlas_name || 'Skeletal Dysplasia Atlas'}</h3>
...
function GeneTableTab({ data }) {
  ...
  return (
    <div>
      <h5 className="mb-3">8-Gene Hereditary Skeletal Dysplasia Reference Table</h5>
          ...
          <tbody>
            {genes.map((g) => (
              <tr key={g.gene}>
                ...
                <td className="text-center" style={{ color: g.cohort_stats?.drug_error_pct > 15 ? '#b71c1c' : '#1b5e20' }}>
                  {g.cohort_stats?.drug_error_pct}%
                </td>
                <td className="text-center" style={{ color: g.cohort_stats?.dx_delayed_pct > 30 ? '#b71c1c' : '#1b5e20' }}>
            ...
            <div className="card-body">
              <div className="row">
                <div className="col-md-6">
                  ...
                  <BarRow label="HSCT/Transplant" pct={gene.cohort_stats?.transplant_pct}    color="#1b5e20" />
                  <BarRow label="Surveillance OK" pct={gene.cohort_stats?.surveillance_adherent_pct} color="#1565c0" />
                </div>
                <div className="col-md-6">
...
function DefinitionsTab({ data }) {
  ...
  return (
    <div>
      <h5 className="mb-3">Clinical Definitions — Hereditary Skeletal Dysplasia Terms</h5>
      {defs.map((d, i) => (
...
/* ── Main page ─────────────────────────────────────────────────────────── */
export default function SkeletalDysplasiaAtlasPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/skeletal-dysplasia-atlas/overview`).then(r => r.json()),
      fetch(`${API}/api/skeletal-dysplasia-atlas/breakdown`).then(r => r.json()),
      fetch(`${API}/api/skeletal-dysplasia-atlas/definitions`).then(r => r.json()),
    ])
      .then(([ov, bd, df]) => {
        setOverview(ov);
        setBreakdown(bd);
