...
const GENE_COLORS = {
  BBS1:     '#1565c0',   // deep blue — BBSome core
  BBS10:    '#0277bd',   // ocean blue — chaperonin complex
  CEP290:   '#6a1b9a',   // deep violet — pan-ciliopathy
  NPHP1:    '#ad1457',   // deep rose — nephronophthisis
  DNAI1:    '#00695c',   // deep teal — PCD/ODA
  DYNC2H1:  '#4e342e',   // deep brown — IFT dynein/JATD
  TMEM67:   '#2e7d32',   // deep green — TZ/hepatic
  RPGRIP1L: '#e65100',   // deep orange — coloboma/TZ
};
...
function Loading() {
  return (
    <div className="text-center py-5">
      ...
      <p className="mt-3 text-muted">Loading ciliopathy atlas…</p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="rounded-3 p-4 mb-4 text-white" style={{ background: 'linear-gradient(135deg,#1565c0,#6a1b9a)' }}>
        <h3 className="mb-1">{ov.atlas_name || 'Ciliopathy Atlas'}</h3>
      ...
      <div className="card border-0 shadow-sm mb-4">
        <div className="card-body">
          <h6 className="fw-semibold mb-3">Gene Colour Legend</h6>
          <div className="d-flex flex-wrap gap-2">
            {Object.entries(GENE_COLORS).map(([gene, color]) => (
              <span key={gene} className="badge text-white px-3 py-2" style={{ backgroundColor: color }}>{gene}</span>
            ))}
          </div>
        </div>
      </div>
...
function GeneTableTab({ data }) {
  if (!data) return <Loading />;
  // breakdown is a dict keyed by gene; convert to array
...
/* ── Main page ────────────────────────────────────────────────────────── */
export default function CiliopathyAtlasPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/ciliopathy-atlas/overview`).then(r => r.json()),
      fetch(`${API}/api/ciliopathy-atlas/breakdown`).then(r => r.json()),
      fetch(`${API}/api/ciliopathy-atlas/definitions`).then(r => r.json()),
    ])
      .then(([ov, bd, df]) => {
        setOverview(ov);
        setBreakdown(bd);
