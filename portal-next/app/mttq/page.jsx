...
const COLOR  = '#00695c';   // deep teal — tRNA-Gln / L-strand / FIFTH tRNA
const LIGHT  = '#e0f2f1';
const COLOR2 = '#00897b';   // medium teal — CPEO / myopathy
...
const COLOR5 = '#1a237e';   // deep indigo — QARS2 DDx / nuclear
...
function OverviewTab({ data }) {
  ...
  const ngs_alert = data.ngs_pitfall_alert || {};
  const junc_alert = data.junction_overlap_alert || {};
  ...
  return (
    <>
      ...
      {/* L-strand NGS pitfall — prominent alert */}
      <div className="alert border-0 mb-4" style={{ backgroundColor: '#fce4ec', borderLeft: `6px solid ${COLOR3}` }}>
        <strong style={{ color: COLOR3 }}>⚠️ L-STRAND NGS PITFALL — MANDATORY QC:</strong>
        <br />
        MT-TQ is encoded on the <strong>L-strand</strong> (same as MT-TE, MT-TP, MT-ND6).
        Standard H-strand variant calling <strong>WILL MISS all pathogenic MT-TQ variants</strong>.
        Laboratories MUST apply L-strand reverse-complement decoding and verify L-strand depth at
        rCRS 4329–4400 (min ≥5000×) before reporting any result for this region.
      </div>

      {/* MT-TI / MT-TQ junction overlap alert */}
      <div className="alert border-0 mb-4" style={{ backgroundColor: '#e8eaf6', borderLeft: `6px solid ${COLOR5}` }}>
        <strong style={{ color: COLOR5 }}>🔗 MT-TI / MT-TQ JUNCTION OVERLAP — rCRS 4329–4331:</strong>
        <br />
        The 5′ start of MT-TQ overlaps the 3′ end of MT-TI at rCRS 4329–4331 —
        the densest tRNA-gene overlap in the human mitochondrial genome. Large mtDNA deletions
        spanning this junction <strong>simultaneously destroy tRNA-Ile AND tRNA-Gln</strong>,
        producing compound CPEO more severe than single-tRNA loss.
        Always co-evaluate MT-TI status when reporting MT-TQ large deletions.
      </div>

      {/* KPIs */}
      <SectionCard title="40-Patient Cohort — Key Statistics (seed-809)" borderColor={COLOR2}>
      ...
      {/* Gene facts */}
      <SectionCard title="Gene Facts — MT-TQ" borderColor={COLOR}>
...
function VariantsTab({ data }) {
  ...
  return (
    <>
      <SectionCard title="Variant Breakdown — 40-patient cohort (seed-809)" borderColor={COLOR}>
        <div className="table-responsive">
...
function DdxTab({ data }) {
  ...
  return (
    <>
      <SectionCard title="Differential Diagnosis Table" borderColor={COLOR5}>
        <div className="table-responsive">
          <table className="table table-sm table-hover small mb-0">
            ...
            <tbody>
              {ddx.map((d, i) => (
                <tr key={i} style={d.entity.includes('MT-TQ') ? { backgroundColor: LIGHT, fontWeight: 600 } : {}}>
                  <td style={{ color: d.entity.includes('MT-TQ') ? COLOR : 'inherit' }}>{d.entity}</td>
                  <td>{d.inheritance}</td>
                  <td>{d.phenotype}</td>
      ...
      <SectionCard title="KEY DDx Exclusion Rules — MT-TQ" borderColor={COLOR3}>
        {[
          { rule: 'NO stroke-like episodes', note: 'Stroke-like episodes (SLE) → MT-TL1 / MELAS (m.3243A>G); NOT MT-TQ' },
          { rule: 'NO myoclonic epilepsy', note: 'Myoclonic epilepsy + MSL → MT-TK / MERRF; NOT MT-TQ' },
          { rule: 'NO isolated HCM without CPEO', note: 'Isolated HCM at low heteroplasmy → MT-TI (m.4300A>G); NOT MT-TQ' },
          { rule: 'NO MIDM', note: 'Maternally inherited diabetes + deafness → MT-TE (m.14709T>C); NOT MT-TQ' },
          { rule: 'BTBGD MANDATORY EXCLUSION', note: 'SLC19A3 biallelic → biotin+thiamine-responsive Leigh mimic; treat first before attributing Leigh MRI to MT-TQ' },
          { rule: 'QARS2 DDx for neonatal CI+CIV + encephalopathy', note: 'Neonatal/infantile onset + encephalopathy + CI+CIV → add WES for QARS2; AR not maternal' },
        ].map((item, i) => (
          <div key={i} className="alert mb-2 py-2 border-0" style={{ backgroundColor: i % 2 === 0 ? '#fce4ec' : '#e8eaf6' }}>
      ...
      <SectionCard title="Perioperative / Emergency Protocol" borderColor={COLOR2}>
        <ul className="mb-0 small">
          ...
          <li><strong>NEVER fast</strong> MT-TQ patients without IV glucose support</li>
          ...
          <li><strong>L-strand NGS pitfall</strong> — ensure anaesthetics team aware; document in surgical consent</li>
        </ul>
      </SectionCard>
      ...
      <SectionCard title="Surveillance Schedule by System" borderColor={COLOR}>
        <div className="table-responsive">
          <table className="table table-sm table-bordered small mb-0">
            ...
            <tbody>
              {[
                { sys: 'Ophthalmology', test: 'Orthoptic exam, ptosis assessment', freq: 'Annual', trigger: 'All MT-TQ patients' },
                { sys: 'Neurology',     test: 'Clinical + lactate + CK', freq: 'Annual',  trigger: 'All MT-TQ patients' },
                { sys: 'Cardiology',    test: 'Echo + Holter', freq: 'Annual', trigger: 'm.4395AG or any cardiomyopathy' },
                { sys: 'Audiology',     test: 'Pure-tone audiogram', freq: 'Annual', trigger: 'm.4370TC or SNHL' },
                { sys: 'Neurology MRI', test: 'Brain MRI with DWI', freq: 'PRN', trigger: 'm.4363AG + high hetero; Leigh-like screening' },
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function MttqPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/mttq/overview`).then(r => r.json()),
      fetch(`${API}/api/mttq/breakdown`).then(r => r.json()),
      fetch(`${API}/api/mttq/definitions`).then(r => r.json()),
    ]).then(([ov, bk, df]) => {
      setOverview(ov);
      setBreakdown(bk);
      setDefs(df);
  ...
  return (
    <div className="container-fluid py-3">
      <div className="d-flex align-items-center gap-2 mb-3">
        ...
        <div>
          <h3 className="fw-bold mb-0" style={{ color: COLOR }}>MT-TQ — tRNA-Gln Expert Dashboard</h3>
          <small className="text-muted">
            Combined CI+CIV Deficiency · CPEO · Myopathy ·{' '}
            <strong style={{ color: COLOR3 }}>L-strand NGS Pitfall</strong> ·
            FIFTH tRNA · rCRS 4329–4400 · 40-patient cohort seed-809
          </small>
        </div>
      </div>
